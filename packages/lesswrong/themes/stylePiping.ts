...
const spoilerStyles = (theme: ThemeType): JssStyles => ({
  ...
  "& .spoiler": {
    ...
    "&:not(:hover)": {
      // using ':not(:hover)' means we don't need to manually reset elements with special colors or backgrounds, instead they just automatically stay the same if we're not hovering
      ...hideSpoilers(theme),
    },
  },
