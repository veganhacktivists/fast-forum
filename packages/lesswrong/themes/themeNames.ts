...
export function isValidSerializedThemeOptions(options: string | object): options is string | AbstractThemeOptions {
  try {
    if (typeof options === "object") {
      ...
      if (isValidUserThemeSetting(optionsObj.name)) return true;
    } else {
      const deserialized = JSON.parse(options as string);
      if (isValidUserThemeSetting(deserialized.name)) return true;
    }
    return false;
  } catch (e) {
...
export const isValidUserThemeSetting = (name: string): name is UserThemeSetting => userThemeSettings.has(name);

export const resolveThemeName = (theme: UserThemeSetting, prefersDarkMode: boolean): UserThemeName =>
  theme === "auto" ? (prefersDarkMode ? "dark" : "default") : theme;

export const abstractThemeToConcrete = (theme: AbstractThemeOptions, prefersDarkMode: boolean): ThemeOptions =>
  themeOptionsAreConcrete(theme) ? theme : { ...theme, name: prefersDarkMode ? "dark" : "default" };
...
export const getDefaultThemeOptions = (): AbstractThemeOptions => defaultThemeOptions.get();

const deserializeThemeOptions = (themeOptions: object | string): AbstractThemeOptions => {
  if (typeof themeOptions === "string") {
    return isValidUserThemeSetting(themeOptions) ? { name: themeOptions } : JSON.parse(themeOptions);
  } else {
...
export const getThemeOptions = (
  themeCookie: string | object,
  user: DbUser | UsersCurrent | null,
): AbstractThemeOptions => deserializeThemeOptions(getSerializedThemeOptions(themeCookie, user));
