...
const TagVersionHistoryButton = ({ tagId, classes }: { tagId: string; classes: ClassesType }) => {
          ...
          {revision && canRevert && (
            <div className={classes.restoreButton}>
              {revertLoading ? (
                ...
                >
                  RESTORE THIS VERSION {revertInProgress && <Loading />}
                </Button>
              )}
            </div>
          )}
...
const TagVersionHistoryButtonComponent = registerComponent("TagVersionHistoryButton", TagVersionHistoryButton, {
  styles,
});
