...
const insertNewLine = (
  ...
  block: ContentBlock,
  insertAt: "after" | "before" | "both" = "after",
) => {
  ...
  const withUnstyledBlock = Modifier.replaceWithFragment(contentState, selectionState, fragment);
...
export const insertNewLineAfter = (editorState: EditorState, block: ContentBlock) => insertNewLine(editorState, block);
