...
function _saveInlineTeX({ after, contentState, teX, displaystyle, entityKey, blockKey, startPos }) {
...
function _saveBlockTeX({ after, contentState, teX, block }) {
  const needRemove = teX.length === 0;
  const blockKey = block.getKey();
  ...
  if (needRemove) {
    ...
    newContentState = Modifier.mergeBlockData(contentState, SelectionState.createEmpty(blockKey), Map({ teX }));
...
export function saveTeX({ block, entityKey, displaystyle, blockKey, startPos, ...common }) {
  return entityKey
