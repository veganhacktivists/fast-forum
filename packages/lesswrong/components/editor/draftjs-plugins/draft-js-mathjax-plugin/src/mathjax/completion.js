...
const computeCompletionList = (cmdPrefix, commands, mostUsedCommands = {}) => {
  ...
  list.sort((c1, c2) => {
    const w1 = (Object.prototype.hasOwnProperty.call(mostUsedCommands, c1) && mostUsedCommands[c1]) || 0;
    const w2 = (Object.prototype.hasOwnProperty.call(mostUsedCommands, c2) && mostUsedCommands[c2]) || 0;
...
function getAllTeX(contentState) {
  ...
  allTeX = blockMap.filter((b) => b.getData().mathjax).reduce((red, b) => red + b.getData().teX, allTeX);
...
export default (status, macros) => (editorState) => ({
  ...
  updateMostUsedTeXCmds(teX, lastTex = "") {
    this.mostUsedTeXCommands = updateMostUsedTeXCmds(teX, this.mostUsedTeXCommands, lastTex);
  },
  computeCompletionList(prefix) {
    return computeCompletionList(prefix, this.teXCommandsAndMacros, this.mostUsedTeXCommands);
  },
});
