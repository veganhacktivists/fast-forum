...
export function isCollaborative(post: DbPost, fieldName: string): boolean {
  ...
  if (post.sharingSettings?.anyoneWithLinkCan && post.sharingSettings.anyoneWithLinkCan !== "none") return true;
...
export const EditorFormComponent = (
  ...
  const [contents, setContents] = useState(() => getInitialEditorContents(value, document, fieldName, currentUser));
  ...
  const showEditorWarning =
    updatedFormType !== "new" && (currentEditorType === "html" || currentEditorType === "draftJS");
  ...
  // On the EA Forum, our bot checks if posts are potential criticism,
  // and if so we show a little card with tips on how to make it more likely to go well.
  const checkIsCriticism = useCallback(
  ...
  const saveRemoteBackup = useCallback(
    async (newContents: EditorContents) => {
   ...
   */
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const throttledSetContentsValue = useCallback(
    _.throttle(
      async () => {
        ...
        updateCurrentValues({
          [fieldName]: { ...(document[fieldName] || {}), ...(await editorRef.current.submitData()) },
        });
      },
      autosaveInterval,
      { leading: true },
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const throttledSaveBackup = useCallback(_.throttle(saveBackup, autosaveInterval, { leading: false }), [
    saveBackup,
    autosaveInterval,
  ]);
  ...
  const wrappedSetContents = useCallback(
  ...
  const fieldHasCommitMessages =
    editableCollectionsFieldOptions[collectionName as CollectionNameString][fieldName].revisionsHaveCommitMessages;
  ...
  const actualPlaceholder =
    (collectionName === "Posts" && getPostPlaceholder(document)) || editorHintText || hintText || placeholder;
