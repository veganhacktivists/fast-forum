...
const SpreadsheetPage = ({ classes }: { classes: ClassesType }) => {
  const {
    query: { tab: selectedTab = "Intro" },
    hash: selectedCell,
  } = useLocation();
  ...
  const linkCell = (url: string, link: string, domain: string, type: string) => (
    <div>
      <div className={classes.link}>
        <HoverPreviewLink href={url}>{link}</HoverPreviewLink>
      </div>
      {domain && (
        <div className={classes.domain}>
          {domain} {type && <span>• {type}</span>}
        </div>
      )}
    </div>
  );
  ...
  const tabs = [
    ...
    },
    {
      ...
      description:
        "Websites that attempt to gently introduce coronavirus or explain things about it. Typically non-exhaustive.",
    ...
    },
    {
      ...
      description:
        "Information about current or predicted prevalence, how COVID-19 is spread, or lower the former/preventing the latter. We may need a prevalence model olympics",
      rows: _.filter(sortedRowsImp, (row: any): boolean => row.category === "Spread & Prevention"),
    },
    {
      label: "Science",
      description:
        "Basic science that does not immediatley prompt action. E.g. 'here's what SC2 targets' is in, 'Here's a drug targeting that' is out.",
      rows: _.filter(sortedRowsImp, (row: any): boolean => row.category === "Science"),
    },
    {
      label: "Medical System",
      description:
        "Information or models on the current state of the medical system, including 'here's how bad hospitals are' and potential treatments.",
  ...
  return (
    <div className={classes.root}>
      <HeadTags
        image={
          "https://res.cloudinary.com/lesswrong-2-0/image/upload/v1585093292/Screen_Shot_2020-03-24_at_4.41.12_PM_qiwqwc.png"
        }
      />
      {selectedTab === "Intro" && (
        <ContentStyles contentType="comment" className={classes.introWrapper}>
          <div className={classes.intro}>
            <p>
              Welcome to the Coronavirus Info-Database, an attempt to organize the disparate papers, articles and links
              that are spread all over the internet regarding the nCov pandemic. We sort, summarize and prioritize all
              links on a daily basis. You can submit new links by pressing the big green button.
            </p>
            <p>
              You can find (and participate) in more LessWrong discussion of COVID-19 on{" "}
              <HoverPreviewLink href={"/tag/coronavirus"}>{"our tag page"}</HoverPreviewLink>.
            </p>
          </div>
          <a
            href="https://docs.google.com/forms/d/e/1FAIpQLSc5uVDXrowWmhlaDbT3kukODdJotWOZXZivdlFmaHQ6n2gsKw/viewform"
      ...
      <div className={classes.tabRow}>
        <LWTooltip key={"Intro"} placement="top" title={<div>{"What is this table about?"}</div>}>
          <QueryLink query={{ tab: "Intro" }}>
            <div className={classNames(classes.tab, { [classes.tabSelected]: selectedTab === "Intro" })}>
              <span className={classes.tabLabel}>INTRO</span>
            </div>
          </QueryLink>
        </LWTooltip>
        {tabs.map((tab) => (
          ...
          >
            <QueryLink query={{ tab: tab.label }}>
              <div className={classNames(classes.tab, { [classes.tabSelected]: tab.label === selectedTab })}>
                <div className={classes.tabLabel}>{tab.displayLabel || tab.label}</div>
                <div className={classes.tabCount}>{tab.rows.length - 1} links</div>
              </div>
            </QueryLink>
          </LWTooltip>
        ))}
          ...
          {selectedTab !== "Intro" ? (
            <TableBody>
              <TableRow key={`row-0`}>
                ...
                <TableCell classes={{ root: classes.leftFixedHeader1 }}>Link</TableCell>
                <TableCell classes={{ root: classes.headerCell }}>Summary</TableCell>
                {currentTab?.showCategory && (
                  <TableCell classes={{ root: classes.headerCellCategory }}>Category</TableCell>
                )}
                <TableCell classes={{ root: classes.headerDateAdded }}>Date Added</TableCell>
                <TableCell classes={{ root: classes.headerLastUpdated }}>Last Updated</TableCell>
                <TableCell classes={{ root: classes.headerTitle }}>Name / Opening Sentence</TableCell>
              </TableRow>
              {rows.map(
                (
                  {
                  ...
                  <TableRow
                    ...
                    className={selectedCell === `#${encodeURIComponent(url)}` ? classes.selectedRow : ""}
                  >
                    <TableCell classes={{ root: classes.leftFixed0 }}>
                      <Link to={{ hash: encodeURIComponent(url), search: `?${qs.stringify({ tab: selectedTab })}` }}>
                        {imp}
                      </Link>
                    </TableCell>
                    <TableCell className={classes.leftFixed1}>
                      {linkCell(url, link, domain, type)}
                      <span className={classes.smallDescription}>{description}</span>
                    </TableCell>
                    <TableCell classes={{ root: classes.cellDescription }}>
                      <span className={classes.description}>{description}</span>
                      {reviewerThoughts && <span className={classes.reviewerThoughts}>{reviewerThoughts}</span>}
                    </TableCell>
                    {currentTab?.showCategory && (
                      <TableCell classes={{ root: classes.cellCategory }}>{category}</TableCell>
                    )}
                    <TableCell classes={{ root: classes.cellDate }}>{dateAdded}</TableCell>
                    <TableCell classes={{ root: classes.cellDate }}>{lastUpdated}</TableCell>
            ...
            <TableBody>
              <TableRow>
                <TableCell className={classes.headerSheet}>Sheet</TableCell>
                <TableCell className={classes.headerCellDescription}>Description</TableCell>
                <TableCell className={classes.headerCell}>Top Links</TableCell>
              </TableRow>
              {tabs.map((tab) => {
                if (tab.label === "All Links") return null;
                return (
                  <TableRow key={`intro-${tab.label}`} className={classes.categoryRow}>
                    ...
                    <TableCell className={classes.cellSheetDescription}>{tab.description}</TableCell>
                    <TableCell className={classes.topLinks}>
                      {[0, 1, 2].map((rowNum) => (
                        <div className={classes.topLinkRow} key={`topLink-${tab.label}-${rowNum}`}>
                          <div className={classes.topLink}>
                            {tab.rows[rowNum] &&
                              linkCell(
                                tab.rows[rowNum].url,
                                tab.rows[rowNum].link,
                                tab.rows[rowNum].domain,
                                tab.rows[rowNum].type,
                              )}
                          </div>
                          <div className={classes.topLinkDescription}>{tab.rows[rowNum]?.description}</div>
                        </div>
                      ))}
                    </TableCell>
                  </TableRow>
