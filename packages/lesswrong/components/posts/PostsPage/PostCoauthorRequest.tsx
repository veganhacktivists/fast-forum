...
const PostCoauthorRequest = ({
  ...
  const onResponse = async (accept: boolean) => {
    setLoading(true);
    const { errors } = await acceptCoauthorRequest({
      variables: {
        postId: post._id,
...
const PostCoauthorRequestComponent = registerComponent("PostCoauthorRequest", PostCoauthorRequest, { styles });
