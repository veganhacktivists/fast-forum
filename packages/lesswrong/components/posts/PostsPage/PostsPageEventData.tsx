...
const PostsPageEventData = ({ classes, post }: { classes: ClassesType; post: PostsList }) => {
  ...
  const eventTypeNode = (Icon: EventTypeIcons[keyof EventTypeIcons], text: string) => (
  ...
  const afterEvent =
    (post.endTime && moment(post.endTime).isBefore(now)) ||
  ...
  let eventCTA = joinEventLink && (
    <Button variant="contained" color="primary" href={joinEventLink} disabled>
      Join Event
    </Button>
  );
  ...
  if (!eventCTA && post.facebookLink) {
    eventCTA = (
      <Button
        variant={afterEvent ? "outlined" : "contained"}
        color="primary"
  ...
  return (
    <Components.Typography variant="body2" className={classes.metadata}>
      <div>
        ...
        {eventType &&
          eventType in eventTypeIcons &&
          eventTypeNode(eventTypeIcons[eventType as keyof EventTypeIcons], eventType)}
        {eventCTA && post.startTime && !post.onlineEvent && <div className={classes.inPersonEventCTA}>{eventCTA}</div>}
      </div>
      {eventCTA && post.startTime && post.onlineEvent && <div className={classes.onlineEventCTA}>{eventCTA}</div>}
      {!post.onlineEvent && (
