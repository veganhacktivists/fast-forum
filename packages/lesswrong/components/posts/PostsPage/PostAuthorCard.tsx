...
const PostAuthorCard = ({
  ...
  return (
    <AnalyticsContext pageSectionContext="postAuthorCard">
      <div className={classes.root}>
        <Typography variant="subheading" component="div" className={classes.about}>
          About the author
        </Typography>
        <div className={classes.usernameRow}>
          {isFriendlyUI && author.profileImageId && (
            <Link to={`/users/${author.slug}?from=post_author_card`} className={classes.photoLink}>
              <CloudinaryImage2
                height={40}
                width={40}
                imgProps={{ q: "100" }}
          ...
          <Typography variant="headline" component="div" className={classes.username}>
            <Link to={`/users/${author.slug}?from=post_author_card`}>{author.displayName}</Link>
          </Typography>
          <div className={classes.btns}>
            {currentUser?._id !== author._id && (
              <NewConversationButton user={author} currentUser={currentUser} from="post_author_card">
        ...
        {author.biography?.html && (
          <ContentStyles contentType="comment" className={classes.bio}>
