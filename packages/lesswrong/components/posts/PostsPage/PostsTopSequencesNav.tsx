...
const PostsTopSequencesNav = ({ post, classes }: { post: PostSequenceNavigation; classes: ClassesType }) => {
    ...
    (ev) => {
      ...
      if (ev.shiftKey && !ev.ctrlKey && !ev.altKey && !ev.metaKey) {
        ...
        if (ev.target === document.body || (ev.target && (ev.target as any).tagName === "A")) {
          if (ev.keyCode === 37) {
            // Left
            if (post.prevPost) navigate(postGetPageUrl(post.prevPost, false, post.prevPost.sequence?._id));
          } else if (ev.keyCode === 39) {
            // Right
            if (post.nextPost) navigate(postGetPageUrl(post.nextPost, false, post.nextPost.sequence?._id));
          }
        }
      }
    },
  ...
  return (
    <div className={classes.root}>
      <SequencesNavigationLink post={post.prevPost} direction="left" />
      ...
      <SequencesNavigationLink post={post.nextPost} direction="right" />
    </div>
  );
};

const PostsTopSequencesNavComponent = registerComponent("PostsTopSequencesNav", PostsTopSequencesNav, {
  styles,
