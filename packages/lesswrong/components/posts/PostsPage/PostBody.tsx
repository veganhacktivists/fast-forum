...
const PostBody = ({
  ...
  if (includeSideComments && document?.sideComments) {
    ...
    const sideComments =
      sideCommentMode === "highKarma"
        ? document.sideComments.highKarmaCommentsByBlock
        : document.sideComments.commentsByBlock;
    const sideCommentsMap = mapValues(sideComments, (commentIds) => (
      <SideCommentIcon post={post} commentIds={commentIds} />
    ));
  ...
  if (enableInlineReactsOnPosts) {
    return (
      <InlineReactSelectionWrapper commentBodyRef={contentRef} voteProps={voteProps} styling="post">
        {content}
