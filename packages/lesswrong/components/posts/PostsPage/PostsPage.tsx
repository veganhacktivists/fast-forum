...
import {
  getConfirmedCoauthorIds,
  isPostAllowedType3Audio,
...
import {
  SideCommentMode,
  SideCommentVisibilityContextType,
  SideCommentVisibilityContext,
} from "../../dropdowns/posts/SetSideCommentVisibility";
...
const POST_DESCRIPTION_EXCLUSIONS: RegExp[] = [/cross-? ?posted/i, /epistemic status/i, /acknowledgements/i];
...
const getStructuredData = ({
  ...
  return {
    ...
    about: post.tags
      .filter((tag) => !!tag.description?.htmlHighlight)
      .map((tag) => ({
        "@type": "Thing",
        name: tag.name,
...
const getDebateResponseBlocks = (responses: CommentsList[], replies: CommentsList[]) =>
  responses.map((debateResponse) => ({
...
const PostsPage = ({
  ...
  const [highlightDate, setHighlightDate] = useState<Date | undefined>(
    post?.lastVisitedAt && new Date(post.lastVisitedAt),
  );
  ...
  const defaultSideCommentVisibility = hasSideComments ? post.sideCommentVisibility ?? "highKarma" : "hidden";
  const [sideCommentMode, setSideCommentMode] = useState<SideCommentMode>(
    defaultSideCommentVisibility as SideCommentMode,
  );
  ...
  const sectionData =
    (post as PostsWithNavigationAndRevision).tableOfContentsRevision || (post as PostsWithNavigation).tableOfContents;
  ...
  const onClickCommentOnSelection = useCallback(
    (html: string) => {
      openDialog({
  ...
  const isCrosspostedQuestion = post.question && post.fmCrosspost?.isCrosspost && !post.fmCrosspost?.hostedHere;
  ...
  const tableOfContents = hasTableOfContents ? <TableOfContents sectionData={sectionData} title={post.title} /> : null;
      ...
      <AnalyticsContext pageSectionContext="postHeader">
        <div className={classes.title}>
          <div className={classes.centralColumn}>
            ...
            {post.eventImageId && (
              <div
                className={classNames(classes.headerImageContainer, {
                  [classes.headerImageContainerWithComment]: commentId,
                })}
              >
                <CloudinaryImage2
  ...
  const postBodySection = (
    <div id="postBody" ref={postBodyRef} className={classes.centralColumn}>
      ...
      {post.socialPreviewData?.imageUrl && (
        <img className={classes.postListItemImage} src={post.socialPreviewData.imageUrl} />
      )}
      ...
      {post.debate && debateResponses && debateResponseReplies && (
        <DebateBody debateResponses={getDebateResponseBlocks(debateResponses, debateResponseReplies)} post={post} />
      )}
    </div>
  );
      ...
      {showRecommendations && recommendationsPosition === "underPost" && (
        <AnalyticsContext pageSectionContext="postBottomRecommendations">
          <div className={classes.recommendations}>
            <PostsPageRecommendationsList strategy="tagWeightedCollabFilter" />
          </div>
        </AnalyticsContext>
      )}
    </div>
  ...
  const { loading, results, loadMore, loadingMore, totalCount } = useEagerResults
    ? eagerPostComments.queryResponse
    : lazyResults;
      ...
      <PostsPageContext.Provider value={post}>
        <SideCommentVisibilityContext.Provider value={sideCommentModeContext}>
          ...
          {commentsTableOfContentsEnabled ? (
            ...
            <ToCColumn tableOfContents={tableOfContents} header={header} rightColumnChildren={rightColumnChildren}>
              {postBodySection}
          ...
          {hasPostRecommendations && (
            <AnalyticsInViewTracker eventProps={{ inViewType: "postPageFooterRecommendations" }}>
...
export type PostParticipantInfo = Partial<
  Pick<PostsDetails, "userId" | "debate" | "hasCoauthorPermission" | "coauthorStatuses">
>;
