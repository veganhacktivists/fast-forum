...
export const contentTypes: ForumOptions<ContentTypeRecord> = {
  LessWrong: {
    frontpage: {
      ...
      tooltipBody: (
        <React.Fragment>
          <p>
            <b>Frontpage Posts</b> are promoted by moderators based on:
          </p>
          <ul>
    ...
    personal: {
      ...
      tooltipBody: (
        <React.Fragment>
          <div>
            <b>Personal Blogpost</b>
          </div>
          <br />
          <div>Members can write whatever they want on their personal blog. Personal blogposts are a good fit for:</div>
          <ul>
            <li>Niche topics</li>
    ...
    curated: {
      ...
      tooltipBody: (
        <div>
          The best 2-3 posts each week, selected by the moderation team. Curated posts are featured at the top of the
          front page and emailed to subscribers.
        </div>
      ),
    ...
    shortform: {
      ...
      tooltipBody: (
        <div>
          Writing that is short in length, or written in a short amount of time. Off-the-cuff thoughts, brainstorming,
          early stage drafts, etc.
        </div>
      ),
    ...
    tags: {
      ...
      tooltipBody: (
        <div>{taggingAltName2} pages, which organize LessWrong posts and concepts in a more durable format.</div>
      ),
      Icon: TagIcon,
  ...
  AlignmentForum: {
    ...
    curated: {
      tooltipTitle: "Curated Post",
      tooltipBody: <div>The best posts, selected by the moderation team.</div>,
      linkTarget: curatedUrl,
      Icon: StarIcon,
    },
    shortform: {
    ...
    tags: {
      ...
      tooltipBody: (
        <div>
          {taggingAltName2} pages, which organize {siteNameWithArticleSetting.get()} posts and concepts in a more
          durable format.
        </div>
      ),
      Icon: TagIcon,
  ...
  EAForum: {
    frontpage: {
      tooltipTitle: "Frontpage Post",
      tooltipBody: <div>Posts that are relevant to doing good effectively.</div>,
    ...
    personal: {
      ...
      tooltipBody: (
        <React.Fragment>
          <div>Users can write whatever they want on their personal blog. This category is a good fit for:</div>
          <ul>
    ...
    shortform: {
      ...
      tooltipBody: (
        <div>
          Writing that is brief, or written very quickly. Perfect for off-the-cuff thoughts, brainstorming, early stage
          drafts, etc.
        </div>
      ),
    ...
    tags: {
      tooltipTitle: `${taggingAltName} Edits and Discussion`,
      tooltipBody: <div>{taggingAltName2} pages, which organize posts and concepts in a more durable format.</div>,
      Icon: TagIcon,
      linkTarget: getAllTagsPath(),
    },
    subforumDiscussion: {
  ...
  default: {
    ...
    tags: {
      ...
      tooltipBody: (
        <div>
          {taggingAltName2} pages, which organize {forumTitleSetting.get()} posts and concepts in a more durable format.
        </div>
      ),
      Icon: TagIcon,
      linkTarget: getAllTagsPath(),
...
const ContentTypeWrapper: FC<{ classes: ClassesType; className?: string }> = ({ classes, className, children }) =>
  isFriendlyUI ? (
    <>{children}</>
  ) : (
    <Components.Typography variant="body1" component="span" className={classNames(classes.root, className)}>
      {children}
...
const ContentType = ({
  ...
  const innerComponent = isFriendlyUI ? (
    ...
    <span>
      <contentData.Icon className={classes.icon} />
      {label ? " " + label : ""}
    </span>
  );
