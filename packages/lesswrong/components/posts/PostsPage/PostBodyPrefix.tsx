...
const PostBodyPrefix = ({
  ...
  const {
    AlignmentCrosspostMessage,
    AlignmentPendingApprovalMessage,
    LinkPostMessage,
    PostsRevisionMessage,
    LWTooltip,
    ReviewVotingWidget,
    ReviewPostButton,
    ContentItemBody,
  ...
  return (
    <>
      {reviewIsActive() && postEligibleForReview(post) && postIsVoteable(post) && (
        <div className={classes.reviewVoting}>
          ...
          <ReviewPostButton
            ...
            reviewMessage={
              <LWTooltip
                title={`Write up your thoughts on what was good about a post, how it could be improved, and how you think stands the tests of time as part of the broader ${forumTitleSetting.get()} conversation`}
                placement="bottom"
              >
      ...
      {post.shortform && post.draft && <div className={classes.contentNotice}>{shortformDraftMessage}</div>}
      {post.shortform && !post.draft && (
        <div className={classes.contentNotice}>
          {isFriendlyUI ? (
            <>
              This is a special post for quick takes by <Components.UsersNameDisplay user={post.user} />. Only they can
              create top-level comments. Comments here also appear on the <Link to="/quicktakes">Quick Takes page</Link>{" "}
              and <Link to="/allPosts">All Posts page</Link>.
            </>
          ) : (
            <>
              This is a special post for short-form writing by <Components.UsersNameDisplay user={post.user} />. Only
              they can create top-level comments. Comments here also appear on the{" "}
              <Link to="/shortform">Shortform Page</Link> and <Link to="/allPosts">All Posts page</Link>.
            </>
          )}
        </div>
      )}
      ...
      {post.rejected && (
        <div className={classes.rejectionNotice}>
      ...
      {!post.rejected && post.authorIsUnreviewed && !post.draft && (
        <div className={classes.contentNotice}>
          ...
          <LWTooltip
            title={
              <p>
                New users' first posts on {siteNameWithArticleSetting.get()} are checked by moderators before they
                appear on the site. Most posts will be approved within {forumNewUserProcessingTime} hours; posts that
                are spam or that don't meet site standards will be deleted. After you've had a post approved, future
                posts will appear immediately without waiting for review.
              </p>
            }
          >
