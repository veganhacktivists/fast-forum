...
const RSVPs = ({
  ...
  const cancelRSVP = async (rsvp: RSVPType) =>
    await cancelMutation({ variables: { postId: post._id, name: rsvp.name, userId: rsvp.userId } });

  const rsvpCounts: Partial<Record<RsvpResponse, number>> = mapValues(
  ...
  return (
    <ContentStyles contentType="post" className={classes.body}>
      <div className={classes.topRow}>
        <span className={classes.rsvpMessage}>The host has requested RSVPs for this event</span>
        <span className={classes.buttons}>
          <Button
            color="primary"
            variant="outlined"
            className={classes.goingButton}
            onClick={() => openRSVPForm("yes")}
          >
      ...
      {post.isEvent && post.rsvps?.length > 0 && (
        <>
          <div className={classes.rsvpCounts}>
            {Object.keys(responseToText).map((response: RsvpResponse) => (
              <span key={response} className={classes.rsvpCount}>
          ...
          <div className={classes.rsvpBlock}>
            {post.rsvps.map((rsvp: RSVPType) => {
              ...
              return (
                <span className={classes.rsvpItem} key={`${rsvp.name}-${rsvp.response}`}>
                  <div>
                    <ResponseIcon response={rsvp.response} classes={classes} />
                    <span className={classes.rsvpName}>{rsvp.name}</span>
                    {canCancel && (
                      <span className={classes.remove} onClick={() => cancelRSVP(rsvp)}>
                        {"x"}
                      </span>
                    )}
                  </div>
...
function ResponseIcon({ response, classes }: { response: RsvpResponse; classes: ClassesType }) {
  switch (response) {
    case "yes":
