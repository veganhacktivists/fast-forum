...
export const usePostsUserAndCoauthors = (post: PostsList | SunshinePostsList | PostsBestOfList) => {
  ...
  let topCommentAuthor = post.question ? post.bestAnswer?.user : post.lastPromotedComment?.user;
  ...
  const authors = [post.user, ...coauthors].filter((user): user is UsersMinimumInfo => !!user);
