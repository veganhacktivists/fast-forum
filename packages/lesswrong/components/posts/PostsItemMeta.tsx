...
const PostsItemMeta = ({ post, read, classes }: { post: PostsList; read?: boolean; classes: ClassesType }) => {
  ...
  return (
    <span className={classNames({ [classes.read]: read })}>
      {!post.shortform && !post.isEvent && (
        <span className={classes.info}>
          <LWTooltip
            title={
              <div>
                This post has {baseScore || 0} karma
                <br />({post.voteCount} votes)
              </div>
            }
          >
            <span className={classes.karma}>{baseScore || 0}</span>
          </LWTooltip>
        </span>
      )}
      ...
      {afBaseScore && (
        <span className={classes.info}>
          <LWTooltip title={<div>{afBaseScore} karma on alignmentforum.org</div>}>
