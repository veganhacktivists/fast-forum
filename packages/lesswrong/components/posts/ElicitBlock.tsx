...
const ElicitBlock = ({ classes, questionId = "IyWNjzc5P" }: { classes: ClassesType; questionId: String }) => {
  ...
  const userHasPredicted =
    currentUser && some(sortedPredictions, (prediction: any) => prediction?.creator?.lwUser?._id === currentUser._id);
    ...
    <ContentStyles contentType="comment" className={classes.root}>
        ...
        {times(10, (bucket) => (
          <div
            ...
            className={classNames(classes.histogramBucket, {
              [classes.histogramBucketCurrentUser]: roughlyGroupedData[`${bucket * 10}`]?.some(
                ({ creator }) => currentUser && creator?.sourceUserId === currentUser._id,
              ),
            })}
          ...
          >
            {times(10, (offset) => {
              ...
              const isCurrentUserSlice = finelyGroupedData[`${prob}`]?.some(
                ({ creator }) => currentUser && creator?.sourceUserId === currentUser._id,
              );
              ...
              return (
                  ...
                  onClick={() => {
                    if (currentUser) {
                      ...
                      const filteredPredictions = predictions.filter(
                        (prediction: any) => prediction?.creator?.sourceUserId !== currentUser._id,
                      );
                      ...
                      const newPredictions = isCurrentUserSlice
                        ? filteredPredictions
                        : [
                ...
                >
                  <div className={classes.additionalVoteArea} style={{ height: `${(1 / (maxSize + 1)) * 100 || 0}%` }}>
                    <div className={classNames(classes.sliceNumber, { [classes.invertedSliceNumber]: prob > 94 })}>
                      {prob}%
                    </div>
                  </div>
                  {!predictionsHidden && (
                    <div
                      ...
                      style={{
                        height: `${(roughlyGroupedData[`${bucket * 10}`]?.length / (maxSize + 1)) * 100 || 0}%`,
                      }}
                    />
                  )}
                </div>
            ...
            {!predictionsHidden && roughlyGroupedData[`${bucket * 10}`] && (
              <div className={classes.usersInBucket}>
                {roughlyGroupedData[`${bucket * 10}`]?.map(({ creator, prediction }, i) => (
                  <span key={creator?._id} className={classes.name}>
                    {creator?.lwUser ? (
                      <UsersName user={creator?.lwUser} tooltipPlacement={"bottom"} />
                    ) : (
                      creator?.displayName
                    )}{" "}
                    ({prediction}%){i !== roughlyGroupedData[`${bucket * 10}`].length - 1 && ","}
                  </span>
                ))}
              </div>
            )}
      ...
      <div className={classNames(classes.titleSection, { [classes.hiddenTitleSection]: hideTitle })}>
        ...
        <div className={classes.title}>
          ...
          {!loading && predictionsHidden && <a onClick={() => setRevealed(true)}> (Reveal)</a>}
        </div>
