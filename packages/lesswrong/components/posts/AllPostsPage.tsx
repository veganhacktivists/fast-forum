...
const AllPostsPage = ({ classes }: { classes: ClassesType }) => {
  ...
  const currentShowLowKarma =
    parseInt(query.karmaThreshold) === MAX_LOW_KARMA_THRESHOLD || currentUser?.allPostsShowLowKarma || false;
  ...
  const { SingleColumnSection, SectionTitle, SortButton, SettingsButton, PostsListSettings, HeadTags, AllPostsList } =
    Components;
...
const AllPostsPageComponent = registerComponent("AllPostsPage", AllPostsPage, { styles });
