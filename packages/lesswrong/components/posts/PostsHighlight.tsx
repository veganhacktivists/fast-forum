...
const TruncatedSuffix: FC<{
  ...
  return (
    <Link to={postGetPageUrl(post)} eventProps={{ intent: "expandPost" }}>
      {isFriendlyUI ? "Continue reading" : `(Continue Reading – ${wordsLeft} more words)`}
    </Link>
  );
};
...
const HighlightBody = ({
  ...
  return (
    <Components.ContentStyles
      ...
      className={classNames({ [classes.smallerFonts]: smallerFonts })}
    >
      ...
      <Components.ContentItemTruncated
        ...
        getTruncatedSuffix={({ wordsLeft }: { wordsLeft: number }) => (
          <div className={classes.highlightContinue}>
            <TruncatedSuffix post={post} forceSeeMore={forceSeeMore} wordsLeft={wordsLeft} clickExpand={clickExpand} />
          </div>
        )}
...
const ForeignPostsHighlightBody = ({
  post,
  maxLengthWords,
  forceSeeMore = false,
  smallerFonts,
