...
const MultiToCLayout = ({ segments, classes }: { segments: ToCLayoutSegment[]; classes: ClassesType }) => {
  const tocVisible = true;
  const gridTemplateAreas = segments
    .map((_segment, i) => `"... toc${i} gap1 content${i} gap2 rhs${i} gap3 ..."`)
    .join("\n");
  return (
    <div className={classNames(classes.root)} style={{ gridTemplateAreas }}>
      {segments.map((segment, i) => (
        <React.Fragment key={i}>
            ...
            <>
              <div className={classes.toc} style={{ gridArea: `toc${i}` }}>
                <div className={classes.stickyBlockScroller}>
                  <div className={classes.stickyBlock}>{segment.toc}</div>
                </div>
              </div>
            </>
          )}
          ...
          {segment.rightColumn && (
            <div className={classes.rhs} style={{ gridArea: `rhs${i}` }}>
              {segment.rightColumn}
            </div>
          )}
