...
const getPostCategory = (post: PostsBase) => {
  ...
  else if (post.question) return "Question";
  else if (post.reviewedByUserId) return `Personal Blogpost`;
  else return null;
};
...
const LWPostsPreviewTooltip = ({
  ...
  const truncatedHighlight = truncate(
    highlight,
    expanded ? 200 : 100,
    "words",
              ...
              {postsList && (
                <span>
                  ...
                  {renderWordCount && (
                    <span>
                      {" "}
                      <span className={classes.wordCount}>({wordCount} words)</span>
                    </span>
                  )}
                </span>
              )}
          ...
          <div onClick={() => setExpanded(true)} className={classes.postPreview}>
            ...
            {expanded && (
              <Link to={postGetPageUrl(post)}>
                <div className={classes.continue}>(Continue Reading)</div>
              </Link>
            )}
          </div>
        )}
