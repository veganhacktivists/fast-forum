...
const PostsNewForm = ({ classes }: { classes: ClassesType }) => {
  ...
  let prefilledProps = templateDocument
    ? prefillFromTemplate(templateDocument)
    : {
  ...
  if (!userCanPost(currentUser)) {
    return (
      <SingleColumnSection>
        <Typography variant="display1">You don't have permission to post</Typography>
      </SingleColumnSection>
    );
  }
