...
const PostsEditForm = ({ documentId, classes }: { documentId: string; classes: ClassesType }) => {
  ...
  if (document?.linkSharingKey && !query?.key) {
    return (
      <Components.PermanentRedirect url={postGetEditUrl(document._id, false, document.linkSharingKey)} status={302} />
    );
  }
  ...
  const EditPostsSubmit = (props: SubmitToFrontpageCheckboxProps & PostSubmitProps) => {
    return (
      <div className={classes.formSubmit}>
        ...
        <PostSubmit saveDraftLabel={saveDraftLabel} feedbackLabel={"Get Feedback"} {...props} />
      </div>
    );
  };
          ...
          <EditorContext.Provider value={[editorState, setEditorState]}>
            <WrappedSmartForm
              ...
              successCallback={(post: any, options: any) => {
                const alreadySubmittedToAF =
                  post.suggestForAlignmentUserIds && post.suggestForAlignmentUserIds.includes(post.userId);
                if (!post.draft && !alreadySubmittedToAF)
