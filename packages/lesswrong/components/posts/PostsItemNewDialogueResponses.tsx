...
const PostsItemNewDialogueResponses = ({ postId, unreadCount }: { postId: string; unreadCount: number }) => {
  ...
  return loading ? (
    ...
    data.latestDialogueMessages && data.latestDialogueMessages.length ? (
      <ContentItemBody dangerouslySetInnerHTML={{ __html: data.latestDialogueMessages.join("") }} />
    ) : (
      <NoContent>No new responses found</NoContent>
    )
  ) : (
    <div></div>
  );
...
const PostsItemNewDialogueResponsesComponent = registerComponent(
