...
const PostsItemDate = ({
  ...
  if (post.isEvent && !post.startTime) {
    return (
      <LWTooltip placement="right" title={<span>To Be Determined</span>}>
        <PostsItem2MetaInfo className={classes.startTime}>TBD</PostsItem2MetaInfo>
      </LWTooltip>
    );
  }
  ...
  const ago = includeAgo && timeFromNow !== "now" ? <span className={classes.xsHide}>&nbsp;ago</span> : null;
      ...
      <LWTooltip
        ...
        title={
          <div>
            ...
            <div>
              Posted on <ExpandedDate date={post.postedAt} />
            </div>
          </div>
        }
      >
  ...
  return (
    <LWTooltip placement="right" title={<ExpandedDate date={post.postedAt} />}>
      <PostsItem2MetaInfo className={classes.postedAt}>
        {timeFromNow}
        {ago}
      </PostsItem2MetaInfo>
