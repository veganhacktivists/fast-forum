...
export const CuratedIcon = ({ hasColor, classes }: { hasColor?: boolean; classes: ClassesType<typeof styles> }) => {
    ...
    <span className={classes.postIcon}>
      ...
      >
        <Link to={curatedUrl}>
          <ForumIcon
            icon="Star"
            className={classNames(classes.curatedIcon, { [classes.curatedIconColor]: hasColor && isFriendlyUI })}
          />
        </Link>
      </LWTooltip>
    </span>
...
const PostsItemIcons = ({
  ...
  return (
    <span className={classes.iconSet}>
      ...
      {(post.debate || post.collabEditorDialogue) && (
        <span className={classes.postIcon}>
          <LWTooltip title="Dialogue" placement="right">
            <ForumIcon
              icon={isFriendlyUI ? "ChatBubbleLeftRight" : "ChatBubbleLeftRightFilled"}
      ...
      {!hidePersonalIcon && !post.frontpageDate && !post.isEvent && (
        <span className={classes.postIcon}>
      ...
      {!isAF && post.af && (
        <span className={classes.postIcon}>
          <LWTooltip
            title={
              <div>
                Crossposted from AlignmentForum.org
                <div>
                  <em>(Click to visit AF version)</em>
                </div>
              </div>
            }
            placement="right"
          >
            <a href={`https://alignmentforum.org${postGetPageUrl(post)}`}>
