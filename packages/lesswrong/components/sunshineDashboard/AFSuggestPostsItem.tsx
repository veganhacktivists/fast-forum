...
export const defaultAFModeratorPMsTagSlug = new DatabasePublicSetting<string>(
  "defaultAFModeratorPMsTagSlug",
...
const AFSuggestPostsItem = ({ post, classes }: { post: SuggestAlignmentPost; classes: ClassesType }) => {
  ...
  const userHasSelfSuggested =
    post.suggestForAlignmentUsers && post.suggestForAlignmentUsers.map((user) => user._id).includes(post.userId);

  return (
    <span {...eventHandlers}>
      <Components.SunshineListItem hover={hover}>
        <Components.SidebarHoverOver hover={hover} anchorEl={anchorEl}>
          {userHasSelfSuggested && (
            <Components.ContentStyles contentType="comment" className={classes.afSubmissionHeader}>
        ...
        <Link to={postGetPageUrl(post)} className="sunshine-sidebar-posts-title">
          {post.title}
        </Link>
        <div>
          <Components.SidebarInfo>{post.baseScore}</Components.SidebarInfo>
          <Components.SidebarInfo>
            <Link to={userGetProfileUrl(post.user)}>{post.user && post.user.displayName}</Link>
          </Components.SidebarInfo>
          {post.postedAt && (
            <Components.SidebarInfo>
        ...
        <Components.SidebarInfo>
          Endorsed by{" "}
          {post.suggestForAlignmentUsers && post.suggestForAlignmentUsers.map((user) => user.displayName).join(", ")}
        </Components.SidebarInfo>
        {hover && (
          <Components.SidebarActionMenu>
            {userHasVoted ? (
              <Components.SidebarAction
                title="Unendorse for Alignment"
