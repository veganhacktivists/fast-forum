...
const AFSuggestUsersItem = ({ user }: { user: SuggestAlignmentUser }) => {
        ...
        <C.SunshineListItem hover={hover}>
          <C.SidebarHoverOver hover={hover} anchorEl={anchorEl} width={250}>
            <C.Typography variant="body2">
              <Link to={userGetProfileUrl(user)}>{user.displayName}</Link>
              <br />
              <C.MetaInfo>
          ...
          <div>
            ...
            <C.MetaInfo>{user.karma || 0}</C.MetaInfo>
            <C.MetaInfo>Ω {user.afKarma || 0}</C.MetaInfo>
