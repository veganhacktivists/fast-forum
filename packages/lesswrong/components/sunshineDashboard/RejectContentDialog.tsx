...
const RejectContentDialog = ({
  ...
  const dialogContent = (
    <div className={classes.rejectionCheckboxes}>
      {rejectionTemplates.map((template, i) => {
        const top6 = i < 6;
        return (
          <div key={`rejection-reason-${template.name}`} className={classes.reason}>
            <LWTooltip
              placement="right-end"
              tooltip={false}
              title={
            ...
            >
              <div className={top6 ? classes.topReason : classes.nonTopReason}>
                ...
                />
                {template.name}{" "}
                <Link to={`/admin/moderationTemplates#${template._id}`} target="_blank">
  ...
  return (
    <Paper>
      <div className={classes.dialogContent}>
        ...
        <Button onClick={() => setHideTextField(!hideTextField)}>Edit Message</Button>
      </div>
    </Paper>
  );
};
