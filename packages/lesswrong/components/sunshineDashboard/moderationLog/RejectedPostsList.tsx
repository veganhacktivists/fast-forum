...
export const RejectedPostsList = ({ classes }: { classes: ClassesType }) => {
  const { children, loading, loadMore, loadMoreProps, itemProps } = usePostsList({
    terms: { view: "rejected" },
    enableTotal: true,
  });
  ...
  return (
    <SingleColumnSection className={classes.root}>
      {itemProps?.map(({ post }) => (
        <div key={post._id} className={classes.rejectedPost}>
          ...
          <div className={classes.title}>
            <Link rel="nofollow" to={postGetPageUrl(post)}>
              {post.title}
            </Link>
          </div>
      ...
      <SectionFooter>
        <LoadMore {...loadMoreProps} loading={loading} loadMore={loadMore} sectionFooterStyles />
        {children}
      </SectionFooter>
    </SingleColumnSection>
  );
