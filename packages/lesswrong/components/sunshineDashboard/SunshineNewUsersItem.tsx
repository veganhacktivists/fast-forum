...
const SunshineNewUsersItem = ({
  ...
  const { SunshineListItem, SidebarHoverOver, SunshineNewUsersInfo, MetaInfo, FormatDate, FirstContentIcons } =
    Components;
  ...
  return (
    <div {...eventHandlers} className={user.sunshineFlagged ? classes.flagged : undefined}>
      <SunshineListItem hover={hover}>
        ...
        <div>
          <MetaInfo className={classes.info}>{user.karma || 0}</MetaInfo>
          ...
          {!user.reviewedByUserId && (
            <MetaInfo className={classes.info}>{getUserEmail(user) || "This user has no email"}</MetaInfo>
          )}
        </div>
      </SunshineListItem>
    </div>
