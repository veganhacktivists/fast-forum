...
export const CommentsReviewInfoCard = ({
  ...
  const headerColumn = (
    <div className={classes.headerColumn}>{actions.map((action) => COMMENT_MODERATOR_ACTION_TYPES[action.type])}</div>
  );

  const voteDistributionColumn = (
    <div className={classes.votesColumn}>
      <span>Votes: </span>
      <LWTooltip title="Big Upvotes">
        <span className={classes.bigUpvotes}>{commentVotes.bigUpvote}</span>
      </LWTooltip>
      <LWTooltip title="Upvotes">
        <span className={classes.upvotes}>{commentVotes.smallUpvote}</span>
      </LWTooltip>
      <LWTooltip title="Downvotes">
        <span className={classes.downvotes}>{commentVotes.smallDownvote}</span>
      </LWTooltip>
      <LWTooltip title="Big Downvotes">
        <span className={classes.bigDownvotes}>{commentVotes.bigDownvote}</span>
      </LWTooltip>
    </div>
  );

  const seeInContextUrl = `${commentGetPageUrlFromIds({ postId: comment.postId, commentId: comment._id })}#${comment._id}`;
  const seeInContextColumn = (
    <div>
  ...
  const modActionsColumn = (
    <div>
      {dismissed ? (
        <Loading />
      ) : (
        <LWTooltip title="Dismiss" placement="top">
  ...
  const commentsRow = (
    <div className={classes.commentsRow}>
      <CommentWithReplies
