...
export const UserRateLimitItem = ({ userId, classes }: { userId: string; classes: ClassesType }) => {
  ...
  return (
    <div className={classes.root}>
      ...
      {existingRateLimits.length < 2 && (
        <div>
          Set Rate Limit:{" "}
          <Select value="" onChange={(e) => createRateLimit(e.target.value)} className={classes.newRateLimit}>
            <MenuItem value={COMMENTS_THREE_PER_DAY}>Comments (3 per day for 3 weeks)</MenuItem>
            <MenuItem value={COMMENTS_ONE_PER_DAY}>Comments (1 per day for 3 weeks)</MenuItem>
            <MenuItem value={COMMENTS_ONE_PER_THREE_DAYS}>Comments (1 per 3 days for 3 weeks)</MenuItem>
            <MenuItem value={POSTS_ONE_PER_WEEK}>Posts (1 per week for 6 weeks)</MenuItem>
            <MenuItem value="custom">Custom</MenuItem>
          </Select>
        </div>
      )}
      {(createNewRateLimit || editingExistingRateLimitId) && (
      ...
      {existingRateLimits.length > 0 &&
        existingRateLimits.map((existingRateLimit) => (
