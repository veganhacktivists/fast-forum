    ...
    if (confirm(`Are you sure you want to immediately delete this ${report.comment ? "comment" : "post"}?`)) {
      if (report.comment) {
        ...
        void updatePost({
          ...
          data: { status: report.reportedAsSpam ? postStatuses.STATUS_SPAM : postStatuses.STATUS_DELETED },
        });
      }
      void updateReport({
        selector: { _id: report._id },
    ...
    <span {...eventHandlers}>
      <SunshineListItem hover={hover}>
        ...
        <SidebarInfo>
          ...
          <em>"{report.description}"</em> – {report.user && report.user.displayName},{" "}
          <FormatDate date={report.createdAt} />
        </SidebarInfo>
        {hover && (
          <SidebarActionMenu>
            ...
            {(post || comment) && (
              <SidebarAction
                title={`Spam${forumTypeSetting.get() === "EAForum" ? "" : "/Eugin"} (delete immediately)`}
