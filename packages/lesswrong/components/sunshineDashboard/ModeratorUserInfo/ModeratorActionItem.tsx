...
export const ModeratorActionItem = ({
  classes,
  user,
  moderatorAction,
  comments,
  ...
  return (
    <div key={`${user._id}_${moderatorAction.type}`} className={classes.root}>
      {`${MODERATOR_ACTION_TYPES[moderatorAction.type]}${suffix}`}{" "}
      <MetaInfo className={classes.date}>{editing ? endedAtEditForm : endedAt}</MetaInfo>
