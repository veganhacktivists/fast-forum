...
export const UserReviewStatus = ({ classes, user }: { classes: ClassesType; user: SunshineUsersList }) => {
  const { FormatDate, UsersNameWrapper, AltAccountInfo } = Components;

  const approvalStatus = user.banned
    ? "Banned"
    : user.reviewedByUserId && user.snoozedUntilContentCount
      ? `Snoozed, ${user.snoozedUntilContentCount}`
      : "Approved";
  ...
  return (
    <div className={classes.root}>
      {user.reviewedByUserId && user.reviewedAt ? (
        <div className={classes.reviewedAt}>
          Reviewed <FormatDate date={user.reviewedAt} /> ago by <UsersNameWrapper documentId={user.reviewedByUserId} />{" "}
          ({approvalStatus})
        </div>
      ) : null}
      {user.banned ? (
        <p>
          <em>
            Banned until <FormatDate format="YYYY-MM-DD" date={user.banned} />
          </em>
        </p>
      ) : null}

      {user.allCommentingDisabled && <div className={classes.accountFlag}>All commenting disabled</div>}
      {user.commentingOnOtherUsersDisabled && (
        <div className={classes.accountFlag}>Commenting on other users disabled</div>
      )}
      ...
      {firstClientId?.firstSeenReferrer && (
        <div className={classes.qualitySignalRow}>
          Initial referrer: <a href={firstClientId?.firstSeenReferrer}>{firstClientId?.firstSeenReferrer}</a>
        </div>
      )}
      {firstClientId?.firstSeenLandingPage && (
        <div className={classes.qualitySignalRow}>
          Initial landing page:{" "}
          <Link to={firstClientId?.firstSeenLandingPage}>{firstClientId?.firstSeenLandingPage}</Link>
        </div>
      )}
