...
export const ContentSummaryRows = ({
  classes,
  comments,
  posts,
  user,
  ...
  const averagePostKarma = posts?.length ? (
    <LWTooltip title="average karma">
      <span className={classes.average}>{getAverageBaseScore(posts)}</span>
    </LWTooltip>
  ) : null;
  const averageCommentKarma = comments?.length ? (
    <LWTooltip title="average karma">
      <span className={classes.average}>{getAverageBaseScore(comments)}</span>
    </LWTooltip>
  ) : null;
    ...
    <div>
      <Row>
        <div>
          Sort by:{" "}
          <span
            className={classNames(classes.sortButton, { [classes.sortSelected]: contentSort === "baseScore" })}
            onClick={() => setContentSort("baseScore")}
          >
            karma
          </span>
          <span
            className={classNames(classes.sortButton, { [classes.sortSelected]: contentSort === "postedAt" })}
            onClick={() => setContentSort("postedAt")}
          >
            postedAt
          </span>
        </div>
        <div>
          Display as{" "}
          <span
            className={classNames(classes.sortButton, { [classes.sortSelected]: contentDisplay === "karma" })}
            onClick={() => setContentDisplay("karma")}
          >
            karma
          </span>
          <span
            className={classNames(classes.sortButton, { [classes.sortSelected]: contentDisplay === "titles" })}
            onClick={() => setContentDisplay("titles")}
          >
            titles
          </span>
        </div>
      ...
      <div className={classNames(classes.contentSummaryRow, { [classes.displayTitles]: contentDisplay === "titles" })}>
        ...
        {postKarmaPreviews.map((post) => (
          <PostKarmaWithPreview
            key={post._id}
            post={post}
            reviewedAt={user.reviewedAt}
            displayTitle={contentDisplay === "titles"}
          />
        ))}
      ...
      <div className={classNames(classes.contentSummaryRow, { [classes.displayTitles]: contentDisplay === "titles" })}>
        ...
        {commentKarmaPreviews.map((comment) => (
