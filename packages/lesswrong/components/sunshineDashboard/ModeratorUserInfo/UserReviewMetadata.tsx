...
export const UserReviewMetadata = ({ classes, user }: { user: SunshineUsersList; classes: ClassesType }) => {
  const { MetaInfo, FormatDate } = Components;
  return (
    <div className={classes.root}>
      <MetaInfo>{user.email}</MetaInfo>
      <MetaInfo className={classes.info}>
        <FormatDate date={user.createdAt} />
      </MetaInfo>
      <MetaInfo className={classes.info}>{user.karma || 0} karma</MetaInfo>
    </div>
  );
};
