...
export const ModeratorActions = ({
  ...
  const actionRow = (
    <div className={classes.row}>
      <LWTooltip
        title="Snooze and Approve 10 (Appear in sidebar after 10 posts and/or comments. User's future posts are autoapproved)"
        placement="top"
      >
        <AddAlarmIcon className={classNames(classes.snooze10, classes.modButton)} onClick={() => handleSnooze(10)} />
      </LWTooltip>
      <LWTooltip
        title="Snooze and Approve 1 (Appear in sidebar on next post or comment. User's future posts are autoapproved)"
        placement="top"
      >
      ...
      >
        <AlarmOffIcon
          className={classNames(classes.modButton, {
      ...
      <LWTooltip
        title={
          user.sunshineFlagged ? (
            "Unflag this user"
          ) : (
            <div>
              <div>Flag this user for more in-depth review</div>
              <div>
                <em>(This will not remove them from sidebar)</em>
              </div>
            </div>
          )
        }
      ...
      <LWTooltip title="Return this user to the review queue">
        <VisibilityOutlinedIcon
          className={classNames(classes.modButton, { [classes.disabledButton]: user.needsReview })}
  ...
  const permissionsRow = (
    <div className={classes.row}>
      <LWTooltip title={`${user.postingDisabled ? "Enable" : "Disable"} this user's ability to create posts`}>
        <div
          className={classNames(classes.permissionsButton, { [classes.permissionDisabled]: user.postingDisabled })}
      ...
      <LWTooltip
        title={`${user.allCommentingDisabled ? "Enable" : "Disable"} this user's to comment (including their own ${isFriendlyUI ? "quick takes" : "shortform"})`}
      >
        <div
          className={classNames(classes.permissionsButton, {
            [classes.permissionDisabled]: user.allCommentingDisabled,
          })}
          onClick={handleDisableAllCommenting}
        >
          All Comments
        </div>
      </LWTooltip>
      <LWTooltip
        title={`${user.commentingOnOtherUsersDisabled ? "Enable" : "Disable"} this user's ability to comment on other people's posts`}
      >
        <div
          className={classNames(classes.permissionsButton, {
            [classes.permissionDisabled]: user.commentingOnOtherUsersDisabled,
          })}
          onClick={handleDisableCommentingOnOtherUsers}
        >
          Other Comments
        </div>
      </LWTooltip>
      <LWTooltip
        title={`${user.conversationsDisabled ? "Enable" : "Disable"} this user's ability to start new private conversations`}
      >
        <div
          className={classNames(classes.permissionsButton, {
            [classes.permissionDisabled]: user.conversationsDisabled,
          })}
          onClick={handleDisableConversations}
        >
          Conversations
        </div>
      </LWTooltip>
