...
const AFSuggestCommentsItem = ({ comment, classes }: { comment: SuggestAlignmentComment; classes: ClassesType }) => {
  ...
  const userHasVoted =
    comment.suggestForAlignmentUserIds && comment.suggestForAlignmentUserIds.includes(currentUser._id);
  const userHasSelfSuggested =
    comment.suggestForAlignmentUsers &&
    comment.suggestForAlignmentUsers.map((user) => user._id).includes(comment.userId);
    ...
    <span {...eventHandlers}>
      <Components.SunshineListItem hover={hover}>
        ...
        <Components.SidebarInfo>
          Endorsed by{" "}
          {comment.suggestForAlignmentUsers &&
            comment.suggestForAlignmentUsers.map((user) => user.displayName).join(", ")}
        </Components.SidebarInfo>
        {hover && (
          <Components.SidebarActionMenu>
            {userHasVoted ? (
              <Components.SidebarAction
                ...
                onClick={() => commentUnSuggestForAlignment({ currentUser, comment, updateComment })}
              >
                <UndoIcon />
              </Components.SidebarAction>
            ) : (
              <Components.SidebarAction
                ...
                onClick={() => commentSuggestForAlignment({ currentUser, comment, updateComment })}
              >
                <PlusOneIcon />
              </Components.SidebarAction>
            )}
