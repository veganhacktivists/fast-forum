...
export function getDownvoteRatio(user: UserKarmaInfo): number {
  ...
  const sumOfVoteCounts =
    smallUpvoteReceivedCount + bigUpvoteReceivedCount + smallDownvoteReceivedCount + bigDownvoteReceivedCount;
  const denormalizedVoteCountSumDiff = Math.abs(sumOfVoteCounts - voteReceivedCount);
  const voteCountsAreValid = voteReceivedCount > 0 && denormalizedVoteCountSumDiff / voteReceivedCount <= 0.05;
...
const UsersReviewInfoCard = ({
  ...
  const {
    ...
    LWTooltip,
    UserReviewStatus,
    SunshineNewUserPostsList,
    ContentSummaryRows,
    SunshineNewUserCommentsList,
    ModeratorActions,
    UsersName,
    NewUserDMSummary,
    SunshineUserMessages,
    FirstContentIcons,
    UserAutoRateLimitsDisplay,
  } = Components;
  ...
  const votesRow = (
    <div className={classes.votesRow}>
      ...
      <LWTooltip title="Big Upvotes">
        <span className={classes.bigUpvotes}>{user.bigUpvoteCount || 0}</span>
      </LWTooltip>
      <LWTooltip title="Upvotes">
        <span className={classes.upvotes}>{user.smallUpvoteCount || 0}</span>
      </LWTooltip>
      <LWTooltip title="Downvotes">
        <span className={classes.downvotes}>{user.smallDownvoteCount || 0}</span>
      </LWTooltip>
      <LWTooltip title="Big Downvotes">
        <span className={classes.bigDownvotes}>{user.bigDownvoteCount || 0}</span>
      </LWTooltip>
    </div>
  );
      ...
      <div className={classes.columns}>
        ...
        <div className={classes.contentColumn}>
          ...
          {user.website && (
            <div>
              Website:{" "}
              <a href={`https://${user.website}`} target="_blank" rel="noopener noreferrer" className={classes.website}>
                {user.website}
              </a>
            </div>
          )}
          ...
          <div
            className={classNames(classes.content, { [classes.contentCollapsed]: !contentExpanded })}
          ...
          {renderExpand && (
            <a className={classes.expandButton} onClick={() => setContentExpanded(!contentExpanded)}>
