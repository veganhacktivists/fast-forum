...
const SunshineNewTagsItem = ({ tag, classes }: { tag: SunshineTagFragment; classes: ClassesType }) => {
  ...
  return (
    <span {...eventHandlers}>
      <SunshineListItem hover={hover}>
        <SidebarHoverOver hover={hover} anchorEl={anchorEl}>
          <ContentStyles contentType="comment" className={classes.tagInfo}>
            ...
            <ContentItemBody
              ...
              description={`tag ${tag._id}`}
            />
          </ContentStyles>
          <div className={classes.postCount}>{tag.postCount} posts</div>
          {results &&
            results.map((tagRel) => (
              <div key={tagRel._id} className={classes.post}>
        ...
        <div>
          <SidebarInfo>{tag.postCount}</SidebarInfo>
          <SidebarInfo>
            <Link to={userGetProfileUrl(tag.user)}>{tag.user && tag.user.displayName}</Link>
          </SidebarInfo>
        </div>
        {hover && (
          <SidebarActionMenu>
...
const SunshineNewTagsItemComponent = registerComponent("SunshineNewTagsItem", SunshineNewTagsItem, {
  styles,
