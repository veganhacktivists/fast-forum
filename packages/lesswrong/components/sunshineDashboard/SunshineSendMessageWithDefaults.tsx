...
const SunshineSendMessageWithDefaults = ({
  user,
  embedConversation,
  classes,
}: {
  ...
  return (
    <div className={classes.root}>
      <span className={classes.sendMessageButton} onClick={(ev) => setAnchorEl(ev.currentTarget)}>
        New Message
      </span>
      <Menu onClick={() => setAnchorEl(null)} open={!!anchorEl} anchorEl={anchorEl}>
        <MenuItem value={0}>
        ...
        {defaultResponses &&
          defaultResponses.map((template, i) => (
              ...
              >
                <MenuItem>
                  <NewConversationButton
                    user={user}
                    currentUser={currentUser}
                    templateQueries={{ templateId: template._id, displayName: user.displayName }}
                    includeModerators
                    embedConversation={embedConversation}
                  >
                    {template.name}
...
const SunshineSendMessageWithDefaultsComponent = registerComponent(
