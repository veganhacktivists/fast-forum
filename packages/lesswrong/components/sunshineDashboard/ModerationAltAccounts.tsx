...
const ModerationAltAccounts = ({ classes }: { classes: ClassesType }) => {
  const { SingleColumnSection, SectionTitle, MenuItem, ContentStyles } = Components;
  const currentUser = useCurrentUser();

  const { query } = useLocation();
  const { slug, clientId, userId, ip } = query;
  const { initialAccountIdentifierType, initialAccountIdentifier } = (userId
    ? { initialAccountIdentifierType: "userId", initialAccountIdentifier: userId }
    : null) ??
    ...
    (ip ? { initialAccountIdentifierType: "ip", initialAccountIdentifier: ip } : null) ?? {
      initialAccountIdentifierType: "slug",
  ...
  if (!currentUser || !currentUser.isAdmin) {
    return <SingleColumnSection>You must be logged in as an admin to use this page.</SingleColumnSection>;
  }
  ...
  return (
    <SingleColumnSection>
      ...
      <ContentStyles contentType="comment">
        <p>
          This is a moderation tool for identifying users with alternate accounts, to help with investigating cases of
          suspected ban-evasion, sockpuppet downvoting, and similar issues.
        </p>

        <p>
          <i>Please be conservative in your use of this tool.</i> The intended purpose is to identify users who are
          evading bans, or coordinating large numbers of accounts to vote abusively. Try to view the minimum amount of
          information necessary to confirm (or rule out) that an account is violating site rules, and only proceed look
          at identifying information after a rule-violation is confirmed. Mere criticism is not abusive behavior, even
          if it's wrong!
        </p>

        <p>
          Accounts that are linked by clientID logged in from the same browser, without clearing cookies in between;
          they are very unlikely to be different people (though it's possible if they have shared a device). Accounts
          linked by IP address are more likely to be different people; they could share an office, group house, or (less
          likely but still possible) an ISP or a VPN provider.
        </p>
      </ContentStyles>
      ...
      <div className={classes.selectUser}>
        ...
        >
          {accountIdentifierTypes.map((selection) => (
            <MenuItem key={selection.key} value={selection.key}>
              {selection.label}
            </MenuItem>
          ))}
        </Select>
        <Input
          value={startingIdentifier}
          onChange={(ev) => {
      ...
      <div className={classes.userInfoArea}>
        ...
        {startingIdentifierType === "userId" && (
          <AltAccountsNodeUserByID userId={startingIdentifier} classes={classes} />
        )}
        {startingIdentifierType === "clientId" && (
          <AltAccountsNodeClientID clientId={startingIdentifier} classes={classes} />
        )}
        {startingIdentifierType === "ip" && (
          <AltAccountsNodeIPAddress ipAddress={startingIdentifier} classes={classes} />
        )}
      </div>
    </SingleColumnSection>
  );
...
const AltAccountsNodeUserBySlug = ({ slug, classes }: { slug: string; classes: ClassesType }) => {
...
const AltAccountsNodeUserByID = ({ userId, classes }: { userId: string; classes: ClassesType }) => {
...
const AltAccountsNodeUser = ({ user, classes }: { user: UserAltAccountsFragment; classes: ClassesType }) => {
  ...
  return (
    <div>
      <div>
        <CensoredUserName user={user} classes={classes} />
      </div>

      <ul>
        <li>
          Vote count: {user.voteCount} (small downvotes: {user.smallDownvoteCount}; big downvotes:{" "}
          {user.bigDownvoteCount})
        </li>
        {expandedClientIDs ? (
          <li className={classes.openListItem}>
            <div>Client IDs</div>
            <ul>
              {user.associatedClientIds.map((clientId) => (
                <li key={clientId.clientId}>
...
const AltAccountsNodeClientID = ({ clientId, classes }: { clientId: string; classes: ClassesType }) => {
    ...
    <div>
      ...
      {clientIdInfo && (
        <div>
          ...
          <div>
            First seen: <FormatDate date={clientIdInfo.createdAt} />
          </div>
          <ul>
            {expanded ? (
              <li className={classes.openListItem}>
...
const AltAccountsNodeIPAddress = ({ ipAddress, classes }: { ipAddress: string; classes: ClassesType }) => {
  const { Loading } = Components;
  const [expanded, setExpanded] = useState(false);
  ...
  if (loading || !data?.moderatorViewIPAddress) return <Loading />;
...
const CensoredUserName = ({ user, classes }: { user: UserAltAccountsFragment; classes: ClassesType }) => {
  ...
  if (revealName) {
    ...
    return (
      <span className={classes.clickToReveal} onClick={() => setRevealName(true)}>
        {user._id}
      </span>
    );
  }
