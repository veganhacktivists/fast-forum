...
const ModerationDashboard = ({ classes }: { classes: ClassesType }) => {
  ...
  const {
    results: usersToReview = [],
    totalCount: totalUsersToReviewCount,
    loadMoreProps,
    refetch,
    loading,
  } = useMulti({
  ...
  const {
    results: allUsers = [],
    loadMoreProps: allUsersLoadMoreProps,
      ...
      <div className={classes.row}>
          ...
          <div className={classes.toc}>
            {usersToReview.map((user) => {
              return (
                <div
                  key={user._id}
                  className={classNames(classes.tocListing, { [classes.flagged]: user.sunshineFlagged })}
                >
        ...
        <div className={classNames({ [classes.hidden]: currentView !== "allUsers" })}>
          <div className={classes.toc}>
            {allUsers.map((user) => {
              return (
                <div key={user._id} className={classes.tocListing}>
                  <a href={`${location.pathname}${location.search ?? ""}#${user._id}`}>{user.displayName}</a>
                </div>
              );
            })}
            <div className={classes.loadMore}>
          ...
          <div className={classes.topBar}>
            <div
              ...
              className={classNames(classes.tabButton, {
                [classes.tabButtonSelected]: currentView === "sunshineNewUsers",
              })}
            >
