...
import { mailchimpForumDigestListIdSetting, mailchimpRegionSetting } from "../../lib/publicSettings";

const mailchimpRegion = mailchimpRegionSetting.get();
const mailchimpAudience = mailchimpForumDigestListIdSetting.get();
// mailchimp link to sign up for the EA Forum's digest
export const eaForumDigestSubscribeURL = `https://fastcommunity.${mailchimpRegion}.list-manage.com/subscribe?u=2928fd536ce1603e85d159ef6&id=${mailchimpAudience}`;

const styles = (theme: ThemeType): JssStyles => ({
  root: {
    marginBottom: theme.spacing.unit * 4,
    position: "relative",
    backgroundColor: isFriendlyUI ? theme.palette.grey[0] : theme.palette.panelBackground.recentDiscussionThread,
 ...
 */
const RecentDiscussionSubscribeReminder = ({ classes }: { classes: ClassesType }) => {
  ...
  const adminUiMessage = currentUser?.isAdmin ? (
    <div className={classes.adminNotice}>
      You are seeing this UI element because you're an admin. Admins are shown a random version of the
      subscribe-reminder even if they're already subscribed, to make sure it still works and isn't annoying.
    </div>
  ) : null;
  ...
  if (adminBranch === -1 && currentUser?.isAdmin) return <div />;
  ...
  const AnalyticsWrapper = ({ children, branch }: { children: React.ReactNode; branch: string }) => {
    return (
      <AnalyticsContext pageElementContext="subscribeReminder" branch={branch}>
  ...
  const eaForumSubscribePrompt = (
    <>
      ...
      <div className={classes.messageDescription}>
        You'll get a weekly email with the best posts from the past week. The Forum team selects the posts to feature
        based on personal preference and Forum popularity, and also adds some announcements and a classic post.
      </div>
    </>
  );
  ...
  if (loading) {
    ...
    return (
      <AnalyticsWrapper branch="already-subscribed">
        <div className={classes.message}>
    ...
    return (
      <AnalyticsWrapper branch="needs-email-verification-subscribed-in-other-branch">
        <div className={classes.message}>
    ...
    const subscribeTextNode = isEAForum ? (
      eaForumSubscribePrompt
    ) : (
      <div className={classes.message}>
    ...
    return (
      <AnalyticsWrapper branch="logged-out">
        {subscribeTextNode}
        {isEAForum ? (
    ...
    // checkbox.
    return (
      <AnalyticsWrapper branch="missing-email">
        <div className={classes.message}>
          Your account does not have an email address associated. Add an email address to subscribe to {emailType} and
              ...
              if (emailAddress && SimpleSchema.RegEx.Email.test(emailAddress?.value)) {
                ...
                try {
                  ...
                  const userSubscriptionData: Partial<MakeFieldsNullable<DbUser>> = isEAForum
                    ? { subscribedToDigest: subscribeChecked }
    ...
    const subscribeTextNode =
      forumTypeSetting.get() === "EAForum" ? (
        eaForumSubscribePrompt
      ) : (
        <div className={classes.message}>
          You previously unsubscribed from all emails from LessWrong. Re-subscribe to get the best posts emailed to you!{" "}
          {subscriptionDescription}
        </div>
      );
    return (
      <AnalyticsWrapper branch="previously-unsubscribed">
        {subscribeTextNode}
        <Button
          className={classes.subscribeButton}
          onClick={async (ev) => {
    ...
    return (
      <AnalyticsWrapper branch="logged-in-not-subscribed">
        {subscribeTextNode}
        <Button
          className={classes.subscribeButton}
          onClick={async (ev) => {
    ...
    return (
      <AnalyticsWrapper branch="needs-email-verification">
        <div>
          <div className={classes.message}>
