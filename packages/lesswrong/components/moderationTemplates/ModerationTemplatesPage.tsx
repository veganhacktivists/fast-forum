...
export const ModerationTemplatesPage = ({ classes }: { classes: ClassesType }) => {
  const {
    WrappedSmartForm,
    SingleColumnSection,
    SectionTitle,
    ModerationTemplateItem,
    BasicFormStyles,
    Loading,
    Row,
    ToCColumn,
  ...
  return (
    <ToCColumn tableOfContents={<TableOfContents sectionData={sectionData} title={"Moderation Templates"} />}>
      <SingleColumnSection>
        ...
        {loading && <Loading />}
        {nonDeletedTemplates.map((template) => (
          <div id={template._id} key={template._id}>
            <ModerationTemplateItem template={template} />
          </div>
        ))}

        <a role="button" onClick={() => setShowDeleted(!showDeleted)}>
          Show Deleted
        </a>
...
const ModerationTemplatesPageComponent = registerComponent("ModerationTemplatesPage", ModerationTemplatesPage, {
  styles,
});
