...
const AlignmentPendingApprovalMessage = ({ post, classes }: { post: PostsBase; classes: ClassesType }) => {
  const currentUser = useCurrentUser();
  if (!currentUser) return null;

  const userSubmittedPost =
  ...
  if (!post.af && userSubmittedPost && forumTypeSetting.get() === "AlignmentForum") {
    return (
      <div className={classes.root}>
        <p>
          This post is pending approval to the Alignment Forum and is currently only visible to you. However, it is
          already visible (and commentable) to everyone on LessWrong.{" "}
...
const AlignmentPendingApprovalMessageComponent = registerComponent(
