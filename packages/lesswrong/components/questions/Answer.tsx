...
const Answer = ({
  ...
  const menuIcon = isFriendlyUI ? undefined : <MoreHorizIcon className={classes.menuIcon} />;
          ...
          <AnalyticsContext pageElementContext="answerItem">
            <HoveredReactionContextProvider>
              <div className={classes.answer} id={comment._id}>
                ...
                {comment.promotedByUser && (
                  <div className={classes.metaNotice}>Pinned by {comment.promotedByUser.displayName}</div>
                )}
                {showEdit ? (
                  <Components.CommentsEditForm comment={comment} successCallback={hideEdit} cancelCallback={hideEdit} />
                ) : (
                  <CommentBody commentBodyRef={commentBodyRef} comment={comment} voteProps={voteProps} />
                )}
                <CommentBottom
                  comment={comment}
                  post={post}
                  ...
                  replyButton={
                    !replyFormIsOpen && (
                      <Typography
                        variant="body2"
                        onClick={() => setReplyFormIsOpen(true)}
