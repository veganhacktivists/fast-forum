...
const RelatedQuestionsList = ({ post, classes }: { post: PostsDetails; classes: ClassesType }) => {
  const { PostsItem, SectionTitle } = Components;

  const sourcePostRelations = _.filter(post.sourcePostRelations, (rel) => !!rel.sourcePost);
  const targetPostRelations = _.filter(
    post.targetPostRelations,
    (rel) => rel.sourcePostId === post._id && !!rel.targetPost,
  );
  ...
  return (
    <div className={classes.root}>
      ...
      {sourcePostRelations.map(
        (rel, i) => rel.sourcePost && <PostsItem key={rel._id} post={rel.sourcePost} index={i} />,
      )}
      ...
      {targetPostRelations.map((rel, i) => {
        ...
        const subQuestionTargetPostRelations = _.filter(
          post.targetPostRelations,
          (rel) => rel.sourcePostId === parentQuestionId,
        );
        ...
        return (
          <div key={rel._id} className={classes.subQuestion}>
            ...
            />
            {showSubQuestions && (
              <div className={classes.subSubQuestions}>
                {subQuestionTargetPostRelations.map(
                  (rel, i) =>
                    rel.targetPost && (
                      <PostsItem
                        key={rel._id}
