...
const NewAnswerCommentQuestionForm = ({
  post,
  refetch,
  classes,
}: {
  ...
  const getNewForm = () => {
    switch (selection) {
      ...
      case "comment":
        return <CommentsNewForm post={post} type="comment" />;
    }
  };

  return (
    <div className={classes.root} onFocus={() => afNonMemberDisplayInitialPopup(currentUser, openDialog)}>
      ...
      <div className={classes.form}>
        ...
        <div className={classes.responseForm}>{getNewForm()}</div>
      </div>
    </div>
  );
};

const NewAnswerCommentQuestionFormComponent = registerComponent(
  "NewAnswerCommentQuestionForm",
  NewAnswerCommentQuestionForm,
  { styles },
);
