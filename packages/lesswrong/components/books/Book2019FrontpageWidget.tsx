...
const Book2019FrontpageWidget = ({ classes }: { classes: ClassesType }) => {
  ...
  const BookMarketingText = ({
    ...
    return (
      <ContentStyles contentType="post" className={classes.bookExplanation}>
        <div className={classes.closeButton} onClick={hideClickHandler}>
          X
        </div>
        <h1 className={classes.mainHeading}>{title}</h1>
        <h4 className={classes.secondaryHeading}>{subtitle}</h4>
      ...
      <Book2019Animation
        successContent={
          <BookMarketingText
            ...
            description={
              <>
                <Link to="/posts/QB6BkkpwiecfF6Ekq/thanksgiving-prayer">Dear Global Economy</Link>, we thank thee for
                thy economies of scale, thy professional specialization, and thy international networks of trade under
                Ricardo's Law of Comparative Advantage, without which we would all starve to death while trying to
                assemble the ingredients for such a [book] as this. Amen.
              </>
            }
            buttons={
              <>
                <div className={classes.mobileCloseButton} onClick={hideClickHandler}>
                  Hide
                </div>
                ...
                <BookCheckout
                  ignoreMessages
                  text={"Buy Another"}
                  link={"https://www.amazon.com/Engines-Cognition-Essays-LessWrong-Community/dp/1736128515/"}
                />
              </>
            }
          />
...
const Book2019FrontpageWidgetComponent = registerComponent("Book2019FrontpageWidget", Book2019FrontpageWidget, {
  styles,
});
