...
const ConfigurableRecommendationsList = ({ configName }: { configName: string }) => {
    ...
    <SingleColumnSection>
      <SectionTitle
        title={
          ...
          >
            <Link to={"/recommendations"}>{archiveRecommendationsName}</Link>
          </LWTooltip>
        }
      >
      ...
      <RecommendationsList algorithm={settingsOrDefault} />
    </SingleColumnSection>
  );
};

const ConfigurableRecommendationsListComponent = registerComponent(
