...
export function getRecommendationSettings({
  ...
  if (currentUser?.recommendationSettings && configName in currentUser.recommendationSettings) {
    return deepmerge(
      defaultAlgorithmSettings,
      currentUser.recommendationSettings[configName as keyof UsersCurrent["recommendationSettings"]] || {},
    );
  } else {
...
const RecommendationsAlgorithmPicker = ({
  settings,
  configName,
  onChange,
  showAdvanced = false,
          ...
          <span className={classes.setting}>
            <SectionFooterCheckbox
              ...
              onClick={(ev: React.MouseEvent) =>
                applyChange({ ...settings, hideContinueReading: !settings.hideContinueReading })
              }
      ...
      <span className={classes.settingGroup}>
        ...
        <span className={classes.setting}>
          <SectionFooterCheckbox
            ...
            onClick={(ev: React.MouseEvent) =>
              applyChange({ ...settings, [includeExtra.machineName]: !settings[includeExtra.machineName] })
            }
      ...
      {showAdvanced && (
        <div>
          <div>
            ...
            <select
              onChange={(ev) =>
                applyChange({ ...settings, method: ev.target.value as DefaultRecommendationsAlgorithm["method"] })
              }
          ...
          <div>
            ...
            <Input
              ...
              onChange={(ev) =>
                applyChange({ ...settings, personalBlogpostModifier: ev.target.value as unknown as number })
              }
            />
          </div>
          <div>
...
const RecommendationsAlgorithmPickerComponent = registerComponent(
