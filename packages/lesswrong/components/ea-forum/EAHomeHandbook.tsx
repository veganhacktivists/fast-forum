...
const eaHomeSequenceFirstPostId = new PublicInstanceSetting<string | null>(
  "eaHomeSequenceFirstPostId",
  null,
  "optional",
); // Post ID for the first post in the EAHomeHandbook Sequence

const EAHomeHandbook = ({ classes, documentId }: { classes: ClassesType; documentId: string }) => {
  ...
  const handleDismiss = () => {
    ...
    flash({
      messageString:
        "We won't show this again. If you want to read the this in the future, you can access it from the sidebar menu.", // TODO: s/this/something/
    });
  };
        ...
        <div className={classes.overImage}>
          ...
          {document.user && (
            <Typography variant="display1" className={classNames(classes.overImageText, classes.description)}>
              Intro Sequence by <Link to={`/users/${document.user.slug}`}>{document.user.displayName}</Link>
            </Typography>
          )}
          <Button
            variant="contained"
...
const EAHomeHandbookComponent = registerComponent("EAHomeHandbook", EAHomeHandbook, { styles });
