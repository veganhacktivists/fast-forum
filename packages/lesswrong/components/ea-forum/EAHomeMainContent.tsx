 ...
 */
const EAHomeMainContent = ({ FrontpageNode, classes }: { FrontpageNode: ComponentType; classes: ClassesType }) => {
  ...
  useEffect(() => {
    ...
    Array.from(topicsBarRef.current.children).forEach((topic: HTMLElement) => {
      ...
      if (
        topic.offsetLeft + topic.offsetWidth - offsets.current[offsets.current.length - 1] >
        tabsWindowRef.current.offsetWidth
      ) {
  ...
  const activeCoreTopic = useMemo(() => coreTopics?.find((t) => t._id === activeTab._id), [coreTopics, activeTab]);
    ...
    <>
      <AnalyticsContext pageSectionContext="topicsBar">
        <SingleColumnSection className={classes.tabsSection}>
          <div className={classes.tabsRow}>
            {leftArrowVisible && (
              <div onClick={scrollLeft} className={classNames(classes.arrow, classes.leftArrow)}>
                <ForumIcon icon="ThickChevronLeft" className={classes.arrowIcon} />
              </div>
            )}
            <div
              className={classNames(classes.tabsWindowContainer, {
            ...
            {rightArrowVisible && (
              <div onClick={scrollRight} className={classNames(classes.arrow, classes.rightArrow)}>
      ...
      {activeTab.name === "Frontpage" ? (
        ...
        <AnalyticsContext pageSectionContext="topicSpecificPosts">
          <SingleColumnSection>
            {spotlight && <DismissibleSpotlightItem spotlight={spotlight} className={classes.spotlightMargin} />}
            <SectionTitle title="New & upvoted" noTopMargin>
              <Link to={`/topics/${activeTab.slug}`} className={classes.learnMoreLink}>
                View more
              </Link>
            </SectionTitle>
            <PostsList2 terms={topicPostTerms} itemsPerPage={30} hideTag />
          </SingleColumnSection>
        </AnalyticsContext>
      )}
    </>
