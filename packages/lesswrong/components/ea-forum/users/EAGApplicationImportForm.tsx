...
const EAGApplicationImportFormWrapper = () => {
  ...
  return <EAGApplicationImportForm currentUser={currentUserEdit} />;
};

const EAGApplicationImportForm = ({ currentUser, classes }: { currentUser: UsersEdit; classes: ClassesType }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (eagData) {
        ...
        [
          "Which_Event",
          "Which_event_s_are_you_registering_for_EA_Globa",
          "Which_event_s_are_you_applying_to_registering_for",
          "Which_event_s_are_you_registering_for",
        ].forEach((field) => {
        ...
        setImportedData({
          ...
          careerStage: careerStage
            ?.map((stage: string) => CAREER_STAGES.find((s) => stage.includes(s.label))?.value)
  ...
  const handleChangeField = (
    ...
    field: keyof EAGApplicationDataType,
  ) => {
    e.preventDefault();
  ...
  const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement>, copyAll = false) => {
    ...
    for (let field in updatedFormData) {
      ...
      if (
        Array.isArray(updatedFormData[field as UpdatedFormDataKey]) &&
        !updatedFormData[field as UpdatedFormDataKey].length
      ) {
      ...
      <AnalyticsContext pageContext="eagApplicationImportForm">
        <div className={classNames(classes.root, classes.loggedOutView)}>
          ...
          <Typography variant="body2" className={classes.loggedOutMessage}>
            <a href={`/auth/auth0?returnTo=${pathname}`} className={classes.loggedOutLink}>
              Login
            </a>{" "}
            or{" "}
            <a href={`/auth/auth0?screen_hint=signup&returnTo=${pathname}`} className={classes.loggedOutLink}>
              Sign Up
            </a>{" "}
            to import information from your EA Global application to your FAST Forum profile.
          </Typography>
        </div>
      </AnalyticsContext>
    );
  ...
  const body = !importedData ? (
    <>
      ...
      <Typography variant="body2" className={classes.subheading}>
        ...
        <Link to="/contact" className={classes.contactUs}>
          contact us
        </Link>
        .
      </Typography>
      ...
      <form className={classes.form}>
        ...
        <div className={classes.formRow}>
          ...
          <Input
            ...
            onChange={(e) => handleChangeField(e, "organization")}
          />
          <div className={classes.arrowCol}>
...
const EAGApplicationImportFormWrapperComponent = registerComponent(
  "EAGApplicationImportFormWrapper",
  EAGApplicationImportForm,
);
