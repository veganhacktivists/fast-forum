...
import {
  WrappedDataByYearV2,
  WrappedMostReadAuthor,
  WrappedMostReadTopic,
  WrappedReceivedReact,
  WrappedRelativeMostReadCoreTopic,
  WrappedTopComment,
  WrappedTopPost,
  WrappedTopShortform,
  useForumWrappedV2,
 ...
 */
const formattedPercentile = (percentile: number) => Math.ceil((1 - percentile) * 100) || 1;

/**
 * Formats the karma change number as a string with a + or -
 */
const formattedKarmaChangeText = (karmaChange: number) => `${karmaChange >= 0 ? "+" : ""}${karmaChange}`;
 ...
 */
const Post = ({ post, classes }: { post: WrappedTopPost | PostsListWithVotesAndSequence; classes: ClassesType }) => {
  ...
  return (
    <article className={classes.post} ref={authorExpandContainer} onClick={onClick}>
      ...
      <div className={classes.postTitleAndMeta}>
        <div className={classes.postTitle}>
          {isRecommendedPost ? <PostsItemTooltipWrapper post={post}>{titleNode}</PostsItemTooltipWrapper> : titleNode}
        </div>
        {isRecommendedPost && (
          <div className={classes.postMetaRow}>
            <InteractionWrapper>
              <TruncatedAuthorsList post={post} expandContainer={authorExpandContainer} className={classes.postMeta} />
            </InteractionWrapper>
            <span className={classes.postMeta}>{readTimeText}</span>
          </div>
        )}
      </div>
      {isRecommendedPost && (
 ...
 */
const Comment = ({ comment, classes }: { comment: WrappedTopComment | WrappedTopShortform; classes: ClassesType }) => {
  ...
  return (
    <article className={classes.comment}>
      {"postTitle" in comment && (
        <div className={classes.commentPostTitle}>
          <Link to={postGetPageUrl({ _id: comment.postId, slug: comment.postSlug })}>{comment.postTitle}</Link>
        </div>
      )}
        ...
        <div className={classes.commentAuthorAndDate}>
          ...
          <div className={classes.commentDate}>
            <LWTooltip placement="right" title={<ExpandedDate date={comment.postedAt} />}>
              <CommentLinkWrapper>{moment(new Date(comment.postedAt)).fromNow()}</CommentLinkWrapper>
            </LWTooltip>
          </div>
        </div>
      ...
      <ContentStyles contentType="comment">
        <div className={classes.commentBody}>{htmlToTextDefault(comment.contents.html)}</div>
      </ContentStyles>
    </article>
  );
};
...
const KarmaChangeXAxis = () => {
  return (
    <>
      <text y="100%" fontSize={12} fill="#FFF" textAnchor="start">
        Jan
      </text>
      <text x="100%" y="100%" fontSize={12} fill="#FFF" textAnchor="end">
        Dec
      </text>
    </>
  );
 ...
 */
const EngagementPercentileSection = ({ data, classes }: { data: WrappedDataByYearV2; classes: ClassesType }) => {
  ...
  return (
    <AnalyticsContext pageSectionContext="engagementPercentile">
      <section className={classes.section}>
        <h1 className={classes.heading3}>
          You’re a top <span className={classes.highlight}>{formattedPercentile(data.engagementPercentile)}%</span>{" "}
          reader of the EA Forum
        </h1>
        <p className={classNames(classes.textRow, classes.text, classes.mt16)}>
          You read {data.postsReadCount} posts this year
        </p>
        <div className={classes.chart}>
          <aside className={classes.engagementChartMark} style={{ left: markPosition }}>
            <div className={classes.engagementChartArrow}>{drawnArrow}</div>
            you
          </aside>
 ...
 */
const EngagementHoursSection = ({ engagementHours, classes }: { engagementHours: number; classes: ClassesType }) => {
  return (
    <AnalyticsContext pageSectionContext="engagementHours">
      <section className={classes.section}>
        <h1 className={classes.heading3}>
          You spent <span className={classes.highlight}>{engagementHours.toFixed(1)}</span> hours on the FAST Forum in
          2023
        </h1>
 ...
 */
const DaysVisitedSection = ({ daysVisited, classes }: { daysVisited: string[]; classes: ClassesType }) => {
  return (
    <AnalyticsContext pageSectionContext="daysVisited">
      <section className={classes.section}>
        <h1 className={classes.heading3}>
...
const MostReadTopicsSection = ({
  ...
  return (
    <AnalyticsContext pageSectionContext="mostReadTopics">
      <section className={classes.section}>
        <h1 className={classes.heading3}>
          When you were reading a post, it was most often about{" "}
          <span className={classes.highlight}>{topics[0].name}</span>
        </h1>
 ...
 */
const RelativeMostReadTopicsSection = ({
  ...
  const relativeMostReadTopics = relativeMostReadCoreTopics
    .map((topic) => {
      return {
        ...topic,
  ...
  return (
    <AnalyticsContext pageSectionContext="relativeMostReadTopics">
      <section className={classes.section}>
        <h1 className={classes.heading3}>
          You read more <span className={classes.highlight}>{relativeMostReadTopics[0].tagName}</span> posts than
          average
        </h1>
        <div className={classes.topicsChart}>
          ...
          <aside className={classes.relativeTopicsChartMarkAvg}>
            <div className={classes.relativeTopicsChartArrowAvg}>{drawnArrow}</div>
            average
          </aside>
 ...
 */
const MostReadAuthorsSection = ({ authors, classes }: { authors: WrappedMostReadAuthor[]; classes: ClassesType }) => {
    ...
    <AnalyticsContext pageSectionContext="mostReadAuthors">
      <section className={classes.section}>
        ...
        <div className={classes.authors}>
          {authors.map((author) => {
            return (
              <article key={author.slug} className={classes.author}>
                <Components.UsersProfileImage size={40} user={author} />
                <div>
                  <h2 className={classes.authorName}>
                    <Link to={getUserProfileLink(author.slug)}>{author.displayName}</Link>
                  </h2>
 ...
 */
const ThankAuthorSection = ({ authors, classes }: { authors: WrappedMostReadAuthor[]; classes: ClassesType }) => {
  const currentUser = useCurrentUser();
  ...
  const topAuthorPercentByEngagementPercentile =
    (topAuthorByEngagementPercentile && Math.ceil(100 * (1 - topAuthorByEngagementPercentile.engagementPercentile))) ||
    1;
  ...
  return (
    <AnalyticsContext pageSectionContext="thankAuthor">
      <section className={classes.section}>
        <h1 className={classes.heading3}>
          You’re in the top <span className={classes.highlight}>{topAuthorPercentByEngagementPercentile}%</span> of{" "}
          {topAuthorByEngagementPercentile.displayName}’s readers
        </h1>
        ...
        <div className={classes.messageAuthor}>
          <div className={classes.topAuthorInfo}>
            <div>To:</div>
            <div>
              <Components.UsersProfileImage size={24} user={topAuthorByEngagementPercentile} />
            </div>
 ...
 */
const TopPostSection = ({ data, classes }: { data: WrappedDataByYearV2; classes: ClassesType }) => {
  ...
  return (
    <AnalyticsContext pageSectionContext="topPost">
      <section className={classes.section}>
        <h1 className={classes.heading3}>
 ...
 */
const TopCommentSection = ({ data, classes }: { data: WrappedDataByYearV2; classes: ClassesType }) => {
  ...
  return (
    <AnalyticsContext pageSectionContext="topComment">
      <section className={classes.section}>
        <h1 className={classes.heading3}>
 ...
 */
const TopQuickTakeSection = ({ data, classes }: { data: WrappedDataByYearV2; classes: ClassesType }) => {
  ...
  return (
    <AnalyticsContext pageSectionContext="topQuickTake">
      <section className={classes.section}>
        <h1 className={classes.heading3}>
 ...
 */
const KarmaChangeSection = ({ data, classes }: { data: WrappedDataByYearV2; classes: ClassesType }) => {
  ...
  return (
    <AnalyticsContext pageSectionContext="karmaChange">
      <section className={classes.section}>
        <h1 className={classes.heading3}>
          Your overall karma change this year was{" "}
          <span className={classes.highlight}>{formattedKarmaChangeText(data.karmaChange)}</span>
        </h1>
        <div className={classes.chart}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart width={350} height={200} data={data.combinedKarmaVals}>
              ...
              <XAxis
                dataKey="date"
                tick={false}
                axisLine={{ strokeWidth: 3, stroke: "#FFF" }}
                height={16}
                label={<KarmaChangeXAxis />}
              />
              <Area
...
const ReactsReceivedSection = ({
  ...
  return (
    <AnalyticsContext pageSectionContext="reactsReceived">
      <section className={classNames(classes.section, classes.sectionTall)}>
        {allReactsReceived?.map((react, i) => {
          return (
            <div key={i} className={classes.backgroundReact} style={{ top: react.top, left: react.left }}>
              <react.Component />
            </div>
          );
        })}
            ...
            <div className={classes.otherReacts}>
              ...
              <div className={classNames(classes.stats, classes.mt26)}>
                {receivedReacts.slice(1).map((react) => {
                  return (
                    <article key={react.name} className={classes.stat}>
 ...
 */
const ThankYouSection = ({ classes }: { classes: ClassesType }) => {
  return (
    <AnalyticsContext pageSectionContext="thankYou">
      <section className={classes.section}>
        <h1 className={classes.heading3}>
          Thank you!{" "}
        ...
        <div className={classNames(classes.lightbulbIcon, classes.mt30)}>{lightbulbIcon}</div>
...
const SummarySection = ({
  ...
  return (
    <AnalyticsContext pageSectionContext="summary">
              ...
              <div className={classes.summaryBox}>
                ...
                <div className={classNames(classes.summaryList, classes.mt12)}>
                  {data.mostReadAuthors.map((author) => {
                    return (
                      <div key={author.slug} className={classes.summaryListItem}>
              ...
              <div className={classes.summaryBox}>
                ...
                <div className={classNames(classes.summaryList, classes.mt12)}>
                  {data.mostReadTopics.map((topic) => {
                    return (
                      <div key={topic.slug} className={classes.summaryListItem}>
                        <CoreTagIcon
                          tag={topic}
                          fallbackNode={<div className={classes.summaryTopicIconPlaceholder}></div>}
                        />
                        <Link to={tagGetUrl({ slug: topic.slug })}>{topic.name}</Link>
                      </div>
                    );
                  })}
                </div>
 ...
 */
const RecommendationsSection = ({ classes }: { classes: ClassesType }) => {
  return (
    <AnalyticsContext pageSectionContext="recommendations">
      <section className={classes.section}>
        <h1 className={classes.heading3}>Posts you missed that we think you’ll enjoy</h1>
        <NoSSR>
 ...
 */
const MostValuablePostsSection = ({ classes }: { classes: ClassesType }) => {
  const { ForumIcon, PostsByVoteWrapper } = Components;

  return (
    <AnalyticsContext pageSectionContext="mostValuablePosts">
      <section className={classNames(classes.section, classes.sectionNoFade)}>
        <h1 className={classes.heading3}>Which posts from 2023 were most valuable for you?</h1>
        <p className={classNames(classes.textRow, classes.text, classes.mt16)}>
          These are your upvotes from 2023. Your choice of the most valuable posts will be really useful for helping us
          decide what to feature on the Forum. (We’ll only look at anonymized data.)
        </p>
          ...
          <section className={classes.section}>
            ...
            <div className={classes.loginWrapper}>
              <p className={classes.loginText}>
                ...
                <a
                  href={`/auth/auth0?screen_hint=signup&returnTo=${encodeURIComponent("/wrapped")}`}
            ...
            <CloudinaryImage2
              ...
              wrapperClassName={classes.loginImgWrapper}
        ...
        <AnalyticsContext pageSectionContext="top">
          <section className={classes.section}>
            ...
            {currentUser.wrapped2023Viewed && (
              <button
                className={classNames(classes.summaryLinkWrapper, classes.skipToSummaryBtn)}
