...
import {
  DIGEST_STATUS_OPTIONS,
  InDigestStatusOption,
  StatusField,
  getEmailDigestPostListData,
  getStatusFilterOptions,
} from "../../../lib/collections/digests/helpers";
...
const EditDigest = ({ classes }: { classes: ClassesType }) => {
  ...
  const visiblePosts = useMemo(() => {
    ...
    let visiblePosts = posts.filter((post) => {
      return (
        emailDigestFilter.includes(postStatuses[post._id].emailDigestStatus) &&
  ...
  const tagOptions = useMemo(() => {
    ...
    return coreAndPopularTags.reduce(
      (prev: Record<string, SettingsOption>, next) => {
        prev[next._id] = {
          label: `${next.name} (${postSet.filter((p) => p.tags.some((t) => t._id === next._id)).length})`,
        };
        return prev;
      },
    ...
    <div className={classes.root}>
      <div className={classes.topSection}>
        <div className={classes.headlineRow}>
          ...
          <LWTooltip title="Click to copy email digest post list" placement="bottom" className={classes.copyDigest}>
            <ForumIcon
        ...
        <thead>
          <tr className={classes.headerRow}>
            <th className={classes.centeredColHeader}>
              Email?
              <div className={classNames(classes.total, { [classes.totalHigh]: emailTotal > 14 })}>{emailTotal}</div>
            </th>
            <th className={classes.centeredColHeader}>
              On-site?
              <div className={classNames(classes.total, { [classes.totalHigh]: onsiteTotal > 30 })}>{onsiteTotal}</div>
            </th>
            <th>Post</th>
            <th>Tags</th>
            ...
            <th className={classes.centeredColHeader}>
              Your vote
              <div className={classes.toggleLink} onClick={() => setVotesVisible(!votesVisible)}>
