...
const EditDigestTableRow = ({
  post,
  postStatus,
  statusIconsDisabled,
  handleClickStatusIcon,
  visibleTagIds,
  setTagFilter,
  votesVisible,
  classes,
    ...
    <tr className={classes.row}>
        ...
        <div>
          <div>
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className={classNames(classes.title, classes.link, { [classes.isRead]: post.isRead })}
            >
              {post.title}
            </a>{" "}
          ...
          <div className={classes.postIcons}>
            ...
            <ForumIcon
              icon="Star"
              className={classNames(classes.curatedIcon, { [classes.hiddenIcon]: !post.curatedDate })}
            />
          </div>
        </div>
      </td>
      ...
      <td className={classes.tagsCol}>
        {visibleTags
          ...
          .map((tag) => {
            return (
              <div key={tag._id} className={classes.tag} onClick={() => setTagFilter(tag._id)}>
                {tag.name}
              </div>
            );
          })}
      ...
      <td className={classes.suggestedCurationCol}>{post.suggestForCuratedUsernames}</td>
      <td className={classes.voteCol}>{votesVisible && voteIcon}</td>
      {/* <td className={classes.ratingCol}>{post.rating}</td> */}
      <td className={classes.commentsCol}>
        {post.commentCount > 0 && (
          <a href={`${postGetPageUrl(post)}#comments`} target="_blank" rel="noreferrer" className={classes.link}>
