...
export const EditDigestHeader = ({ digest, classes }: { digest: DigestsMinimumInfo; classes: ClassesType }) => {
  ...
  const endNode =
    isEditingEndDate && digest.endDate ? (
      <div className={classes.datePicker}>
        <DatePicker
          label="End date"
      ...
      <span onClick={() => setIsEditingEndDate(true)} className={classNames({ [classes.date]: digest.endDate })}>
        {endFormatted}
      </span>
    );
