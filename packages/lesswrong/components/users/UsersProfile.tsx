...
const UsersProfileFn = ({ terms, slug, classes }: { terms: UsersViewTerms; slug: string; classes: ClassesType }) => {
      ...
      <div className={classes.meta}>
        {!isAF && (
          <Tooltip title={`${userKarma} karma`}>
            <span className={classes.userMetaInfo}>
              ...
              <Components.MetaInfo title="Karma">{userKarma}</Components.MetaInfo>
            </span>
          </Tooltip>
        )}

        {!!userAfKarma && (
          <Tooltip title={`${userAfKarma} karma${!isAF ? " on alignmentforum.org" : ""}`}>
            <span className={classes.userMetaInfo}>
              <Components.OmegaIcon className={classNames(classes.icon, classes.specificalz)} />
              <Components.MetaInfo title="Alignment Karma">{userAfKarma}</Components.MetaInfo>
            </span>
          </Tooltip>
        )}
        ...
        <Tooltip title={`${userPostCount} posts`}>
          <span className={classes.userMetaInfo}>
            ...
            <Components.MetaInfo title="Posts">{userPostCount}</Components.MetaInfo>
          </span>
        </Tooltip>
        ...
        <Tooltip title={`${userCommentCount} comments`}>
          <span className={classes.userMetaInfo}>
            ...
            <Components.MetaInfo title="Comments">{userCommentCount}</Components.MetaInfo>
          </span>
        </Tooltip>

        <Tooltip
          title={`${tagRevisionCount || 0} ${taggingNameIsSet.get() ? taggingNameSetting.get() : "wiki"} edit${tagRevisionCount === 1 ? "" : "s"}`}
        >
          <span className={classes.userMetaInfo}>
            <PencilIcon className={classNames(classes.icon, classes.specificalz)} />
            <Components.MetaInfo>{tagRevisionCount || 0}</Components.MetaInfo>
          </span>
        </Tooltip>
      </div>
    );
    ...
    return (
      <div className={classNames("page", "users-profile", classes.profilePage)}>
        ...
        <AnalyticsContext pageContext={"userPage"}>
          ...
          <SingleColumnSection>
            <div
              className={classNames(classes.usernameTitle, {
            ...
            <Typography variant="body2" className={classes.userInfo}>
              ...
              {currentUser && currentUser._id === user._id && (
                <Link to="/manageSubscriptions">{preferredHeadingCase("Manage Subscriptions")}</Link>
              )}
              {showMessageButton && (
              ...
              {userCanEditUser(currentUser, user) && (
                <Link to={userGetEditUrl(user)}>{preferredHeadingCase("Account Settings")}</Link>
              )}
            </Typography>
            ...
            {user.htmlBio && (
              <ContentStyles contentType="post">
                <ContentItemBody
                  ...
                  nofollow={(user.karma || 0) < nofollowKarmaThreshold.get()}
                />
              </ContentStyles>
            )}
          </SingleColumnSection>
          ...
          {/* Sequences Section */}
          {displaySequenceSection(ownPage, user) && (
            <SingleColumnSection>
              <SectionTitle title="Sequences">{ownPage && <SequencesNewButton />}</SectionTitle>
              <Components.SequencesGridWrapper terms={ownPage ? sequenceAllTerms : sequenceTerms} showLoadMore={true} />
            </SingleColumnSection>
          )}
          ...
          {ownPage && nonAFMember && (
            <SingleColumnSection>
              <Components.LWTooltip
                inlineBlock={false}
                title="Your posts are pending approval to the Alignment Forum and are only visible to you on the Forum. 
          ...
          <SingleColumnSection>
            <SectionTitle
              title={`${taggingNameIsSet.get() ? taggingNameCapitalSetting.get() : "Wiki"} Contributions`}
            />
          ...
          <AnalyticsContext pageSectionContext="commentsSection">
            {ownPage && nonAFMember && (
              <SingleColumnSection>
                <Components.LWTooltip
                  inlineBlock={false}
                  title="Your comments are pending approval to the Alignment Forum and are only visible to you on the Forum. 
              They are visible to everyone on LessWrong."
                >
                  <SectionTitle title={"Comment Submissions"} />
                </Components.LWTooltip>
                <Components.RecentComments
                  terms={{ view: "afSubmissions", authorIsUnreviewed: null, limit: 5, userId: user._id }}
                />
              </SingleColumnSection>
            )}
            <SingleColumnSection>
...
const UsersProfileComponent = registerComponent("UsersProfile", UsersProfileFn, { styles });
