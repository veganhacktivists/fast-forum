 ...
 */
const UserNameDeleted = ({ userShownToAdmins }: { userShownToAdmins?: UsersMinimumInfo | null }) => {
...
const UserNameDeletedWithAdminHover = ({ user }: { user: UsersMinimumInfo }) => {
  ...
  return (
    <span {...eventHandlers}>
      <LWTooltip
        title={
          <div>
            This user account has been deleted. The username is only visible to site admins, and only visible on
            hover-over.
          </div>
        }
        inlineBlock={false}
      >
        {hover ? <Link to={userGetProfileUrl(user)}>{userGetDisplayName(user)}</Link> : "[anonymous]"}
      </LWTooltip>
    </span>
  );
};
