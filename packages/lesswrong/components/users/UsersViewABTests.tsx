...
const UsersViewABTests = ({ classes }: { classes: ClassesType }) => {
  ...
  return (
    <SingleColumnSection>
      ...
      <div className={classes.explanatoryText}>
        <p>
          Your A/B test key is {getUserABTestKey(currentUser ? { user: currentUser } : { clientId })}. This is used to
          randomize your test group in future A/B tests. You can see which A/B tests are active and which group you are
          in below, and override your group allocation. (This may cause data you generate to not be counted in certain
          experiments.)
        </p>

        {!currentUser && <p>You need to log in to override your A/B test group allocation.</p>}
      </div>

      {Object.keys(allABtests).length === 0 ? (
        <p className={classes.explanatoryText}>There aren't any A/B tests active right now</p>
      ) : (
        <table className={classes.abTestsTable}>
          ...
          <tbody>
            {_.keys(allABtests).map((abTestName) => (
              <tr key={abTestName}>
                ...
                <td>
                  ...
                  {!currentUser && abTestsMetadata[abTestName].groups[allABtests[abTestName]].description}
                </td>
              </tr>
            ))}
          </tbody>
