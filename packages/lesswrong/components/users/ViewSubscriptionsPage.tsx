...
const SubscriptionsList = ({
  ...
  return (
    <div>
      ...
      {results.length === 0 && <div className={classes.subscribedItem}>{noSubscriptionsMessage}</div>}
...
const SubscribedItem = ({
  ...
  return (
    <div className={classes.subscribedItem}>
      <div className={classes.subscribedItemDescription}>{renderDocument(document)}</div>
      <NotifyMeButton
        document={document}
...
const ViewSubscriptionsPage = ({ classes }: { classes: ClassesType }) => {
  const { SingleColumnSection, SubscriptionsList, UsersNameDisplay } = Components;
  const currentUser = useCurrentUser();

  if (!currentUser) {
    return <SingleColumnSection>Log in to manage your subscriptions.</SingleColumnSection>;
  }
  ...
  return (
    <SingleColumnSection>
      ...
      <SubscriptionsList
        ...
        renderDocument={(comment: CommentsListWithParentMetadata) => (
          <Link
