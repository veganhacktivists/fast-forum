...
import {
  dialogueMatchingPageNoSSRABTest,
  offerToAddCalendlyLink,
  showRecommendedContentInMatchForm,
} from "../../lib/abTests";
...
export type DialogueUserRowProps<V extends boolean> = V extends true
...
type UserTableProps<V extends boolean> = V extends false
...
const xsGridBaseStyles = (theme: ThemeType) => ({
  ...
  background: `repeating-linear-gradient(${theme.palette.background.default} 0 60px,transparent 60px 120px)`,
});
...
const useScrollGradient = (ref: React.RefObject<HTMLDivElement>) => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (element) {
        const atTop = element.scrollTop <= element.scrollHeight * 0.1;
...
export const getRowProps = (
  tableProps: Omit<UserTableProps<boolean>, "classes" | "gridClassName" | "showHeaders">,
): DialogueUserRowProps<boolean>[] => {
      ...
      {readPosts.length > 0 ? (
        readPosts.map((post, index) => (
          <PostsTooltip key={index} postId={post._id}>
            <Link key={index} to={postGetPageUrl(post)}>
              • {post.title}{" "}
            </Link>
            <br />
          </PostsTooltip>
...
const Headers = ({
  ...
  titles: string[];
  headerClasses: { [title: string]: string[] };
  classes: ClassesType<typeof styles>;
}) => {
  return (
    <>
      {titles.map((title, index) => (
        <h5 key={index} className={classNames(classes.header, ...(headerClasses[title] ?? []))}>
          {" "}
          {title}{" "}
        </h5>
      ))}
...
export type ExtendedDialogueMatchPreferenceTopic = DbDialogueMatchPreference["topicPreferences"][number] & {
  matchedPersonPreference?: "Yes" | "Meh" | "No";
...
const UserRecommendedContent = ({ classes, targetUserId }: UserRecommendedContentProps) => {
  ...
  const tagsSentence = topTags
    .slice(0, 4)
    .map((tag) => tag.tag.name)
    .join(", ");
  const numRecommendations =
    (readPosts?.length ?? 0) + (recommendedComments?.length ?? 0) + (tagsSentence === "" ? 0 : 1);
  ...
  const allRecommendations: { reactIconName: string; prefix: string; Content: JSX.Element }[] = [
    ...
    ...recommendedComments.map((comment) => ({
      reactIconName: "elaborate",
      prefix: "comment: ",
      Content: <CommentView comment={comment} />,
    })),
  ];
  ...
  return (
    <AnalyticsContext pageElementContext={"userRecommendedContent"}>
      <div className={classes.contentRecommendationsCard}>
        <div className={classes.cardTitle}>Recommended content</div>
        <div
          className={classNames(classes.recommendedContentContainer, {
            [classes.recommendedContentContainerExpandedMobile]: isExpanded,
          })}
        >
          <div className={classes.contentRecommendationsList}>
            {allRecommendations.slice(0, numShown).map((item, index) => (
              <TopicSuggestion
                key={index}
                reactIconName={item.reactIconName}
                prefix={item.prefix}
...
const NextStepsDialog = ({
  ...
  const {
    create,
    called,
    loading: loadingCreatedMatchPreference,
    data: newMatchPreference,
  } = useCreate({
  ...
  const onSubmit = async () => {
    ...
    const response = await create({
      data: {
        ...
        topicPreferences: updatedTopicPreferences.map((topic) => ({
          ...topic,
          preference: topic.preference ?? "No",
          matchedPersonPreference: undefined,
  ...
  const topicRecommendations:
    | {
        comment: { _id: string; contents: { html: string; plaintextMainText: string } };
        recommendationReason: string;
        yourVote: string;
        theirVote: string;
      }[]
    | undefined = data?.GetTwoUserTopicRecommendations; // Note CommentsList is too permissive here, but making my own type seemed too hard

  const getTopicDict = (
    prefs: DialogueMatchPreferencesDefaultFragment,
    own: boolean,
  ): { [topic: string]: ExtendedDialogueMatchPreferenceTopic } => {
    const prefsDictList = prefs.topicPreferences
      .filter(({ preference }) => preference === "Yes")
      .map((topic) => [
        topic.text,
        { ...topic, preference: own ? "Yes" : undefined, matchedPersonPreference: own ? undefined : "Yes" },
      ]);
  ...
  const [topicPreferences, setTopicPreferences] = useState<ExtendedDialogueMatchPreferenceTopic[]>(
    Object.values(initialTopicDict),
  );
  ...
  useEffect(
    () =>
      setTopicPreferences((topicPreferences) => {
        ...
        const newRecommendedTopicDict = keyBy(
          topicRecommendations.map(({ comment, recommendationReason, yourVote, theirVote }) => ({
  ...
  const [recommendedTopics, userSuggestedTopics] = partition(
    topicPreferences,
    (topic) => topic.matchedPersonPreference !== "Yes",
  );

  if (
    called &&
    !loadingCreatedMatchPreference &&
    !(newMatchPreference as any)?.createDialogueMatchPreference?.data?.generatedDialogueId
  ) {
    return (
      <LWDialog open onClose={onCloseWrapper}>
        <DialogTitle>
          ...
          <p>
            Once they fill out the form, you'll get to see each other's answers and chat about whether a dialogue makes
            sense.
          </p>
        </DialogTitle>
    ...
    <LWDialog open onClose={onCloseWrapper} className={classes.mobileDialog}>
      <div className={classes.dialogBox}>
        <DialogTitle className={classes.dialogueTitle}>
          <span className={classes.matchHeader}>(Match)</span> {targetUserDisplayName}
        </DialogTitle>
        <DialogContent>
          {showRecommendedContent === "show" && (
          ...
          {userSuggestedTopics.length > 0 && (
            <>
              <div className={classes.topicsOrigin}>
                {targetUserDisplayName} was interested in the below. Check ones you like.
              </div>
              <div className={classes.dialogueTopicList}>
                {userSuggestedTopics.map((topic) => (
                  <div className={classes.dialogueTopicRow} key={topic.text}>
                    <Checkbox
                      ...
                      onChange={(event) => {
                        ...
                        const updatedTopic = {
                          ...topicPreferences[topicToUpdateIx],
                          preference: event.target.checked ? ("Yes" as const) : ("No" as const),
                        };
                        setTopicPreferences([
                          ...topicPreferences.slice(0, topicToUpdateIx),
                          updatedTopic,
                          ...topicPreferences.slice(topicToUpdateIx + 1),
                        ]);
                      }}
                    />
                    <div className={classes.dialogueTopicRowTopicText}>{topic.text}</div>
                  </div>
                ))}
              </div>
            </>
            ...
            {recommendedTopics.map((topic) => (
              <div className={classes.dialogueTopicRow} key={topic.text}>
                ...
                <span className={classes.recommendationReasons}>
                  {topic.yourVote && (
                    <LWTooltip title={`You reacted with ${topic.yourVote} to this`}>
                      <ReactionIcon size={13} react={topic.yourVote} />
                    </LWTooltip>
                  )}
                  {topic.theirVote && (
                    <LWTooltip title={`${targetUserDisplayName} reacted with ${topic.theirVote} to this`}>
                      <ReactionIcon size={13} react={topic.theirVote} />
                    </LWTooltip>
                  )}
                  {topic.recommendationReason === "This comment is popular" && (
                    <LWTooltip title="This comment is popular">
                      <ReactionIcon size={13} react={"excitement"} />
                    </LWTooltip>
                  )}
                </span>
              </div>
          ...
          <div className={classes.dialogueFormatGrid}>
            ...
            <ScheduleLabels />
            <div className={classes.schedulingQuestion}>
              <span className={classes.syncText}>Sync:</span> Find a synchronous 1-3hr block to sit down and dialogue
            </div>
            {SYNC_PREFERENCE_VALUES.map((value, idx) => (
              <Checkbox
                key={value}
            ...
            <div className={classes.schedulingQuestion}>
              <span className={classes.syncText}>Async:</span> Have an asynchronous dialogue where you reply where
              convenient
            </div>
            {SYNC_PREFERENCE_VALUES.map((value, idx) => (
              <Checkbox
...
const DialogueCheckBox: React.FC<{
  ...
  async function handleNewMatchAnonymisedAnalytics() {
    ...
    const webhookURL = isProduction
      ? "https://hooks.slack.com/triggers/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"
      : "https://hooks.slack.com/triggers/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"; //
...
const DialogueNextStepsButton: React.FC<DialogueNextStepsButtonProps> = ({
  ...
  if (matchPreference) {
    return <div className={classes.waitingMessage}>Waiting for {targetUserDisplayName}...</div>;
  }
...
const DialogueUserRow = <V extends boolean>(props: DialogueUserRowProps<V> & { classes: ClassesType }): JSX.Element => {
  ...
  return (
    <React.Fragment key={`${targetUser._id}_other`}>
      ...
      />
      <UsersName className={classes.displayName} documentId={targetUser._id} simple={false} />
      <MessageButton targetUserId={targetUser._id} currentUser={currentUser} isMatched={userIsMatched} />
      {!userIsMatched ? (
      ...
      {showKarma && (
        <div className={classNames(classes.hideAtXs, classes.centeredText)}> {targetUser.total_power} </div>
      )}
      {showAgreement && (
        <div className={classNames(classes.hideAtXs, classes.centeredText)}> {targetUser.total_agreement} </div>
      )}
      {showBio && <UserBio key={targetUser._id} classes={classes} userId={targetUser._id} />}
      {showFrequentCommentedTopics && <UserTopTags classes={classes} targetUserId={targetUser._id} />}
      {showPostsYouveRead && (
        <UserPostsYouveRead classes={classes} targetUserId={targetUser._id} limit={8} hideAtSm={showKarma} />
      )}
    </React.Fragment>
  );
};
...
const UserTable = <V extends boolean>(props: UserTableProps<V>) => {
  const { users, classes, gridClassName, userDialogueChecks, showHeaders, tableContext, ...rest } = props;
...
export const DialogueMatchingPage = ({ classes }: { classes: ClassesType<typeof styles> }) => {
  const { captureEvent } = useTracking(); //it is virtuous to add analytics tracking to new components
  ...
  } = useDialogueMatchmaking(currentUser, {
    getMatchedUsers: true,
    getOptedInUsers: true,
    getUserDialogueChecks: { limit: 1000 },
  });
  ...
  const matchedUsersWithSelf: UsersOptedInToDialogueFacilitation[] | undefined =
    matchedUsersResult?.GetDialogueMatchedUsers;
  ...
  const dialogueUsers = userDialogueUsefulData?.dialogueUsers.filter(
    (user) => !matchedUserIds.includes(user._id) && !(user._id === currentUser._id),
  );
  const optedInUsers = usersOptedInToDialogueFacilitation.filter(
    (user) => !matchedUserIds.includes(user._id) && !(user._id === currentUser._id),
  );
  const activeDialogueMatchSeekers = userDialogueUsefulData?.activeDialogueMatchSeekers.filter(
    (user) => !matchedUserIds.includes(user._id) && !(user._id === currentUser._id),
  );
  ...
  if (userDialogueChecks?.length > 1000) {
    throw new Error(
      `Warning: userDialogueChecks.length > 1000, seems user has checked more than a thousand boxes? how is that even possible? let a dev know and we'll fix it...`,
    );
  }
    ...
    <AnalyticsContext pageContext={"dialogueMatchingPage"}>
      <div className={classes.root}>
        ...
        <p className={classes.privacyNote}>
          On privacy: LessWrong team does not look at user’s checks unless you opted in. We do track metadata, like “Two
          users just matched”, to help us know whether the feature is getting used. If one user opts in to revealing
          their checks we can still not see their matches, unless the other part of the match has also opted in.
        </p>
...
const DialogueNextStepsButtonComponent = registerComponent("DialogueNextStepsButton", DialogueNextStepsButton, {
  styles,
});
