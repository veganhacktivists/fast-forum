        ...
        {(settings.updateFrequency === "daily" || settings.updateFrequency === "weekly") && (
          <React.Fragment>
            ...
            {settings.updateFrequency === "weekly" && (
              <React.Fragment>
                {" on "}
                <Select value={dayOfWeek} onChange={(event) => this.setBatchingDayOfWeek(event.target.value, timezone)}>
    ...
    return (
      <div className={classes.root}>
        ...
        <Typography variant="body2">
          Shows reactions, upvotes and downvotes on your posts and comments on top of the page. By default, this is on
          but only updates once per day, to avoid creating a distracting temptation to frequently recheck it. Can be set
          to real time (removing the batching), disabled (to remove it from the header entirely), or to some other
          update interval.
        </Typography>
        <RadioGroup
        ...
        {settings.updateFrequency === "realtime" && (
          <span>
            Warning: Immediate karma updates may lead to over-updating on tiny amounts of feedback, and to checking the
            site frequently when you'd rather be doing something else.
          </span>
        )}
        {
          <FormControlLabel
...
const KarmaChangeNotifierSettingsComponent = registerComponent(
