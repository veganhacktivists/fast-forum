...
const EAUsersProfile = ({ terms, slug, classes }: { terms: UsersViewTerms; slug: string; classes: ClassesType }) => {
  ...
  const [draftsSectionExpanded, setDraftsSectionExpanded] = useState(
  ...
  const privateSectionTabs: Array<UserProfileTabType> = [
    {
      ...
      label: `Drafts`,
      secondaryNode: (
        <LWTooltip title="This section is only visible to you and site admins.">
      ...
      body: (
        <>
          <div className={classes.sectionSubHeadingRow}>
            <Typography variant="headline" className={classes.sectionSubHeading}>
              Draft/hidden posts
            </Typography>
            {ownPage && (
              <Link to="/newPost">
          ...
          <SequencesGridWrapper
            terms={{ view: "userProfilePrivate", userId: user._id, limit: 3 }}
            showLoadMore={true}
          />
        </>
      ),
        ...
        <>
          ...
          {user.howOthersCanHelpMe?.html && (
            <>
              ...
              <ContentStyles contentType="post">
                <ContentItemBody
                  dangerouslySetInnerHTML={{ __html: user.howOthersCanHelpMe.html }}
                  nofollow={userKarma < nofollowKarmaThreshold.get()}
                />
              </ContentStyles>
            </>
          ...
          {user.howICanHelpOthers?.html && (
            <>
              ...
              <ContentStyles contentType="post">
                <ContentItemBody
                  dangerouslySetInnerHTML={{ __html: user.howICanHelpOthers.html }}
                  nofollow={userKarma < nofollowKarmaThreshold.get()}
                />
              </ContentStyles>
            </>
          ...
          <ContentStyles contentType="post">
            <ul>
              {user.organizerOfGroups?.map((group) => {
                return (
                  <li key={group._id}>
                    Organizer of <Link to={`/groups/${group._id}`}>{group.name}</Link>
                  </li>
                );
              })}
              {user.programParticipation?.map((participation) => {
                ...
                return <li key={participation}>{label}</li>;
              })}
            </ul>
          </ContentStyles>
        </>
      ...
      <AnalyticsContext pageContext="userPage">
        <SingleColumnSection>
          <div className={classNames(classes.section, classes.mainSection)}>
            {userCanEditUser(currentUser, user) && (
              <div className={classes.editProfile}>
                <Button type="submit" href={`/profile/${user.slug}/edit`} className={classes.editProfileButton}>
                  Edit profile
                </Button>
              </div>
            )}
            ...
            <Typography
              variant="headline"
              className={classNames(classes.username, { [classes.deletedUsername]: user.deleted })}
            >
              {username}
              {user.deleted && <span className={classes.accountDeletedText}>(account deleted)</span>}
          ...
          {userCanDo(currentUser, "posts.moderate.all") && (
            <div className={classes.sunshineSection}>
              <SunshineNewUsersProfileInfo userId={user._id} />
            </div>
          )}

          {(ownPage || currentUser?.isAdmin) &&
            (draftsSectionExpanded ? (
              <EAUsersProfileTabbedSection tabs={privateSectionTabs} />
            ) : (
              <Button color="primary" onClick={() => setDraftsSectionExpanded(true)} className={classes.showSectionBtn}>
...
const EAUsersProfileComponent = registerComponent("EAUsersProfile", EAUsersProfile, { styles });
