...
const PasswordResetPage = ({ classes }: { classes: ClassesType }) => {
  const { mutate: emailTokenMutation } = useNamedMutation({
    name: "useEmailToken",
    graphqlArgs: { token: "String", args: "JSON" },
  });
  ...
  const ResultComponent =
    useTokenResult?.componentName && Components[useTokenResult.componentName as keyof ComponentTypes];
  return (
    <SingleColumnSection className={classes.root}>
      {!useTokenResult && (
        <>
          ...
          />
          <Button onClick={submitFunction} className={classes.submit}>
            Set New Password
          </Button>
        </>
      )}
