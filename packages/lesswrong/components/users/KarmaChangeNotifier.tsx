...
// zero, respectively.
const ColoredNumber = ({ n, classes }: { n: number; classes: ClassesType }) => {
  if (n > 0) {
      ...
      {noKarmaChanges ? (
        ...
        <div>
          ...
          <div className={classes.votedItems}>
            {karmaChanges.posts &&
              karmaChanges.posts.map((postChange: AnyBecauseTodo) => (
                <MenuItemLink className={classes.votedItemRow} to={postGetPageUrl(postChange)} key={postChange._id}>
                  {postChange.scoreChange !== 0 && (
                  ...
                  <div className={classes.votedItemDescription}>{postChange.title}</div>
                </MenuItemLink>
              ))}
            {karmaChanges.comments &&
              karmaChanges.comments.map((commentChange: AnyBecauseTodo) => (
                <MenuItemLink
                  ...
                  to={commentGetPageUrlFromIds({
                    postId: commentChange.postId,
                    tagSlug: commentChange.tagSlug,
                    tagCommentType: commentChange.tagCommentType as TagCommentType,
                    commentId: commentChange._id,
                  })}
                  key={commentChange._id}
                >
                  {commentChange.scoreChange !== 0 && (
                    <span className={classes.votedItemScoreChange}>
                      <ColoredNumber n={commentChange.scoreChange} classes={classes} />
                    </span>
                  )}
                  {!!commentChange.addedReacts.length && (
                    <span className={classes.votedItemReacts}>
                      <NewReactions reactionChanges={commentChange.addedReacts} classes={classes} />
                    </span>
                  )}
                  <div className={classes.votedItemDescription}>{commentChange.description}</div>
                </MenuItemLink>
              ))}
            {karmaChanges.tagRevisions.map((tagChange: AnyBecauseTodo) => (
              ...
              >
                ...
                <div className={classes.votedItemDescription}>{tagChange.tagName}</div>
              </MenuItemLink>
            ))}
          </div>
        </div>
...
const KarmaChangeNotifier = ({
  ...
  const render = () => {
    ...
    if (settings && settings.updateFrequency === "disabled") return null;
    ...
    const starIsHollow =
      (comments.length === 0 && posts.length === 0 && tagRevisions.length === 0) ||
      cleared ||
      !newKarmaChangesSinceLastVisit;
...
const NewReactions = ({ reactionChanges, classes }: { reactionChanges: ReactionChange[]; classes: ClassesType }) => {
  const { ReactionIcon, LWTooltip } = Components;

  const distinctReactionTypes = new Set<string>();
  for (let reactionChange of reactionChanges) distinctReactionTypes.add(reactionChange.reactionType);
  ...
  return (
    <span>
      {[...distinctReactionTypes.keys()].map((reactionType) => {
        ...
        return (
          <span className={classes.individualAddedReact} key={reactionType}>
            <LWTooltip
            ...
            >
              {EAEmojiComponent && isEAForum ? <EAEmojiComponent /> : <ReactionIcon react={reactionType} />}
            </LWTooltip>
          </span>
        );
      })}
