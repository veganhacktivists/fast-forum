...
const EmailConfirmationRequiredCheckbox = (props: any) => {
  ...
  if (userEmailAddressIsVerified(currentUser)) {
    return <Components.FormComponentCheckbox {...props} />;
  } else {
    return (
      <Components.FormComponentCheckbox disabled {...otherProps} label={`${label} (verify your email address first)`} />
    );
  }
};

const EmailConfirmationRequiredCheckboxComponent = registerComponent(
