...
} from "../../lib/vulcan-users/permissions";
...
const UsersMenu = ({ classes }: { classes: ClassesType }) => {
    ...
    return (
      <div className={classes.root}>
        <Button href="/logout" classes={{ root: classes.userButtonRoot }}>
          <span className={classes.userButtonContents}>{isFriendlyUI ? "Log out" : "LOG OUT"}</span>
        </Button>
      </div>
    );
  }
  ...
  let userButtonNode = (
    <span className={classes.userButtonContents}>
      ...
      {currentUser.deleted && (
        <LWTooltip
          title={
            <div className={classes.deactivatedTooltip}>
  ...
  if (isFriendlyUI && !currentUser.deleted) {
    userButtonNode = (
      <div className={classes.userImageButton}>
  ...
  const canCreateDialogue =
    userCanPost(currentUser) && dialoguesEnabled && userOverNKarmaOrApproved(MINIMUM_COAUTHOR_KARMA)(currentUser);

  const items = {
    divider: DropdownDivider,
    newPost: () =>
      userCanPost(currentUser) ? <DropdownItem title={preferredHeadingCase("New Post")} to="/newPost" /> : null,
    newQuestion: () =>
      userCanPost(currentUser) ? (
        <DropdownItem title={preferredHeadingCase("New Question")} to="/newPost?question=true" />
      ) : null,
    newDialogue: () =>
      canCreateDialogue ? (
        <DropdownItem
    ...
    newEvent: () => (userCanPost(currentUser) ? null : null),
    newSequence: () =>
      showNewButtons && currentUser.karma >= SHOW_NEW_SEQUENCE_KARMA_THRESHOLD ? (
        <DropdownItem title={preferredHeadingCase("New Sequence")} to="/sequencesnew" />
      ) : null,
  } as const;
        ...
        <Paper>
          <DropdownMenu>
            ...
            >
              ...
              {isEAForum && userIsAdminOrMod(currentUser) && (
                <DropdownItem
                  title={"Post stats"}
              ...
              */}
              {currentUser.isAdmin && (
                <div className={classes.adminToggleItem}>
                  <DropdownItem title={preferredHeadingCase("Disable Admin Powers")} onClick={toggleOff} />
                </div>
              )}
              {!currentUser.isAdmin && userIsMemberOf(currentUser, "realAdmins") && (
                <div className={classes.adminToggleItem}>
                  <DropdownItem title={preferredHeadingCase("Re-enable Admin Powers")} onClick={toggleOn} />
                </div>
              )}

              <DropdownDivider />

              <DropdownItem title={preferredHeadingCase("Log Out")} to="/logout" rawLink />
            </div>
          </DropdownMenu>
        </Paper>
      </LWPopper>
