...
const StickyWrapper: FC<{
  ...
  eaHomeLayout ? (
    <StickyBox offsetTop={0} offsetBottom={20}>
      <div
        className={classNames(classes.stickyWrapper, {
...
const Layout = ({
  ...
  const { explicitConsentGiven: cookieConsentGiven, explicitConsentRequired: cookieConsentRequired } =
  ...
  const render = () => {
    ...
    const baseLayoutOptions: LayoutOptions = {
      ...
      renderSunshineSidebar:
        !!currentRoute?.sunshineSidebar &&
        !!(userCanDo(currentUser, "posts.moderate.all") || currentUser?.groups?.includes("alignmentForumAdmins")),
    ...
    const renderSunshineSidebar =
      overrideLayoutOptions.renderSunshineSidebar ?? baseLayoutOptions.renderSunshineSidebar;
    ...
    const showNewUserCompleteProfile =
    ...
    const renderPetrovDay = () => {
      ...
      return currentRoute?.name === "home" && isLW && beforeTime < currentTime && currentTime < afterTime;
    };
                        ...
                        <CommentBoxManager>
                          <Helmet>
                            {theme.typography.fontDownloads &&
                              theme.typography.fontDownloads.map((url: string) => (
                                <link rel="stylesheet" key={`font-${url}`} href={url} />
                              ))}
                          ...
                          <noscript className="noscript-warning">
                            {" "}
                            This website requires javascript to properly function. Consider activating javascript to get
                            access to all site functionality.{" "}
                          </noscript>
                          {googleTagManagerIdSetting.get() && (
                            // Google Tag Manager i-frame fallback
                            <noscript>
                              <iframe
                                src={`https://www.googletagmanager.com/ns.html?id=${googleTagManagerIdSetting.get()}`}
                                height="0"
                                width="0"
                          ...
                          {/* enable during ACX Everywhere */}
                          {renderCommunityMap && (
                            <span className={classes.hideHomepageMapOnMobile}>
                          ...
                          <div
                            className={classNames(classes.standaloneNavFlex, {
                          ...
                          >
                            ...
                            {!renderSunshineSidebar && friendlyHomeLayout && !showNewUserCompleteProfile && (
                              <StickyWrapper
                                eaHomeLayout={friendlyHomeLayout}
                                headerVisible={headerVisible}
                                headerAtTop={headerAtTop}
