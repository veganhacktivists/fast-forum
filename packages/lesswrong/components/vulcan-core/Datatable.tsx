...
const getColumnName = (column: Column) => (typeof column === "string" ? column : column.label || column.name);
...
>({
  ...
  return (
    <DatatableLayout collectionName={collection.options.collectionName}>
      <DatatableContents
        columns={columns}
...
const DatatableHeader = ({ collection, column }: { collection: CollectionBase<any>; column: Column }) => {
  ...
  if (collection) {
    ...
    */
    ...
    return (
      <DatatableHeaderCellLayout className={`datatable-th-${columnName.toLowerCase().replace(/\s/g, "-")}`}>
        {formattedLabel}
...
const DatatableHeaderCellLayout = ({
  children,
  ...otherProps
}: {
  children: React.ReactNode;
    ...
    <DatatableContentsLayout>
      ...
      <DatatableContentsInnerLayout>
        ...
        <DatatableContentsBodyLayout>
          {results.map((document: any, index: number) => (
            <DatatableRow {...props} collection={collection} columns={columns} document={document} key={index} />
          ))}
        </DatatableContentsBodyLayout>
      </DatatableContentsInnerLayout>
      {hasMore && (
...
const DatatableContentsLayout = ({ children }: { children: React.ReactNode }) => (
  <div className="datatable-list">{children}</div>
);
const DatatableContentsInnerLayout = ({ children }: { children: React.ReactNode }) => (
  <table className="table">{children}</table>
);
const DatatableContentsHeadLayout = ({ children }: { children: React.ReactNode }) => (
  <thead>
    <tr>{children}</tr>
  </thead>
);
const DatatableContentsBodyLayout = ({ children }: { children: React.ReactNode }) => <tbody>{children}</tbody>;
const DatatableContentsMoreLayout = ({ children }: { children: React.ReactNode }) => (
  <div className="datatable-list-load-more">{children}</div>
);

const DatatableTitle = ({ title }: { title: string }) => {
...
const DatatableRow = (props: DatatableRowProps) => {
  ...
  return (
    <DatatableRowLayout className={`datatable-item`}>
      {sortedColumns.map((column, index) => (
        <DatatableCell key={index} column={column} document={document} />
      ))}
    </DatatableRowLayout>
  );
};

const DatatableRowLayout = ({
  children,
  ...otherProps
}: {
  children: React.ReactNode;
...
const DatatableCell = ({ column, document }: { column: Column; document: any }) => {
...
const DatatableDefaultCell = ({ column, document }: { column: Column; document: any }) => {
  return (
