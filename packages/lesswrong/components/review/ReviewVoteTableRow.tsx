...
const styles = (theme: ThemeType) => ({
  ...
  votingPhase: {
    ...
    boxShadow: `0 1px 3px 0px ${theme.palette.boxShadowColor(0.05)}`,
  },
  voteIcon: {
    padding: 0,
  },
...
export type voteTooltipType =
  | "Showing votes by 1000+ Karma LessWrong users"
  | "Showing all votes"
...
const ReviewVoteTableRow = ({
  ...
  const currentUserIsAuthor =
    currentUser &&
    (post.userId === currentUser._id || post.coauthors?.map((author) => author?._id).includes(currentUser._id));
  ...
  return (
    <AnalyticsContext pageElementContext="voteTableRow">
      <div
        className={classNames(classes.root, {
          [classes.expanded]: expanded,
          [classes.votingPhase]: reviewPhase === "VOTING",
        })}
        onClick={markAsRead}
      >
        ...
        <div className={classNames(classes.postVote, { [classes.postVoteVotingPhase]: reviewPhase === "VOTING" })}>
              ...
              <LWTooltip
                title={
                  <div>
                    ...
                    <div>
                      <em>(It needs at least 2 to proceed to the Review Phase.)</em>
                    </div>
                  </div>
                }
              >
          ...
          {(reviewPhase === "NOMINATIONS" || reviewPhase === "REVIEWS" || reviewPhase === "COMPLETE") && (
            <PostsItem2MetaInfo className={classes.count}>
          ...
          {(reviewPhase === "REVIEWS" || reviewPhase === "COMPLETE") && (
            <div className={classes.votes}>
              ...
              {currentUserIsAuthor && (
                <LWTooltip title="You can't vote on your own posts">
              ...
              {reviewPhase === "COMPLETE" && !currentUserIsAuthor && (
                <LWTooltip title={"Your review vote for this post"}>
                  <span className={classes.oldVote}>{userReviewVote}</span>
                </LWTooltip>
              )}
            </div>
          )}
          {(reviewPhase === "NOMINATIONS" || reviewPhase === "VOTING") && eligibleToNominate(currentUser) && (
            <div className={classNames(classes.votes, { [classes.votesVotingPhase]: reviewPhase === "VOTING" })}>
              {!currentUserIsAuthor && (
                <ReviewVotingButtons
                  post={post}
                  dispatch={dispatch}
              ...
              {currentUserIsAuthor && (
                <MetaInfo className={classes.cantVote}>You can't vote on your own posts</MetaInfo>
              )}
            </div>
          )}
        </div>
