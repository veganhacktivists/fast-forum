  ...
  return (
    <span
      className={classNames(classes.root, {
        [classes.active]: currentReactions.includes(reaction),
        [classes.textEntryOpen]: textFieldOpen,
      })}
      onClick={createClickHandler(
        postId,
        currentReactions.includes(reaction) ? _.without(currentReactions, reaction) : [...currentReactions, reaction],
        voteForCurrentPost?.score,
      )}
    >
      {textFieldOpen ? (
        <Input
          ...
          endAdornment={
            <InputAdornment position="end">
              <IconButton aria-label="Toggle password visibility" onClick={submitFreeEntryText}>
                <CheckIcon />
              </IconButton>
            </InputAdornment>
          }
