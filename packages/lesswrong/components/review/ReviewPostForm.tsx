...
const ReviewPostForm = ({ classes, post, onClose }: { classes: ClassesType; post: PostsBase; onClose: () => void }) => {
        ...
        <div className={classes.guidelines}>
          {showPrompt && (
            <div>
              ...
              <div className={classes.moderatorsNote}>
                Moderators may promote comprehensive reviews to top-level posts.
              </div>
              <a className={classes.hidePrompt} onClick={() => setShowPrompt(false)}>
                (click to hide)
              </a>
            </div>
          )}
          {!showPrompt && (
            <div onClick={() => setShowPrompt(true)}>
              Reviews should ideally answer... <a onClick={() => setShowPrompt(false)}>(read more)</a>
            </div>
          )}
        </div>
      }
