...
import {
  annualReviewAnnouncementPostPathSetting,
  annualReviewEnd,
  annualReviewNominationPhaseEnd,
  annualReviewReviewPhaseEnd,
  annualReviewStart,
...
const styles = (theme: ThemeType): JssStyles => ({
  ...
  progress: {
    ...
    marginBottom: 2,
    padding: 4,
    backgroundColor: theme.palette.greyAlpha(0.14),
    display: "flex",
    justifyContent: "space-between",
    "&:hover": {
...
export function ReviewOverviewTooltip() {
  ...
  const nominationPhaseDateRange = (
    <span>
      {nominationStartDate.format("MMM Do")} – {nominationEndDate.format("MMM Do")}
    </span>
  );
  const reviewPhaseDateRange = (
    <span>
      {nominationEndDate.clone().format("MMM Do")} – {reviewEndDate.format("MMM Do")}
    </span>
  );
  const votingPhaseDateRange = (
    <span>
      {reviewEndDate.clone().format("MMM Do")} – {voteEndDate.format("MMM Do")}
    </span>
  );
  ...
  if (isEAForum) {
    return (
      <div>
        ...
        <ul>
          <li>
            <em>Nomination</em> ({nominationPhaseDateRange})
          </li>
          <li>
            <em>Review</em> ({reviewPhaseDateRange})
          </li>
          <li>
            <em>Voting</em> ({votingPhaseDateRange})
          </li>
        </ul>
    ...
    return (
      <div>
        <div>
          The {forumTitle} community is reflecting on the best posts from {REVIEW_YEAR}, in three phases:
        </div>
        <ul>
          <li>
            <em>Nomination Voting</em> ({nominationPhaseDateRange})
          </li>
          <li>
            <em>Review</em> ({reviewPhaseDateRange})
          </li>
          <li>
            <em>Final Voting</em> ({votingPhaseDateRange})
          </li>
        </ul>
        <div>
          The {forumTitle} moderation team will incorporate that information, along with their judgment, into a "Best of{" "}
          {REVIEW_YEAR}" sequence.
        </div>
        <p>
          We're currently in the nomination voting phase. Nominate posts by casting a nomination vote, or vote on
          existing nominations to help us prioritize them during the Review Phase.
        </p>
      </div>
    );
  }
...
const FrontpageReviewWidget = ({
  classes,
  showFrontpageItems = true,
  reviewYear,
}: {
  classes: ClassesType;
  showFrontpageItems?: boolean;
  ...
  const {
    ...
    LWTooltip,
    PostsList2,
    UserReviewsProgressBar,
    ReviewVotingProgressBar,
  ...
  const nominationsTooltip = isEAForum ? (
    <div>
      ...
      <div>
        If you've been actively reading {siteNameWithArticleSetting.get()} before now, but didn't register an account,
        reach out to us on intercom.
      </div>
      {activeRange === "NOMINATIONS" && (
        <div>
          <em>{nominationEndDate.fromNow()} remaining</em>
        </div>
      )}
    </div>
  ) : (
    <div>
      ...
      <ul>
        <li>
          Nominate a post by casting a <em>nomination vote</em>. Or, vote on an existing nominated post to help us
          prioritize it during the Review Phase.
        </li>
  ...
  const reviewTooltip = isEAForum ? (
    <>
      <div>
        Review posts for the {REVIEW_NAME_IN_SITU} (Opens {nominationEndDate.clone().format("MMM Do")})
      </div>
      <ul>
      ...
      {activeRange === "REVIEWS" && (
        <div>
          <em>{reviewEndDate.fromNow()} remaining</em>
        </div>
      )}
    </>
  ) : (
    <>
      <div>
        Review posts for the {reviewYear} Review (Opens {nominationEndDate.clone().format("MMM Do")})
      </div>
      <ul>
  ...
  const voteTooltip = isEAForum ? (
    <>
      <div>
        Cast your final votes for the {REVIEW_NAME_IN_SITU}. (Opens {reviewEndDate.clone().format("MMM Do")})
      </div>
      <ul>
      ...
      {activeRange === "REVIEWS" && (
        <div>
          <em>{voteEndDate.fromNow()} remaining</em>
        </div>
      )}
    </>
  ) : (
    <>
      <div>
        Cast your final votes for the {reviewYear} Review. (Opens {reviewEndDate.clone().format("MMM Do")})
      </div>
      <ul>
        <li>Look over nominated posts and vote on them</li>
  ...
  const allEligiblePostsUrl = isEAForum
    ? `/allPosts?timeframe=yearly&before=${reviewYear + 1}-01-01&limit=25&sortedBy=top&filter=unnominated&includeShortform=false`
    : `/allPosts?timeframe=yearly&after=${reviewYear}-01-01&before=${reviewYear + 1}-01-01&limit=100&sortedBy=top&filter=all&includeShortform=false`;
    ...
    <div className={classes.reviewTimeline}>
      <div className={classes.nominationBlock}>
        <LWTooltip
          ...
          title={nominationsTooltip}
          className={classNames(classes.progress, { [classes.activeProgress]: activeRange === "NOMINATIONS" })}
        >
          <div className={classNames(classes.blockText, classes.blockLabel)}>Nomination Voting</div>
          <div className={classNames(classes.blockText, classes.hideOnMobile)}>
            {nominationEndDate.format("MMM Do")}
          </div>
          {activeRange === "NOMINATIONS" && (
            <div
      ...
      <div className={classes.reviewBlock}>
        <LWTooltip
          placement="bottom-start"
          title={reviewTooltip}
          className={classNames(classes.progress, { [classes.activeProgress]: activeRange === "REVIEWS" })}
        >
          ...
          {activeRange === "REVIEWS" && (
            <div
              className={classes.coloredProgress}
              style={{ width: `${dateFraction(currentDate, nominationEndDate, reviewEndDate)}%` }}
            />
          )}
        </LWTooltip>
      </div>
      <div className={classes.votingBlock}>
        <LWTooltip
          placement="bottom-start"
          title={voteTooltip}
          className={classNames(classes.progress, { [classes.activeProgress]: activeRange === "VOTING" })}
        >
          ...
          {activeRange === "VOTING" && (
            <div
              className={classes.coloredProgress}
              style={{ width: `${dateFraction(currentDate, reviewEndDate, voteEndDate)}%` }}
            />
          )}
        </LWTooltip>
      </div>
  ...
  const nominationPhaseButtons = (
    <div className={classes.actionButtonRow}>
      ...
      {showFrontpageItems && isLastDay(nominationEndDate) && (
        <span className={classNames(classes.nominationTimeRemaining, classes.timeRemaining)}>
      ...
      <LWTooltip className={classes.buttonWrapper} title={`Nominate posts you previously upvoted.`}>
        <Link to={`/votesByYear/${reviewYear}`} className={classes.actionButton}>
          <span>
            <span className={classes.hideOnMobile}>Your</span> {isEAForum && "≤"}
            {reviewYear} Upvotes
          </span>
        </Link>
      </LWTooltip>
      ...
      <LWTooltip
        ...
        title={`Nominate posts ${isEAForum ? "in or before" : "from"} ${reviewYear}`}
      >
  ...
  const reviewPhaseButtons = (
    <div className={classes.actionButtonRow}>
      ...
      {isLastDay(reviewEndDate) && <span className={classes.timeRemaining}>{reviewEndDate.fromNow()} remaining</span>}
    </div>
  );

  const votingPhaseButtons = (
    <div className={classes.actionButtonRow}>
      {currentUser && currentUser.karma >= 1000 && (
        <span className={classes.reviewProgressBar}>
      ...
      {/* If there's less than 24 hours remaining, show the remaining time */}
      {isLastDay(voteEndDate) && <span className={classes.timeRemaining}>{voteEndDate.fromNow()} remaining</span>}
    </div>
  );

  const postList = (
    <AnalyticsContext listContext={`frontpageReviewReviews`} reviewYear={`${reviewYear}`}>
      <PostsList2
        itemsPerPage={10}
        terms={{
          view: "reviewVoting",
  ...
  return (
    <AnalyticsContext pageSectionContext="frontpageReviewWidget">
      <div>
        <SectionTitle
          title={
            <LWTooltip title={<ReviewOverviewTooltip />} placement="bottom-start">
              <Link to={"/reviewVoting"}>{getReviewTitle(reviewYear)}</Link>
            </LWTooltip>
          }
        >
...
const dateFraction = (fractionDate: moment.Moment, startDate: moment.Moment, endDate: moment.Moment) => {
  ...
  const result = ((fractionDate.unix() - startDate.unix()) / (endDate.unix() - startDate.unix())) * 100;
  if (result > 100) {
    // eslint-disable-next-line no-console
