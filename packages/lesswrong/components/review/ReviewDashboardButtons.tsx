...
export const ReviewDashboardButtons = ({
  classes,
  reviewYear,
  reviewPhase,
      ...
      <Row justifyContent="space-between">
        <SectionFooter>
          {userIsAdmin(currentUser) && (
            <LWTooltip title={`Look at metrics related to the Review`}>
              <Link to={`/reviewAdmin/${reviewYear}`}>Review Admin</Link>
            </LWTooltip>
          )}
          {reviewPhase === "NOMINATIONS" && (
            <LWTooltip
              title={`Look over your upvotes from ${reviewYear}. (This is most useful during the nomination phase, but you may still enjoy looking them over in the latter phases to help compare)`}
            >
              <Link to={`/votesByYear/${reviewYear}`}>Your {reviewYear} Upvotes</Link>
            </LWTooltip>
          )}
          <LWTooltip title={`Look at all reviews (from this year or other years)`}>
            <Link to={`/reviews/${reviewYear}`}>All Reviews</Link>
          </LWTooltip>
          {showAdvancedDashboard && (
            <LWTooltip title="Look at reviews, update your votes, and see more detailed info from the Nomination Vote results">
              <Link to={`/reviewVoting/${reviewYear}`}>Advanced Dashboard</Link>
            </LWTooltip>
          )}
          {showQuickReview && (
            <LWTooltip title="A simplified review UI">
              <Link to={`/reviewQuickPage`}>Quick Review Page</Link>
            </LWTooltip>
          )}
        </SectionFooter>
        <div>
          <LWTooltip
            title={
              <div>
                <p>
                  Write a broader review, as a top-level post. This can cover a wide variety of "Review shaped"
                  thoughts, like:
                </p>
                <ul>
                  ...
                  <li>
                    Any other meta-reflection on LessWrong, or how your thought processes have evolved since{" "}
                    {reviewYear}.
                  </li>
                </ul>
              </div>
