...
const ReviewVotingPage = ({ classes }: { classes: ClassesType }) => {
  const {
    LWTooltip,
    Loading,
  ...
  const [submitVote] = useMutation(gql`
    mutation submitReviewVote(
      $postId: String
      $qualitativeScore: Int
      $quadraticChange: Int
      $newQuadraticScore: Int
      $comment: String
      $year: String
      $dummy: Boolean
    ) {
      submitReviewVote(
        ...
        quadraticChange: $quadraticChange
        comment: $comment
        newQuadraticScore: $newQuadraticScore
        year: $year
        dummy: $dummy
      ) {
  ...
  function getCostTotal(posts: PostsReviewVotingList[] | null) {
    return (
      posts
        ?.map((post) => getCostData({})[post.currentUserReviewVote?.qualitativeScore || 0].cost)
        .reduce((a, b) => a + b, 0) ?? 0
    );
  }
  ...
  const dispatchQualitativeVote = useCallback(
    async ({ _id, postId, score }: SyntheticQualitativeVote) => {
  ...
  const reSortPosts = useCallback(
    (sortPosts: string, sortReversed: boolean, tagFilter: string | null) => {
      ...
      const filteredPosts = tagFilter
        ? filter(newlySortedPosts, (post) => post.tags.map((tag) => tag._id).includes(tagFilter))
        : newlySortedPosts;
  ...
  const costTotalTooltip =
    costTotal > 500 ? (
      <div>You have spent more than 500 points. Your vote strength will be reduced to account for this.</div>
    ) : (
      <div>You have {500 - costTotal} points remaining before your vote-weight begins to reduce.</div>
    );

  if (!reviewYear) return <SingleColumnSection>{params.year} is not a valid review year.</SingleColumnSection>;

  let voteTooltip = isAF
    ? "Showing votes from Alignment Forum members"
    : ("Showing votes from all LessWrong users" as voteTooltipType);
  switch (sortPosts) {
      ...
      <div>
        <div className={classes.grid}>
          ...
          <div className={classes.rightColumn}>
            {reviewPhase === "VOTING" && currentUser?.noSingleLineComments && (
              <ContentStyles contentType="comment" className={classes.singleLineWarning}>
                <span className={classes.warning}>You have "Do not collapse comments to single line" enabled, </span>
                which is going to make this page pretty bloated. The intended experience is for each post to have a few
                truncated reviews, which you can expand. You may want to disable the option in your{" "}
                <Link to={"/account"}>{accountSettings}</Link>
              </ContentStyles>
            )}
            ...
            <div className={classes.menu}>
              ...
              {sortedPosts && (
                <div className={classes.postCount}>
                  <LWTooltip title="Posts need at least 1 review to enter the Final Voting Phase">
                    <span className={classes.reviewedCount}>{reviewedPosts?.length || 0} Reviewed Posts</span>
                  </LWTooltip>
              ...
              {!isEAForum && eligibleToNominate(currentUser) && costTotal !== null && (
                <div className={classNames(classes.costTotal, { [classes.excessVotes]: costTotal > 500 })}>
                  <LWTooltip title={costTotalTooltip}>{costTotal}/500</LWTooltip>
                </div>
              )}
              ...
              <div className={classes.sortingOptions}>
                ...
                <Select
                  value={sortPosts}
                  onChange={(e) => {
                    updatePostSort(e.target.value);
                  }}
                  disableUnderline
                >
                  {reviewPhase === "NOMINATIONS" && (
                    <MenuItem value={"needsPreliminaryVote"}>
                      <LWTooltip
                        placement="left"
                        title={
                          <div>
                            Prioritizes posts with at least one review, which you haven't yet voted on
                            <div>
                              <em>(intended to reward reviews by making reviewed posts more prominent</em>
                            </div>
                          </div>
                        }
                      >
                        <span>
                          <span className={classes.sortBy}>Sort by</span> Magic (Prioritize reviewed)
                        </span>
                      </LWTooltip>
                    </MenuItem>
                  )}
                      ...
                      <LWTooltip
                        title={
                          <div>
                            ...
                            <div>
                              <em>(Posts need at least 2 positive votes to proceed to the Review Phase</em>
                            </div>
                          </div>
                        }
                      >
                    ...
                    <MenuItem value={"needsReview"}>
                      <LWTooltip
                        title={
                          <div>
                            <p>
                              Prioritizes posts you voted on or wrote, which haven't had a review written, and which
                              have at least 4 points.
                            </p>
                            <p>
                              <em>
                                (i.e. emphasizees posts that you'd likely want to prioritize reviewing, so that they
                                make it to the final voting)
                              </em>
                            </p>
                          </div>
                        }
                      >
                        <span>
                          <span className={classes.sortBy}>Sort by</span> Magic (Needs Review)
                        </span>
                      </LWTooltip>
                    </MenuItem>
                  )}
                  {reviewPhase === "VOTING" && (
                    <MenuItem value={"needsFinalVote"}>
                      <LWTooltip title={<div>Prioritizes posts you haven't voted on yet</div>}>
                        <span>
                          <span className={classes.sortBy}>Sort by</span> Magic (Needs Vote)
                        </span>
                      </LWTooltip>
                    </MenuItem>
                  )}
                  {reviewPhase === "COMPLETE" && (
                    <MenuItem value={"finalReviewVoteScoreHighKarma"}>
                      <span className={classes.sortBy}>Sort by</span> Final Vote Total (1000+ Karma Users)
                    </MenuItem>
                  )}
                  {reviewPhase === "COMPLETE" && (
                    <MenuItem value={"finalReviewVoteScoreAllKarma"}>
                      <span className={classes.sortBy}>Sort by</span> Final Vote Total (All Users)
                    </MenuItem>
                  )}
                  {reviewPhase === "COMPLETE" && isLW && (
                    <MenuItem value={"finalReviewVoteScoreAF"}>
            ...
            <div
              className={classNames({
                [classes.postList]: reviewPhase !== "VOTING",
                [classes.postLoading]: postsLoading || loading,
              })}
            >
              {postsHaveBeenSorted &&
                sortedPosts?.map((post) => {
                  const currentVote =
                    post.currentUserReviewVote !== null
                      ? {
