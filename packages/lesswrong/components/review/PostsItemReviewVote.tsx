...
const PostsItemReviewVote = ({
  ...
  return (
    <div onMouseLeave={() => setAnchorEl(null)}>
      <LWTooltip
        title={`${nominationsPhase ? "Nominate this post by casting a nomination vote" : "Update your vote"}`}
        placement="right"
      >
        <div
          className={classNames(classes.buttonWrapper, { [classes.marginRight]: marginRight })}
          onClick={(e) => setAnchorEl(e.target)}
        >
          {voteIndex !== 0 ? (
            <span className={classNames(classes.button, [classes[voteIndex]])}>{displayVote}</span>
          ) : (
