...
export const UserReviewsProgressBar = ({ classes, reviewYear }: { classes: ClassesType; reviewYear: ReviewYear }) => {
  const TARGET_NUM = 3;
      ...
      title={
        <div>
          <p>
            <em>
              {totalReviews
                ? `You've written ${totalReviews} reviews${totalReviews >= TARGET_NUM ? "!" : "."}`
    ...
    >
      <div className={classes.root}>
        ...
        {range(0, uncheckedBoxes).map((a) => (
          <CheckBoxOutlineBlankIcon className={classes.icon} key={`${currentUser?._id}`} />
        ))}
      </div>
    </LWTooltip>
  );
