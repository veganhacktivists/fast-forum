...
export const ReviewsPage = ({ classes }: { classes: ClassesType }) => {
  ...
  return (
    <SingleColumnSection>
      <div>
        <Link className={classNames(classes.yearLink, { [classes.selected]: "all" === params.year })} to="/reviews/all">
          All
        </Link>
        {reviewYears.map((year) => (
          <Link
            className={classNames(classes.yearLink, { [classes.selected]: year === reviewYear })}
            to={`/reviews/${year}`}
            key={year}
          >
            {year}
      ...
      <ReviewsList
        reviewYear={reviewYear ?? undefined}
        title={`Reviews ${reviewYear ?? "(All Years)"}`}
