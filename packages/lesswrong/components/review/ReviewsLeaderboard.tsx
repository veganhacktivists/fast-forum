...
export const ReviewsLeaderboard = ({
  classes,
  reviews,
  reviewYear,
}: {
  ...
  return (
    <div className={classes.root}>
      ...
      {truncatedRows.map((reviewUser) => {
        return (
          <div className={classes.row} key={reviewUser.user?._id}>
            <Row justifyContent="flex-start">
              <div className={classes.username}>
                <UsersNameDisplay user={reviewUser.user} />
              </div>
              <div className={classes.karma}>{reviewUser.totalKarma}</div>
              <div className={classes.reviews}>
                {reviewUser.reviews.map((review) => {
                  return (
                    <LWTooltip
                      placement="bottom-start"
                      title={
                        <div className={classes.card}>
                          <CommentsNode treeOptions={{ showPostTitle: true }} comment={review} />
                        </div>
                      }
                      tooltip={false}
                      key={review._id}
      ...
      <a className={classes.showAll} onClick={() => setTruncated(!truncated)}>
        {truncated ? (
          <>
            Show All Reviewers ({NUM_DEFAULT_REVIEWS}/{sortedUserRows.length})
          </>
        ) : (
          "Show Fewer"
        )}
