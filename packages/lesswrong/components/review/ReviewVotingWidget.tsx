...
const ReviewVotingWidget = ({
  ...
  const dispatchQualitativeVote = useCallback(
    async ({ _id, postId, score }: { _id: string | null; postId: string; score: VoteIndex }) => {
  ...
  return (
    <ErrorBoundary>
      <div className={classes.root}>
        {showTitle && (
          <p>
            Vote on this post for the{" "}
            <LWTooltip title={<ReviewOverviewTooltip />}>
              <Link to={"/reviewVoting"}>{REVIEW_NAME_IN_SITU}</Link>
            </LWTooltip>
          </p>
        )}
