...
const FrontpageNominationPhase = ({
  classes,
  settings,
}: {
  classes: ClassesType;
  ...
  const reviewTooltip = (
    <div>
      ...
      <ul>
        ...
        <li>
          <em>Review</em> (Dec 2nd – 31st)
        </li>
        <li>
          <em>Voting</em> (Jan 1st – 7th
        </li>
        <li>
  ...
  const review2018MonthlyUrl =
    "/allPosts?after=2018-01-01&before=2019-01-01&limit=14&timeframe=monthly&includeShortform=false&reverse=true";
  ...
  return (
    <div>
      <LWTooltip placement="top-start" title={reviewTooltip}>
        <div>
          <SectionSubtitle>
            <Link to={"/reviews"}>The LessWrong 2018 Review</Link>
            {(currentUser?.karma || 0) >= 1000 ? (
              <div className={classes.timeRemaining}>
                <em>
                  You have until Dec 1st to nominate posts. (Posts need 2+ nominations,{" "}
...
const FrontpageNominationPhaseComponent = registerComponent("FrontpageNominationPhase", FrontpageNominationPhase, {
  styles,
});
