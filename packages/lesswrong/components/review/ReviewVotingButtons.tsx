...
const styles = (theme: ThemeType) => {
  ...
  return {
    ...
    button: {
      ...
      "&:hover": {
        backgroundColor: theme.palette.greyAlpha(0.075),
      },
    },
    selectionHighlight: {
...
const ReviewVotingButtons = ({
  ...
  dispatch: any;
  currentUserVote: SyntheticQualitativeVote | null;
  costTotal?: number;
}) => {
  ...
  return (
    <AnalyticsContext pageElementContext="reviewVotingButtons">
      <div className={classes.root}>
        {([1, 2, 3, 4, 5, 6, 7] as const).map((i) => {
          return (
            <LWTooltip title={getCostData({ costTotal })[i].tooltip} key={`${getCostData({ costTotal })[i]}-${i}`}>
              <span
