...
const BookmarksList = ({
  ...
  const sortedBookmarkedPosts = sortBy(
    bookmarkedPosts,
    (post) => -findIndex(currentUser?.bookmarkedPostsMetadata || [], (bookmark) => bookmark.postId === post._id),
  );

  return (
    <AnalyticsContext pageSubSectionContext="bookmarksList">
      <div>
        {showMessageIfEmpty && !loading && sortedBookmarkedPosts && !sortedBookmarkedPosts.length && (
          <div className={classes.empty}>
            {isEAForum ? "You haven't saved any posts yet." : "You haven't bookmarked any posts yet."}
          </div>
        )}
        {sortedBookmarkedPosts &&
          sortedBookmarkedPosts.map((post: PostsListWithVotes, i: number) => (
            <PostsItem key={post._id} post={post} bookmark showBottomBorder={i < sortedBookmarkedPosts.length - 1} />
          ))}
