...
import { gql, NetworkStatus } from "@apollo/client"; import { useQuery } from "@apollo/client";
...
const ReadHistoryTab = ({ classes }: { classes: ClassesType }) => {
  ...
  const yesterdaysPosts = readHistory.filter((post) =>
    moment(post.lastVisitedAt).isSame(moment().subtract(1, "day"), "day"),
  );
  const olderPosts = readHistory.filter((post) =>
    moment(post.lastVisitedAt).isBefore(moment().subtract(1, "day"), "day"),
  );

  return (
    <AnalyticsContext pageSectionContext="bookmarksTab">
