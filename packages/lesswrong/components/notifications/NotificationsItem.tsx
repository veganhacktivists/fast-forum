...
const NotificationsItem = ({
  ...
  const PreviewTooltip: FC<{ children: ReactNode }> = useCallback(
    ({ children }) => {
      const { PostsTooltip, ConversationPreview, PostNominatedNotification } = Components;

      if (notificationType.onsiteHoverView) {
        return (
          <TooltipWrapper title={notificationType.onsiteHoverView({ notification })} classes={classes}>
            {children}
          </TooltipWrapper>
        );
      }
      ...
      if (notification.type === "postNominated") {
        return (
          <TooltipWrapper title={<PostNominatedNotification postId={notification.documentId} />} classes={classes}>
            {children}
          </TooltipWrapper>
        );
      }
      ...
      switch (notification.documentType) {
        ...
        case "comment":
          ...
          return postId ? (
            <PostsTooltip postId={postId} commentId={notification.documentId} {...tooltipProps}>
              {children}
            </PostsTooltip>
          ) : null;
        case "message":
          return (
            <TooltipWrapper
              title={<ConversationPreview conversationId={parsedPath?.params?._id} currentUser={currentUser} />}
              classes={classes}
            >
              {children}
            </TooltipWrapper>
  ...
  return (
    <PreviewTooltip>
      ...
      >
        ...
        <div className={classes.notificationLabel}>{renderMessage()}</div>
      </a>
    </PreviewTooltip>
  );
};
