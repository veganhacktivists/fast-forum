...
const NotifyMeButton = ({
  ...
  const maybeMenuItemButton = asMenuItem ? (
    <MenuItem onClick={onSubscribe}>
      <a className={classNames(classes.root, className)}>{button}</a>
    </MenuItem>
  ) : asButton ? (
  ...
  const maybeToolipButton = tooltip ? (
    <Components.LWTooltip title={tooltip}>{maybeMenuItemButton}</Components.LWTooltip>
  ) : (
    maybeMenuItemButton
  );
