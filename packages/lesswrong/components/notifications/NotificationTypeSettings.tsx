  ...
  return (
    <div className={classes.root}>
      ...
      <Typography variant="body2" component="div" className={classes.settings}>
        <Select value={currentValue.channel} onChange={(event) => modifyValue({ channel: event.target.value })}>
        ...
        {currentValue.channel !== "none" && (
          <React.Fragment>
            {" "}
            <Select
              value={currentValue.batchingFrequency}
              onChange={(event) => modifyValue({ batchingFrequency: event.target.value })}
            >
        ...
        {currentValue.channel !== "none" &&
          (currentValue.batchingFrequency === "daily" || currentValue.batchingFrequency === "weekly") && (
            <React.Fragment>
              {" at "}
              <BatchTimePicker
