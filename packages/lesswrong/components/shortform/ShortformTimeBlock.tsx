...
const ShortformItem: FC<{ comment: ShortformComments }> = ({ comment }) => {
  ...
  if (isFriendlyUI) {
    return <Components.QuickTakesListItem quickTake={comment} />;
  }
  return (
    ...
    <div>
      <div className={classes.shortformGroup}>
        <div className={classes.subtitle}>
          <ContentType type="shortform" label={isFriendlyUI ? "Quick takes" : "Shortform"} />
        </div>
        ...
        {comments.length < totalCount! && (
          <div className={classes.loadMore}>
            <LoadMore loadMore={loadMore} count={comments.length} totalCount={totalCount} />
          </div>
        )}
      </div>
    </div>
