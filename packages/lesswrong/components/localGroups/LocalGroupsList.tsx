...
const LocalGroupsList = ({
  ...
  const { LocalGroupsItem, Loading, PostsNoResults, SectionFooter, LoadMore, SingleColumnSection, SectionTitle } =
    Components;

  const MaybeTitleWrapper = ({ children }: { children: ReactNode }) =>
    heading ? (
      <SingleColumnSection>
    ...
    <MaybeTitleWrapper>
      <div>
        <div className={classes.localGroups}>
          {results.map((group) => (
            <LocalGroupsItem key={group._id} group={group} />
          ))}
        </div>
        <SectionFooter>
