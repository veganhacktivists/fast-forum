  ...
  return (
    <div>
      {results.map((event) => (
        ...
        >
          <EventComponent event={event} onClick={onClick} classes={classes} />
        </LWTooltip>
      ))}
    </div>
  );
...
const TabNavigationEventSingleLine = ({
  ...
  return (
    <MenuItemLink onClick={onClick} to={postGetPageUrl(event)} rootClass={classes.eventWrapper}>
...
const TabNavigationEventTwoLines = ({
  event,
  onClick,
  classes,
}: {
  ...
  const shortDate = event.startTime && moment(event.startTime).tz(timezone).format("ddd MMM D");
  ...
  if (event.googleLocation) {
    ...
    for (let locationType of locationTypePreferenceOrdering) {
      for (let addressComponent of event.googleLocation.address_components) {
        if (addressComponent.types.indexOf(locationType) >= 0) return addressComponent.long_name;
      }
    }
    return null;
  } else {
...
const EventSidebarTooltip = ({ event, classes }: { event: PostsList; classes: ClassesType }) => {
  ...
  return (
    <div>
      ...
      <div className={classes.tooltipLogisticsTitle}>{event.onlineEvent ? "Online Event" : "Location"}</div>
      {!event.onlineEvent && <div>{event.location}</div>}
      <div className={classes.tooltipLogisticsTitle}>Time</div>
      <div>{event.startTime ? <EventTime post={event} /> : <span>Start time TBD</span>}</div>
      {highlight && (
        <React.Fragment>
