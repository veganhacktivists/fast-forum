...
const CommunityHome = ({ classes }: { classes: ClassesType }) => {
  ...
  const render = () => {
    ...
    const { SingleColumnSection, SectionTitle, PostsList2, GroupFormLink, SectionFooter, Typography, SectionButton } =
      Components;
    ...
    const WelcomeText = () =>
      isEAForum ? (
        <Typography variant="body2" className={classes.welcomeText}>
          <p>
            On the map above you can find upcoming events (blue pin icons) and local groups (green star icons), and
            other users who have added themselves to the map (purple person icons).
          </p>
          <p>
            Not all groups have been added to this page yet. For more, visit the{" "}
            <a
              className={classes.link}
              href="https://eahub.org/groups?utm_source=forum.effectivealtruism.org&utm_medium=Organic&utm_campaign=Forum_Homepage"
            >
              EA Hub Groups Directory
            </a>
            .
          </p>
        </Typography>
        ...
        <Typography variant="body2" className={classes.welcomeText}>
          On the map above you can find nearby events (blue arrows), local groups (green house icons), and other users
          who have added themselves to the map (purple person icons)
        </Typography>
      );
    ...
    return (
      <React.Fragment>
        <AnalyticsContext pageContext="communityHome">
            ...
            <SectionFooter>
              ...
              <a onClick={openEventNotificationsForm}>
                {currentUser?.nearbyEventsNotifications
                  ? `Edit my event/groups notification settings`
                  : `Sign up for event/group notifications`}
              </a>
            </SectionFooter>
          </SingleColumnSection>
          <SingleColumnSection>
          ...
          <SingleColumnSection>
            <SectionTitle title="Online Groups">{canCreateGroups && <GroupFormLink isOnline={true} />}</SectionTitle>
            <AnalyticsContext listContext={"communityGroups"}>
              <Components.LocalGroupsList terms={onlineGroupsListTerms} />
            </AnalyticsContext>
          </SingleColumnSection>
          <SingleColumnSection>
            <SectionTitle title="Local Groups">{canCreateGroups && <GroupFormLink />}</SectionTitle>
            {currentUserLocation.loading ? (
              <Components.Loading />
            ) : (
