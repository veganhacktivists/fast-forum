...
const LocalGroupPage = ({
  ...
  let topSection =
    group.googleLocation && !group.isOnline ? (
      <CommunityMapWrapper
  ...
  if (group.bannerImageId) {
    topSection = (
      <div className={classes.imageContainer}>
        <CloudinaryImage2
          imgProps={{ ar: "191:100", w: "765" }}
          publicId={group.bannerImageId}
          className={classes.bannerImg}
        />
      </div>
    );
    smallMap = group.googleLocation && !group.isOnline && (
      <CommunityMapWrapper
  ...
  const groupHasContactInfo =
    group.facebookLink ||
    group.facebookPageLink ||
    group.meetupLink ||
    group.slackLink ||
    group.website ||
    group.contactInfo;
  ...
  if (isEAForum) {
    upcomingEventsList = !!upcomingEvents?.length ? (
      ...
      <Typography variant="body2" className={classes.noUpcomingEvents}>
        No upcoming events.{" "}
        <NotifyMeButton
          showIcon={false}
          document={group}
  ...
  let tbdEventsList: JSX.Element | null = (
  ...
  if (isEAForum) {
    tbdEventsList = tbdEvents?.length ? (
      <>
        ...
        <div className={classes.eventCards}>
          <EventCards events={tbdEvents} loading={tbdEventsLoading} hideSpecialCards hideGroupNames />
  ...
  const canEditGroup =
    currentUser && group && group.organizerIds.includes(currentUser._id)
  ...
  return (
    <div className={classes.root}>
      <HeadTags
        ...
        image={
          group.bannerImageId && `https://res.cloudinary.com/cea/image/upload/q_auto,f_auto/${group.bannerImageId}.jpg`
        }
      />
      {topSection}
      <SingleColumnSection>
        <div className={classes.titleRow}>
          <div>
            {isEAForum ? (
              <Typography variant="display1" className={classes.groupName}>
                {groupNameHeading}
              </Typography>
            ) : (
            ...
            {group.categories?.length > 0 && (
              <div className={classes.groupCategories}>
                {group.categories.map((category) => {
                  return (
                    <div key={category} className={classes.groupCategory}>
                      {GROUP_CATEGORIES.find((option) => option.value === category)?.label}
                    </div>
                  );
                })}
              </div>
          ...
          <div>
            {currentUser && (
              <SectionButton className={classes.notifyMe}>
                <NotifyMeButton
                  showIcon
                  document={group}
            ...
            <SectionFooter className={classes.organizerActions}>
              {canCreateEvent && (!isEAForum || isAdmin || isGroupAdmin) && (
                <SectionButton>
              ...
              {canEditGroup && (!isEAForum || isAdmin || isGroupAdmin) && <GroupFormLink documentId={groupId} />}
            </SectionFooter>
          </div>
        </div>
