...
export const SpotlightStartOrContinueReading = ({
  classes,
  spotlight,
  className,
}: {
  ...
  if (firstPost) {
    return (
      <div className={classNames(classes.firstPost, className)}>
        {prefix}
        <PostsTooltip post={firstPost}>
    ...
    return (
      <div className={classNames(classes.boxesRoot, className)}>
        {posts.map((post) => (
          <PostsTooltip key={`${spotlight._id}-${post._id}`} post={post} flip={false} inlineBlock>
