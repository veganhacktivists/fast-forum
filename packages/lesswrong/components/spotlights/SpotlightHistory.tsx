...
export const SpotlightHistory = ({ classes }: { classes: ClassesType }) => {
  ...
  const title = userCanDo(currentUser, "spotlights.edit.all") ? (
    <Link to={"/spotlights"}>Spotlight Items</Link>
  ) : (
  ...
  return (
    <SingleColumnSection>
      ...
      {spotlights.map((spotlight) => (
        <SpotlightItem key={spotlight._id} spotlight={spotlight} />
      ))}
