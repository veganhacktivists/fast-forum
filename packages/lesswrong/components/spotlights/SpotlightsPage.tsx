...
export const SpotlightsPage = ({ classes }: { classes: ClassesType }) => {
  const {
    Loading,
    SectionTitle,
    SingleColumnSection,
  ...
  return (
    <ToCColumn tableOfContents={<TableOfContents sectionData={sectionData} title={"Spotlights"} />}>
      <SingleColumnSection>
        ...
        {upcomingSpotlights.map((spotlight) => (
          <SpotlightItem
            key={`spotlightpage${spotlight._id}`}
            spotlight={spotlight}
            refetchAllSpotlights={refetch}
            showAdminInfo
          />
        ))}
