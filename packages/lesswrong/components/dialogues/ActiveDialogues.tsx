...
export const ActiveDialogues = ({ classes }: { classes: ClassesType<typeof styles> }) => {
  ...
  useOnNotificationsChanged(currentUser, (message) => {
    if (message.eventType === "activeDialoguePartners" && currentUser?._id && message.data) {
      const otherActiveDialogues = message.data.filter(
        (dialogue) => !location.pathname.includes(dialogue.postId) && !(location.query.postId === dialogue.postId),
      ); // don't show a dialogue as active on its own page
    ...
    <div className={classes.root}>
      ...
      {activeDialoguesWithoutSelf.map((dialogue) => (
        <div className={classes.activeDialogueContainer} key={dialogue.postId}>
          <div
            className={classNames(classes.activeDot, {
