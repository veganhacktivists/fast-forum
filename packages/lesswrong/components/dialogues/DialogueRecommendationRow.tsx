...
const ExpandCollapseText = ({ classes, isExpanded, numHidden, toggleExpansion }: ExpandCollapseTextProps) => {
  ...
  if (isExpanded) {
    text = (
      <>
        <span className={classes.caret}>▲</span> hide
      </>
    );
  } else {
    text = (
...
export const CommentView: React.FC<CommentViewProps> = ({ comment, classes }) => {
  ...
  return (
    <PostsTooltip postId={comment.postId} commentId={comment._id}>
      ...
      <Link className={classes.commentSourcePost} to={postGetPageUrl(post)}>
        {" "}
        on "{post.title}"{" "}
      </Link>
    </PostsTooltip>
...
const DialogueRecommendationRow = ({
  targetUser,
  checkId,
  userIsChecked,
  userIsMatched,
  ...
  showSuggestedTopics,
  onHide,
}: DialogueRecommendationRowProps) => {
  ...
  const preTopicRecommendations: TopicRecommendationWithContents[] | undefined =
    topicData?.GetTwoUserTopicRecommendations;
  const topicRecommendations = preTopicRecommendations?.filter((topic) =>
    ["agree", "disagree"].includes(topic.theirVote),
  ); // todo: might want better type checking here in future for values of theirVote
  ...
  const allRecommendations: { reactIconName: string; prefix: string; Content: JSX.Element }[] = [
    ...topicRecommendations.map((topic) => ({
      reactIconName: topic.theirVote,
      prefix: topic.theirVote + ": ",
      Content: <>{topic.comment.contents.plaintextMainText}</>,
    })),
  ...
  return (
    <div>
      <div
        key={targetUser._id}
        className={classNames(classes.dialogueUserRow, {
          [classes.dialogueUserRowExpandedMobile]: isExpanded,
        })}
      >
        <div
          className={classNames(classes.dialogueLeftContainer, {
        ...
        >
          ...
          <PostsItem2MetaInfo
            className={classNames(classes.dialogueMatchUsername, {
...
const DialogueRecommendationRowComponent = registerComponent("DialogueRecommendationRow", DialogueRecommendationRow, {
  styles,
});
