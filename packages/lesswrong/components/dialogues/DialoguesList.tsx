...
const DialogueMatchRow = ({ currentUser, rowProps, classes, onHide }: DialogueMatchRowProps) => {
  const { DialogueCheckBox, UsersName, MessageButton, DialogueNextStepsButton, PostsItem2MetaInfo, ReactionIcon } =
    Components;
    ...
    <div key={targetUser._id} className={classes.dialogueUserRow}>
      <div className={classes.dialogueLeftContainer}>
        ...
        <PostsItem2MetaInfo className={classes.dialogueMatchUsername}>
          <UsersName documentId={targetUser._id} simple={false} />
        </PostsItem2MetaInfo>
      </div>
      {!matchPreference && (
      ...
      <IconButton
        ...
        onClick={() => onHide({ dialogueCheckId: checkId, targetUserId: targetUser._id })}
      >
        <CloseIcon />
      </IconButton>
    </div>
...
const DialoguesList = ({
  ...
  const hideRecommendation = ({
    ...
    dialogueCheckId: string | undefined;
    targetUserId: string;
  }) => {
  ...
  const neededChecksForUserIds = [
    ...matchedUsers.map((user) => user._id),
    ...manyRecommendedUsers.map((user) => user._id),
  ];
  ...
  const recommendedDialoguePartnersRowPropsList = manyRecommendedUsers.map((targetUser) => ({
    targetUser,
    ...getUserCheckInfo(targetUser, userDialogueChecks),
  }));

  const dialoguesTooltip = (
    <div>
      <p>Dialogues between a small group of users. Click to see more.</p>
    </div>
  );

  const myDialoguesTooltip = (
    <div>
      <div>These are the dialogues you are involved in (both drafts and published)</div>
    </div>
  );

  const matchmakingTooltip = (
    <div>
      <p> Click here to go to the dialogue matchmaking page.</p>
    </div>
  );

  const dialogueSettingsTooltip = (
    <div>
  ...
  const explanatoryNoteBox = currentUser.showRecommendedPartners &&
    showReciprocityRecommendations &&
    recommendedDialoguePartnersRowPropsList.length > 0 && (
      <div className={classes.explanatoryNoteBox}>
        <Typography component="span" className={classes.explanatoryNote} variant="body2">
          {
            <div>
              Check a user you'd maybe dialogue with. They can't see your checks unless you match. If you match, you
              both get to enter topics and then choose whether to dialogue. (
              <a
                className={classes.link}
                href="https://www.lesswrong.com/posts/d65Ax6vbNgztBE8cy/new-lesswrong-feature-dialogue-matching"
              >
                Learn more
              </a>
              )
  ...
  const userMatches =
    currentUser.showMatches &&
    matchRowPropsList
      .filter((rowProps) => !rowProps.hideInRecommendations)
      .map((rowProps, index) => (
        <DialogueMatchRow
          key={index}
          currentUser={currentUser}
  ...
  const reciprocityRecommendations =
    showReciprocityRecommendations &&
    currentUser.showRecommendedPartners &&
    recommendedDialoguePartnersRowPropsList
      .filter((rowProps) => !rowProps.hideInRecommendations)
      .map((rowProps, index) => (
        <DialogueRecommendationRow
          key={index}
  ...
  const dialoguesList =
    currentUser.showDialoguesList &&
    dialoguePosts?.map((post, i: number) => (
      <PostsItem key={post._id} post={post} showBottomBorder={i < dialoguePosts.length - 1} />
    ));
    ...
    <div className={classes.subsection}>
      <AnalyticsContext pageSubSectionContext="myDialogues">
        <LWTooltip placement="top-start" title={myDialoguesTooltip}>
          <Link to={"/dialogues"}>
            <SectionSubtitle className={classes.subheading}>My Dialogues (only visible to you)</SectionSubtitle>
          </Link>
        </LWTooltip>
        {myDialogues?.map((post, i: number) => (
          <PostsItem key={post._id} post={post} showBottomBorder={i < myDialogues.length - 1} />
        ))}
      </AnalyticsContext>
    </div>
  );
  ...
  return (
    <AnalyticsContext pageSubSectionContext="dialoguesList">
      <SingleColumnSection>
        <SectionTitle
          href="/dialogues"
          title={
        ...
        {dialogueSectionSettings}
        {dialogueMatchmakingEnabled.get() && (
