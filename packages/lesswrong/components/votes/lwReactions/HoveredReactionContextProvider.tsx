...
export const HoveredReactionContextProvider = ({ children }: { children: React.ReactNode }) => {
  const [hoveredReactions, dispatch] = useReducer(hoveredReactionsReducer, []);

  return (
    <HoveredReactionListContext.Provider value={hoveredReactions}>
      <SetHoveredReactionContext.Provider value={dispatch}>{children}</SetHoveredReactionContext.Provider>
    </HoveredReactionListContext.Provider>
  );
};

const HoveredReactionContextProviderComponent = registerComponent(
