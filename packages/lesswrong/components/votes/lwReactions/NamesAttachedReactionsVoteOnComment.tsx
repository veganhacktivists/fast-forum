...
import {
  ...
  VoteOnReactionType,
  reactionsListToDisplayedNumbers,
  getNormalizedReactionsListFromVoteProps,
  getNormalizedUserVoteFromVoteProps,
  QuoteLocator,
...
export const useNamesAttachedReactionsVoting = (
  ...
  async function addCurrentUserReaction(
  ...
  async function setCurrentUserReaction(
    reactionName: string,
    reaction: VoteOnReactionType | null,
    quote: QuoteLocator | null,
  ) {
  ...
  function getAlreadyUsedReactTypesByKarma(): string[] {
    ...
    const alreadyUsedReactionTypesByKarma = orderBy(alreadyUsedReactionTypes, (r) =>
      alreadyUsedReactions[r]!.length > 0 ? alreadyUsedReactions[r]![0].karma : 0,
    );
  ...
  return {
    ...
    toggleReaction,
    setCurrentUserReaction,
    getAlreadyUsedReactTypesByKarma,
    getAlreadyUsedReacts,
  };
};

export function reactionVoteIsMatch(
  react: UserVoteOnSingleReaction,
  name: EmojiReactName,
  quote: QuoteLocator | null,
): boolean {
  if (react.react !== name) return false;
  if (quote) {
...
const NamesAttachedReactionsVoteOnComment = ({
  document,
  hideKarma = false,
  collection,
  votingSystem,
  ...
  return (
    <span className={classes.root}>
      <OverallVoteAxis document={document} hideKarma={hideKarma} voteProps={voteProps} showBox={true} />
      <AgreementVoteAxis document={document} hideKarma={hideKarma} voteProps={voteProps} />
    </span>
  );
};
...
const NamesAttachedReactionsCommentBottom = ({
  ...
  return (
    <span className={classes.footerReactionsRow} ref={anchorEl}>
      ...
      {(hiddenReacts.length > 0 || alwaysShowReactionOverviewButton) && (
        <ReactionOverviewButton voteProps={voteProps} classes={classes} />
      )}
...
const HoverableReactionIcon = ({
  ...
  const {
    hover,
    eventHandlers: { onMouseOver, onMouseLeave },
  } = useHover();
  const { ReactionIcon, LWPopper } = Components;
  const { getCurrentUserReaction, getCurrentUserReactionVote, toggleReaction } =
    useNamesAttachedReactionsVoting(voteProps);
  ...
  const alreadyUsedReactions: NamesAttachedReactionsList | undefined =
    getNormalizedReactionsListFromVoteProps(voteProps)?.reacts;
  ...
  return (
    <span onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <span
        className={classNames(classes.footerReaction, {
      ...
      >
        ...
        {hover && reactionRowRef?.current && (
          <LWPopper open={!!hover} anchorEl={reactionRowRef.current} placement="bottom-end" allowOverflow={true}>
            {/*
...
const ReactionOverview = ({
  ...
  const { getCurrentUserReactionVote, setCurrentUserReaction, getAlreadyUsedReactTypesByKarma, getAlreadyUsedReacts } =
    useNamesAttachedReactionsVoting(voteProps);
      ...
      <div className={classes.reactOverview}>
        ...
        <div className={classes.alreadyUsedReactions}>
          {alreadyUsedReactionTypesByKarma.map((r) => {
            ...
            return (
              <div key={`${r}`} className={classes.overviewSummaryRow}>
...
const NamesAttachedReactionsHoverSingleReaction = ({
  react,
  voteProps,
  classes,
  commentBodyRef,
  ...
  return (
    <div className={classes.footerReactionHover}>
      <ReactionHoverTopRow
        reactionName={react}
...
export const AddReactionButton = ({
  ...
  return (
    <LWTooltip disabled={open} inlineBlock={false} title={<>Click to react to this comment</>}>
      <span
        ref={buttonRef}
        onClick={(ev) => {
          setOpen(true);
      ...
      >
        ...
        {open && (
          ...
          >
            <PopperCard open={open} anchorEl={buttonRef.current} placement="bottom-end" allowOverflow={true}>
...
const NamesAttachedReactionsVoteOnCommentComponent = registerComponent(
  "NamesAttachedReactionsVoteOnComment",
  NamesAttachedReactionsVoteOnComment,
  {
    styles,
...
const NamesAttachedReactionsCommentBottomComponent = registerComponent(
  "NamesAttachedReactionsCommentBottom",
  NamesAttachedReactionsCommentBottom,
  {
    styles,
    hocs: [withErrorBoundary],
