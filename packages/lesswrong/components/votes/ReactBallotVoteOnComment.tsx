...
import {
  ...
  reactBallotAxes,
  ReactBallotAxis,
  ReactBallotStandaloneReaction,
  reactBallotStandaloneReactions,
      ...
      VoteIconComponent={({ eventHandlers, voted, ...rest }) => {
        return (
          ...
          >
            ...
            <span className={classes.buttonLabel}>{direction === "up" ? axis.goodLabel : axis.badLabel}</span>
          </div>
        );
      }}
      axis={axis.name}
...
const AxisScoreDisplay = ({
  ...
  return (
    <div className={classes.axisScore}>
...
const BallotStandaloneReaction = ({
  reaction,
  voteProps,
  classes,
}: {
  ...
  return (
    <div
      className={classNames(classes.voteButton, classes.standaloneReaction, {
        [classes.voteButtonSelected]: isSelected,
      })}
      onClick={async (ev) => {
        if (!currentUser) {
          openDialog({
...
const ReactBallotVoteOnComment = ({
  document,
  hideKarma = false,
  collection,
  votingSystem,
  classes,
}: ReactBallotVoteOnCommentProps) => {
  ...
  return (
    <span className={classes.root} {...eventHandlers}>
      <OverallVoteAxis document={document} hideKarma={hideKarma} voteProps={voteProps} showBox={false} />
      ...
      {hover && (
        <PopperCard open={!!hover} anchorEl={anchorEl} placement="bottom-start">
          <div className={classes.hoverBallot}>
            ...
            {chunk(reactBallotStandaloneReactions, 2).map((row, i) => (
              <div key={i} className={classes.hoverBallotRow}>
                {row.map((reaction) => (
                  <BallotStandaloneReaction
                    key={reaction.name}
                    reaction={reaction}
                    voteProps={voteProps}
                    classes={classes}
...
const ReactBallotVoteOnCommentComponent = registerComponent("ReactBallotVoteOnComment", ReactBallotVoteOnComment, {
  styles,
});
