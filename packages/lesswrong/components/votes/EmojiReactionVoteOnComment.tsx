...
const BallotEmojiReaction = ({
  ...
  return (
    <div
      className={classNames(classes.voteButton, { [classes.voteButtonSelected]: isSelected })}
      onClick={async () => {
        if (!currentUser) {
          openDialog({
...
const EmojiReactionsAxis = ({
  ...
  return (
    <span className={classes.emojiReactionsAxisRoot} {...eventHandlers}>
      <span className={classes.scores}>
        ...
        {!hasReactions && (
          <svg
            className={classNames("MuiSvgIcon-root", classes.addReactionIcon)}
            fill="none"
            height="16"
            viewBox="0 0 16 16"
            width="16"
      ...
      {hover && (
        <PopperCard open={!!hover} anchorEl={anchorEl} placement="bottom-start">
          <div className={classes.hoverBallot}>
            {emojiReactions.map((react) => (
              <BallotEmojiReaction key={react.name} reaction={react} voteProps={voteProps} classes={classes} />
            ))}
          </div>
        </PopperCard>
      )}
...
const EmojiReactionVoteOnComment = ({
  document,
  hideKarma = false,
  collection,
  votingSystem,
  classes,
}: EmojiReactionVoteOnCommentProps) => {
  ...
  return (
    <span className={classes.root}>
      <OverallVoteAxis document={document} hideKarma={hideKarma} voteProps={voteProps} />
      <EmojiReactionsAxis voteProps={voteProps} classes={classes} />
    </span>
  );
};

const EmojiReactionVoteOnCommentComponent = registerComponent(
  "EmojiReactionVoteOnComment",
  EmojiReactionVoteOnComment,
  { styles },
);
