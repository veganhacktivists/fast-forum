...
const CollectionsPage = ({ documentId, classes }: { documentId: string; classes: ClassesType }) => {
  ...
  if (loading || !document) {
    ...
    return (
      <Components.CollectionsEditForm
    ...
    const canEdit =
      userCanDo(currentUser, "collections.edit.all") ||
      (userCanDo(currentUser, "collections.edit.own") && userOwns(currentUser, collection));
    ...
    const posts = collection.books.flatMap((book) =>
      book.sequences.flatMap((sequence) => sequence.chapters.flatMap((chapter) => chapter.posts)),
    );
    const wordCount = posts.reduce((i, post) => i + (post?.contents?.wordCount || 0), 0);
    // eslint-disable-next-line no-console
    console.log(`${wordCount.toLocaleString()} words`);

    const socialImageUrl = collection.gridImageId
      ? makeCloudinaryImageUrl(collection.gridImageId, {
          c: "fill",
          dpr: "auto",
          q: "auto",
          f: "auto",
    ...
    return (
      <ErrorBoundary>
        ...
        <div className={classes.root}>
          <ToCColumn tableOfContents={<CollectionTableOfContents collection={document} />}>
              ...
              <ContentStyles contentType="post" className={classes.description}>
                {html && (
                  <ContentItemBody
                    ...
                    description={`collection ${document._id}`}
                  />
                )}
              </ContentStyles>

              {!collection.hideStartReadingButton && (
                <ButtonUntyped className={classes.startReadingButton} component={Link} to={document.firstPageLink}>
            ...
            <div>
              {collection.books.map((book) => (
                <div className={classes.section} key={`collectionsPage${book._id}`}>
