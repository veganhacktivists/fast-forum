...
const SequencesSmallPostLink = ({
  ...
  return (
    <div className={classNames(classes.title, { [classes.large]: large })}>
      <span className={classes.checkbox}>
        <PostReadCheckbox post={post} />
      </span>
      <PostsTooltip post={post} postsList={isLWorAF} placement={placement} inlineBlock={false} clickable>
        <Link to={postGetPageUrl(post, false, sequenceId)}>{post.title}</Link>
      </PostsTooltip>
    </div>
  );
};
