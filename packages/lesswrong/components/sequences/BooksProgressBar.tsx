...
const BooksProgressBar = ({ book, classes }: { book: BookPageFragment; classes: ClassesType }) => {
  ...
  const readTime =
    totalWordCount > WORDS_PER_HOUR
      ? `${(totalWordCount / WORDS_PER_HOUR).toFixed(1)} hour`
      : `${Math.round(totalWordCount / WORDS_PER_MINUTE)} min`;
  const postsReadTooltip = (
    <div>
      <div>
        {totalWordCount.toLocaleString()} words, {Math.round(totalWordCount / WORDS_PER_PAGE)} pages
      </div>
  ...
  return (
    <div key={book._id} className={classes.root}>
        ...
        {bookPosts.map((post) => (
          <PostsTooltip post={post} key={post._id}>
            <Link to={postGetPageUrl(post)}>
              <div
                className={classNames(classes.postProgressBox, {
                  [classes.read]: post.isRead || clientPostsRead[post._id],
                })}
              />
            </Link>
          </PostsTooltip>
      ...
      <div className={classNames(classes.sequence, classes.progressText)}>
        ...
        <LoginToTrack className={classes.loginText}>login to track progress</LoginToTrack>
      </div>
    </div>
  );
};
