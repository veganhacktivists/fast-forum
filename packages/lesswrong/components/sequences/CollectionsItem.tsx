...
export const CollectionsItem = ({
  ...
  const { Typography, LinkCard, ContentStyles, ContentItemBody, PostsTooltip } = Components;
  ...
  return (
    <div className={classNames(classes.root, { [classes.small]: collection.small })}>
      <LinkCard to={collection.url} className={classes.linkCard}>
        <div className={classes.content}>
          <Typography variant="title" className={classes.title}>
            <Link to={collection.url}>{collection.title}</Link>
          </Typography>
          {collection.subtitle && <div className={classes.subtitle}>{collection.subtitle}</div>}
          <ContentStyles contentType="postHighlight" className={classes.description}>
            {collection.hideSummaryOnMobile ? <div className={classes.hideOnMobile}>{description}</div> : description}
          </ContentStyles>
          {firstPost && (
            <div className={classes.firstPost}>
              First Post:{" "}
              <PostsTooltip inlineBlock postId={firstPost.postId}>
        ...
        {collection.imageUrl && (
          <img src={collection.imageUrl} className={classes.image} style={{ width: collection.imageWidth || 130 }} />
        )}
