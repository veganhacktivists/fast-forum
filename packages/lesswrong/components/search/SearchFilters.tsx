...
// filters by tags
const TagsRefinementList = ({ tagsFilter, setTagsFilter }: RefinementListProvided & TagsRefinementProps) => {
  return (
...
const CustomTagsRefinementList = connectRefinementList(TagsRefinementList) as React.ComponentClass<
  RefinementListExposed & TagsRefinementProps
>;
  ...
  return (
    <div className={classes.filtersColumn}>
      ...
      {["Posts", "Comments", "Sequences", "Users"].includes(tab) && (
        <>
          <div className={classes.filterLabel}>Filter by {tab === "Users" ? "joined" : "posted"} date</div>
          <NumericMenu
      ...
      {["Posts", "Comments", "Users"].includes(tab) && (
        <CustomTagsRefinementList
          attribute="tags"
      ...
      {tab === "Posts" && <ToggleRefinement attribute="curated" label="Curated" value={true} />}
      {tab === "Posts" && (
        <ToggleRefinement attribute="isEvent" label="Exclude events" value={false} defaultRefinement={true} />
      )}
      {tab === "Tags" && <ToggleRefinement attribute="core" label="Core topic" value={true} />}
      <ClearRefinements />
      ...
      {elasticCollectionIsCustomSortable(tab) && (
        <>
          ...
          <Select value={sorting} onChange={(e) => onSortingChange(e.target.value)} className={classes.sort}>
