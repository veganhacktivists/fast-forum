...
export const formatFacetFilters = (facetFilters?: Record<string, boolean | string>): string[][] | undefined =>
  facetFilters ? [Object.keys(facetFilters).map((key) => `${key}:${facetFilters[key]}`)] : undefined;
...
const SearchAutoComplete = ({
  ...
  if (!isSearchEnabled()) {
    ...
    return (
      <input
        type="text"
        placeholder={noSearchPlaceholder}
        onKeyPress={(ev) => {
          if (ev.charCode === 13) {
  ...
  return (
    <InstantSearch indexName={indexName} searchClient={getSearchClient()}>
      <div className={classes.autoComplete}>
        ...
        />
        <Configure hitsPerPage={hitsPerPage} facetFilters={formatFacetFilters(facetFilters)} />
      </div>
    </InstantSearch>
  );
};
