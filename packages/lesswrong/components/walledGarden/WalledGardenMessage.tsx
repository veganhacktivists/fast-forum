...
const WalledGardenMessage = ({ children, classes }: { children: React.ReactNode; classes: ClassesType }) => {
  const { SingleColumnSection, ContentStyles } = Components;
  return (
    <SingleColumnSection className={classes.messageStyling}>
      <ContentStyles contentType="post">{children}</ContentStyles>
    </SingleColumnSection>
  );
};
