...
export const getAddToCalendarLink = (gcalEvent: AnyBecauseTodo) => {
  ...
  if (gcalEvent.description) {
    return <LWTooltip title={<div style={{ whiteSpace: "pre-wrap" }}>{noHtmlDescription}</div>}>{link}</LWTooltip>;
  } else {
    return link;
  }
};

const PortalBarGcalEventItem = ({ classes, gcalEvent }: { classes: ClassesType; gcalEvent: any }) => {
  return (
    <div className={classes.root}>
      <span className={classes.eventName}>{getAddToCalendarLink(gcalEvent)}</span>
      <span className={classes.eventTime}>{eventFormat(gcalEvent.start.dateTime)}</span>
    </div>
  );
};
