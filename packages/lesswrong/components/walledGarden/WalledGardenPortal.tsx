...
const styles = (theme: ThemeType): JssStyles => ({
  ...
  toggleEvents: {
    ...
    alignItems: "center",
    cursor: "pointer",
    textShadow: `0 0 10px ${theme.palette.greyAlpha(0.8)}`,
  },
  closeIcon: {
    height: 48,
    width: 48,
...
const WalledGardenPortal = ({ classes }: { classes: ClassesType }) => {
  ...
  const gardenCode =
    results && results.length > 0 && (results[0] as HasIdType)._id
      ? (results[0] as FragmentTypes["GardenCodeFragment"] | null)
      : null;
  ...
  const moreThanFourHoursAfterCodeExpiry = useCallback(
    (gardenCode) => moment(gardenCode?.endTime).add(4, "hours").isBefore(new Date()),
    [],
  );
  ...
  if (isMobile())
    return (
      <WalledGardenMessage>
        <p>
          You have reached the Walled Garden Portal. Unfortunately, this page does not work on mobile. Please re-visit
          from a desktop broswer.
        </p>
      </WalledGardenMessage>
    );
  ...
  if (!userIsAllowed) {
    ...
    if (codeExpiredBeforeSession)
      return (
        <WalledGardenMessage>
          <p>
            Unfortunately, your invite code is for an event that has already ended. Please request another link from
            your host or return when the Garden is open to the public on Sunday between 12pm and 4pm PT.
          </p>
        </WalledGardenMessage>
      );

    if (deletedOrMalformedCode)
      return (
        <WalledGardenMessage>
          <p>
            Unfortunately, your invite link to the Garden is not valid. Please request another link from your host or
            return when the Garden is open to the public on Sundays between 12pm and 4pm PT.
          </p>
        </WalledGardenMessage>
      );
    ...
    return (
      <SingleColumnSection className={classes.root}>
        <ContentStyles contentType="post">
          ...
          <p>
            If you have an event invite link, please use that to enter. If you have been granted full-access, to{" "}
            {currentUser ? (
              "log in"
            ) : (
              <LoginPopupButton>
  ...
  const enterGardenButton = (
    <AnalyticsTracker
      eventType="walledGardenEnter"
      captureOnMount
      eventProps={{ isOpenToPublic, inviteCodeQuery, isMember: currentUser?.walledGardenInvite }}
    >
  ...
  if ((!!gardenCode && !enteredQuery) || (!!currentUser && !onboarded)) {
    return (
      <SingleColumnSection className={classes.root}>
        <ContentStyles contentType="post">
          <h2>
            <strong>Welcome to the Walled Garden, a curated space for truthseekers!</strong>
          </h2>
          {!!gardenCode && (
            <div>
              {codeIsValid ? (
              ...
              {currentUser?.walledGardenInvite && (
                <p>Of course, as a Walled Garden member, you may enter anytime. :)</p>
              )}
              {!currentUser?.walledGardenInvite && isOpenToPublic && (
                <p>However, the Garden is currently to the public, so you may enter anyway! :)</p>
              )}
            </div>
          )}
          {!!gardenCode && (
            <div className={classes.eventDetails}>
              <p>
                <strong>EVENT DETAILS</strong> <em>May contain important instructions</em>
              </p>
  ...
  return (
    <div className={classes.innerPortalPositioning}>
