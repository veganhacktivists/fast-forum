...
export const gatherTownUserTrackingIsBroken = new DatabasePublicSetting<boolean>(
  "gatherTownUserTrackingIsBroken",
  false,
);
...
const GatherTown = ({ classes }: { classes: ClassesType }) => {
  ...
  const tooltip = currentUser.walledGardenInvite ? (
    <LWTooltip title={<div>Click to read more about this space</div>}>
    ...
    <div className={classes.root}>
      ...
      <div>
        <AnalyticsTracker eventType="link" eventProps={{ to: gatherTownURL }} captureOnMount>
          <div>
            <Link to={gatherTownURL}>Walled Garden Beta</Link>
          </div>
        </AnalyticsTracker>
        {userList && userList.length > 0 && (
          <div className={classes.usersOnlineList}>
            {Object.keys(users).map((user) => (
              <span className={classes.userName} key={user}>
                <FiberManualRecordIcon className={classes.onlineDot} /> {user}{" "}
                {users[user]?.status && `(${users[user].status})`}
              </span>
            ))}
            {tooltip}
          </div>
        )}
        {!loading && (!userList || !userList.length) && (
          <div className={classNames(classes.usersOnlineList, classes.noUsers)}>
