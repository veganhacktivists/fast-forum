...
const HomepageMapFilter = ({ classes }: { classes: ClassesType }) => {
  ...
  return (
    <Paper>
      <LWTooltip
        title="September is Meetups Month, celebrating Astral Codex Everywhere. Find a meetup near you."
        placement="left"
      >
        <div className={classNames(classes.section, classes.title)}>
          <Link to="/posts/ynpC7oXhXxGPNuCgH/acx-meetups-everywhere-2023-times-and-places">Meetups Month</Link>
        </div>
      </LWTooltip>
      <Divider />
      ...
      <LWTooltip title="Hide the map from the frontpage" placement="left" inlineBlock={false}>
        <div className={classNames(classes.section, classes.hideSection)} onClick={handleHideMap}>
          <div className={classes.hideText}>Hide Map</div>
        </div>
      </LWTooltip>
    </Paper>
  );
