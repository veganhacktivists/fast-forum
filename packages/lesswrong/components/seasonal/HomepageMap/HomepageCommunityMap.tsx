...
const LocalEventMapMarkerWrappers = ({
  localEvents,
  classes,
}: {
  ...
  return (
    <React.Fragment>
      {localEvents.map((localEvent) => {
        ...
        return (
          <React.Fragment key={`mapEvent${localEvent._id}`}>
            <Marker latitude={localEvent.lat} longitude={localEvent.lng} offsetLeft={-7} offsetTop={-25}>
...
const LocalEventMapMarkerWrappersComponent = registerComponent(
  "LocalEventMapMarkerWrappers",
  LocalEventMapMarkerWrappers,
  {
...
export const HomepageCommunityMap = ({
  dontAskUserLocation = false,
  classes,
}: {
