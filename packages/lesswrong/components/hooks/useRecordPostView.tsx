...
export const useRecordPostView = (post: ViewablePost): { recordPostView: any; isRead: boolean } => {
  const [increasePostViewCount] = useMutation(
    gql`
  ...
  const recordPostView = useCallback(
    async ({ post, extraEventProperties }) => {
      try {
...
export const useRecordTagView = (tag: TagFragment): { recordTagView: any; isRead: boolean } => {
  ...
  const recordTagView = useCallback(
    async ({ tag, extraEventProperties }) => {
      try {
...
export const ItemsReadContextWrapper = ({ children }: { children: React.ReactNode }) => {
  ...
  return <ItemsReadContext.Provider value={providedContext}>{children}</ItemsReadContext.Provider>;
};
