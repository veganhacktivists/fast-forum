...
type CloseableComponents = {
  [T in keyof ComponentTypes]: FromPartial<ComponentTypes[T]["defaultProps"]> extends { onClose: any } | undefined
    ? T
    : never;
}[keyof ComponentTypes];

interface CommentBoxContextType {
  openCommentBox: <T extends CloseableComponents>({
    componentName,
    componentProps,
  }: {
    componentName: T;
    componentProps: Omit<ComponentProps<ComponentTypes[T]>, "onClose" | "classes">;
  }) => void;
  close: () => void;
}
export const CommentBoxContext = React.createContext<CommentBoxContextType | null>(null);

export const CommentBoxManager = ({ children }: { children: React.ReactNode }) => {
  ...
  const commentBoxContext = useMemo(() => ({ openCommentBox, close }), [openCommentBox, close]);

  return (
    <CommentBoxContext.Provider value={commentBoxContext}>
      {children}
      {CommentBoxComponent && componentName && (
        <CommentBoxComponent {...(componentProps as AnyBecauseHard)} onClose={close} />
      )}
