...
const ProfilePhoto = ({
  ...
  let imgNode = <AccountCircleIcon viewBox="3 3 18 18" className={classNames(classes.img, classes.emptyProfileImg)} />;

  // if for some reason we have no user, just show the placeholder icon
  if (!user) {
    return <div className={className}>{imgNode}</div>;
  }
  ...
  if (user.profileImageId) {
    ...
    imgNode = (
      <div className={classNames(classes.img, classes.defaultProfileImg)}>{getUserInitials(user.displayName)}</div>
    );
  }

  return noLink ? (
    <div className={className}>{imgNode}</div>
  ) : (
    <Link to={`/users/${user.slug}${from ? `?from=${from}` : ""}`} className={className}>
      {imgNode}
    </Link>
  );
};
