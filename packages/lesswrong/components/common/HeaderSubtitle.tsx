...
const HeaderSubtitle = ({ classes }: { classes: ClassesType }) => {
  const { currentRoute } = useSubscribedLocation();
  if (!currentRoute) {
    return null;
  }

  const SubtitleComponent: any = currentRoute.subtitleComponentName
    ? Components[currentRoute.subtitleComponentName]
    : null;
  ...
  if (SubtitleComponent) {
    ...
    return <span className={classes.subtitle}>{subtitleString}</span>;
  } else {
