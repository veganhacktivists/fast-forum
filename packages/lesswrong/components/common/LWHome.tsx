  ...
  return (
    <AnalyticsContext pageContext="homePage">
      <React.Fragment>
        ...
        {reviewIsActive() && getReviewPhase() === "RESULTS" && (
          <SingleColumnSection>
            <FrontpageBestOfLWWidget reviewYear={REVIEW_YEAR} />
          </SingleColumnSection>
        )}

        {reviewIsActive() && getReviewPhase() !== "RESULTS" && showReviewOnFrontPageIfActive.get() && (
          <SingleColumnSection>
            <FrontpageReviewWidget reviewYear={REVIEW_YEAR} />
          </SingleColumnSection>
        )}

        <AnalyticsInViewTracker eventProps={{ inViewType: "latestPosts" }} observerProps={{ threshold: [0, 0.5, 1] }}>
          <HomeLatestPosts />
        </AnalyticsInViewTracker>

        {currentUser && <DialoguesList currentUser={currentUser} />}

        <RecentDiscussionFeed af={false} commentsLimit={4} maxAgeHours={18} />
      </React.Fragment>
    </AnalyticsContext>
  );
};
