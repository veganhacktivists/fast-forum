...
export type CloseableComponents = {
  [T in keyof ComponentTypes]: FromPartial<ComponentTypes[T]["propTypes"]> extends { onClose: any } | undefined
    ? T
    : never;
}[keyof ComponentTypes];

export interface OpenDialogContextType {
  openDialog: <T extends CloseableComponents>({
    componentName,
    componentProps,
    noClickawayCancel,
  }: {
...
export const DialogManager = ({ children }: { children: React.ReactNode }) => {
  const [componentNameWithProps, setComponentNameWithProps] = useState<{
  ...
  const modal = ModalComponent && isOpen && (
    <ModalComponent {...componentNameWithProps?.componentProps} onClose={closeDialog} />
  );
  const withClickaway =
    isOpen &&
    (noClickawayCancel ? modal : <LWClickAwayListener onClickAway={closeDialog}>{modal}</LWClickAwayListener>);
  return (
