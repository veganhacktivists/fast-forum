...
export const MessageContextProvider = ({ children }: { children: ReactNode }) => {
  ...
  const messagesContext = useMemo(() => ({ messages, flash, clear }), [messages, flash, clear]);
  ...
  return <MessageContext.Provider value={messagesContext}>{children}</MessageContext.Provider>;
};

const FlashMessages = ({ classes }: { classes: ClassesType }) => {
      ...
      <Snackbar
        ...
        action={
          messageObject?.action && (
            <Button onClick={messageObject?.action} color="primary">
