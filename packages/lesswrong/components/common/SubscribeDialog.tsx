...
class SubscribeDialog extends Component<SubscribeDialogProps, SubscribeDialogState> {
  ...
  isAlreadySubscribed() {
    if (this.state.view === "curated" && this.props.currentUser && this.props.currentUser.emailSubscribedToCurated)
      return true;
    return false;
  }
  ...
  render() {
    ...
    const viewSelector = (
      <FormControl key="viewSelector" className={classes.viewSelector}>
        ...
        >
          ...
          <MenuItem value="frontpage" disabled={method === "email"}>
            Frontpage
          </MenuItem>
          <MenuItem value="community" disabled={method === "email"}>
            {preferredHeadingCase("All Posts")}
          </MenuItem>
        </Select>
      </FormControl>
    );
    ...
    return (
      <LWDialog fullScreen={fullScreen} open={open} onClose={onClose}>
        {isLWorAF && (
          <Tabs
            value={method}
        ...
        <DialogContent className={classes.content}>
          {method === "rss" && (
            <React.Fragment>
              ...
              {(view === "community" || view === "frontpage") && (
                <div>
                  <DialogContentText>
                    Generate a RSS link to posts in {viewNames[view]} of this karma and above.
                  </DialogContentText>
                  <RadioGroup
                    value={threshold}
                  ...
                  >
                    {thresholds
                      .map((t: AnyBecauseTodo) => t.toString())
                      .map((threshold: AnyBecauseTodo) => (
                        <FormControlLabel
                          control={<Radio />}
                          label={threshold}
                          value={threshold}
          ...
          {method === "email" && [
            ...
            !!currentUser ? (
              [
                !this.emailFeedExists(view) && (
                  <DialogContentText key="dialogNoFeed" className={classes.errorMsg}>
                    Sorry, there's currently no email feed for {viewNames[view]}.
                  </DialogContentText>
                ),
                subscribedByEmail && !userEmailAddressIsVerified(currentUser) && !isEAForum && (
                  <DialogContentText key="dialogCheckForVerification" className={classes.infoMsg}>
                    We need to confirm your email address. We sent a link to {getUserEmail(currentUser)}; click the link
        ...
        <DialogActions>
          ...
          {method === "email" &&
            (this.isAlreadySubscribed() ? (
              ...
              >
                {subscribedByEmail ? "Subscribed!" : "Subscribe to Feed"}
              </Button>
            ))}
...
const SubscribeDialogComponent = registerComponent<ExternalProps>("SubscribeDialog", SubscribeDialog, {
  styles,
  hocs: [withMobileDialog(), withUser, withUpdateCurrentUser, withTracking],
});
