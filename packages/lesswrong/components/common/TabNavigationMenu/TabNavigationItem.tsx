...
const TabNavigationItem = ({ tab, onClick, className, classes }: TabNavigationItemProps) => {
  ...
  return (
    <LWTooltip placement="right-start" title={tab.tooltip || ""} className={classes.tooltip}>
      <MenuItemLink
        onClick={handleClick}
      ...
      >
        ...
        {tab.subItem ? (
          <TabNavigationSubItem>{tab.title}</TabNavigationSubItem>
        ) : (
          <span className={classes.navText}>{tab.title}</span>
        )}
      </MenuItemLink>
    </LWTooltip>
  );
...
const TabNavigationItemComponent = registerComponent("TabNavigationItem", TabNavigationItem, { styles });
