...
const getQuery = ({
  ...
  const fragmentsUsed = _.filter(
    Object.keys(renderers).map((r) => renderers[r].fragmentName),
    (f) => f,
  );
  const queryArgsList = [
    "$limit: Int",
...
const MixedTypeFeed = (args: {
  ...
  if (refetchRef && refetch) refetchRef.current = refetch;
      ...
      if (!queryIsPending.current) {
        ...
        void fetchMore({
          ...
          updateQuery: (prev, { fetchMoreResult }: { fetchMoreResult: any }) => {
            ...
            const deduplicatedResults = fetchMoreResult[resolverName].results.filter(
              (result: any) => !prevKeys.has(keyFunc(result)),
            );
...
const RenderFeedItem = React.memo(({ renderers, item }: { renderers: any; item: any }) => {
