  ...
  return (
    <React.Fragment>
      {(mode === "daily" || mode === "weekly") && (
        <span>
          <Select value={timeOfDay} onChange={(event) => applyChange({ timeOfDay: parseInt(event.target.value) })}>
            {_.range(24).map((hour) => (
      ...
      {mode === "weekly" && (
        <span>
          {" on "}
          <Select value={dayOfWeek} onChange={(event) => applyChange({ dayOfWeek: event.target.value })}>
