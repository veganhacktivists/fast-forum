...
const PodcastEpisodeInput = ({
  ...
  const {
    results: [existingPodcastEpisode] = [],
    refetch: refetchPodcastEpisode,
  ...
  const syncPodcastEpisodeId = useCallback(
    (id: string) => {
  ...
  return loading ? (
    ...
    <>
      <div>
        <Select value={podcastId} onChange={selectPodcastId}>
          {podcasts.map((podcast) => {
            return (
              <MenuItem key={podcast._id} value={podcast._id}>
                {podcast.title}
              </MenuItem>
            );
          })}
