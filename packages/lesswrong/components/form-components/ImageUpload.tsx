...
const formPreviewSizeByImageType: Record<ImageType, { width: number | "auto"; height: number }> = {
  gridImageId: {
    width: 203,
    height: 80,
  },
...
const FormProfileImage: FC<{
  ...
  return <Components.UsersProfileImage user={user ? { ...user, profileImageId } : undefined} size={size} />;
};
...
const TriggerButton: FC<{
  ...
  return (
    <Button onClick={uploadImage} className={classNames("image-upload-button", mainClass)}>
...
const RemoveButton: FC<{
  ...
  const mainClass =
    isFriendlyUI && imageType === "profileImageId" ? classes.removeProfileImageButton : classes.removeButton;
  return (
    <Button title="Remove" onClick={removeImage} className={mainClass}>
      Remove
    </Button>
  );
};
    ...
    <div className={classes.root}>
      ...
      <div className={classes.img}>
        {showUserProfileImage && (
          <FormProfileImage document={document} profileImageId={imageId} size={formPreviewSize.height} />
        )}
        {imageId && !showUserProfileImage && <Components.CloudinaryImage2 publicId={imageId} {...formPreviewSize} />}
      </div>
      <TriggerButton
        imageType={imageType}
        imageId={imageId}
      ...
      <RemoveButton imageType={imageType} imageId={imageId} removeImage={removeImg} classes={classes} />
    </div>
  );
};
