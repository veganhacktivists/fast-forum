...
const PostLinkPreviewSequencePost = ({
  href,
  targetLocation,
  id,
  children,
...
const PostLinkPreviewSequencePostComponent = registerComponent(
  "PostLinkPreviewSequencePost",
  PostLinkPreviewSequencePost,
);

const PostLinkPreviewSlug = ({
  href,
  targetLocation,
  id,
  children,
...
const CommentLinkPreviewLegacy = ({
  href,
  targetLocation,
  id,
  children,
  ...
  if (comment) {
    return (
      <Components.CommentLinkPreviewWithComment comment={comment} post={post} error={error} href={href} id={id}>
        {children}
      </Components.CommentLinkPreviewWithComment>
    );
  }
  return (
    <Components.PostLinkPreviewWithPost href={href} post={post} error={error} id={id}>
      {children}
...
const PostCommentLinkPreviewGreaterWrong = ({
  href,
  targetLocation,
  id,
  children,
  ...
  return (
    <Components.PostLinkCommentPreview href={href} commentId={commentId} post={post || null} id={id}>
      {children}
    </Components.PostLinkCommentPreview>
  );
};
const PostCommentLinkPreviewGreaterWrongComponent = registerComponent(
  "PostCommentLinkPreviewGreaterWrong",
  PostCommentLinkPreviewGreaterWrong,
);

const PostLinkPreviewVariantCheck = ({
  href,
  post,
  targetLocation,
  comment,
  ...
  if (targetLocation.query.commentId) {
    return (
      <PostLinkCommentPreview commentId={targetLocation.query.commentId} href={href} post={post} id={id}>
        {children}
  ...
  if (targetLocation.hash) {
    ...
    if (looksLikeDbIdString(commentId)) {
      return (
        <PostLinkCommentPreview commentId={commentId} href={href} post={post} id={id}>
          {children}
        </PostLinkCommentPreview>
      );
    }
  ...
  if (commentId) {
    return (
      <Components.PostLinkCommentPreview commentId={commentId} post={post} href={href} id={id}>
        {children}
      </Components.PostLinkCommentPreview>
    );
  }
...
const PostLinkPreviewVariantCheckComponent = registerComponent(
...
const PostLinkPreviewWithPost = ({
  ...
  return (
    <PostsTooltip post={post} hash={hash} placement="bottom-start" clickable={!isFriendlyUI} As="span">
...
const CommentLinkPreviewWithComment = ({
  classes,
  href,
  comment,
  post,
  ...
  const { PostsTooltip } = Components;
  return (
    <PostsTooltip post={post} comment={comment} placement="bottom-start" As="span" clickable={!isFriendlyUI}>
...
const CommentLinkPreviewWithCommentComponent = registerComponent(
  ...
  return (
    <span {...eventHandlers}>
      <LWPopper open={hover} anchorEl={anchorEl} placement="bottom-start" clickable={false}>
        <Card>
          <div className={classes.hovercard}>{href}</div>
        </Card>
      </LWPopper>
...
const MetaculusPreview = ({
  ...
  const { AnalyticsTracker, LWPopper } = Components;
  const { anchorEl, hover, eventHandlers } = useHover();
  const [match, www, questionNumber] =
    ...
    <AnalyticsTracker eventType="link" eventProps={{ to: href }}>
      <span {...eventHandlers}>
        ...
        <LWPopper open={hover} anchorEl={anchorEl} placement="bottom-start">
          <div className={classes.background}>
            <iframe
              className={classes.iframeStyling}
              src={`https://d3s0w6fek99l5b.cloudfront.net/s/1/questions/embed/${questionNumber}/?plot=pdf`}
            />
          </div>
        </LWPopper>
      </span>
...
const MetaforecastPreviewComponent = registerComponent("MetaforecastPreview", MetaforecastPreview, {
  styles: metaforecastStyles,
});

const ArbitalLogo = () => (
  <svg x="0px" y="0px" height="100%" viewBox="0 0 27.5 23.333">
    <g>
      <path
        d="M19.166,20.979v-0.772c-1.035,0.404-2.159,0.626-3.334,0.626c-0.789,0-1.559-0.1-2.291-0.288
      ...
      ></path>
      <path
        d="M19.443,2.975c-1.123-1.007-2.441-1.799-3.889-2.311c-0.623-0.221-1.273-0.391-1.943-0.502
      ...
      ></path>
      <path
        d="M23.609,2.971c-1.123-1.005-2.44-1.795-3.888-2.307C19.4,0.551,19.073,0.451,18.74,0.365
          ...
          <Card>
            <ContentStyles contentType="comment" className={classes.hovercard}>
              <div className={classes.headerRow}>
                <a href={href}>
                  <h2>{rawData?.ArbitalPageData?.title}</h2>
                </a>
                <a href="https://arbital.com" title="This article is hosted on Arbital.com">
...
const ViewpointsPreviewComponent = registerComponent("ViewpointsPreview", ViewpointsPreview, {
  styles: viewpointsStyles,
});
