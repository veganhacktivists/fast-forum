...
export const distance = (
  ...
  const a =
    ...
    Math.sin(dLng / 2) * Math.sin(dLng / 2) * Math.cos(toRad(start.lat)) * Math.cos(toRad(end.lat));
...
const LocalGroups = ({
  ...
  if (results && keywordSearch) {
    localGroups = results.filter((group) =>
      `${group.name.toLowerCase()} ${group.nameInAnotherLanguage?.toLowerCase() ?? ""} ${group.location?.toLowerCase() ?? ""}`.includes(
      ...
      {!loading && !localGroups?.length ? (
        ...
        <div className={classes.localGroupsList}>
          {localGroups?.map((group) => {
            ...
            return (
              <div key={group._id} className={classes.localGroup}>
                ...
                <div className={classes.localGroupContent} style={rowStyle}>
                  <div className={classes.localGroupNameRow}>
                    ...
                    <div className={classes.localGroupDistance}>{distanceToGroup}</div>
                  </div>
          ...
          {!includeInactive && (
            <div className={classes.postGroupsCTA}>
              <Button color="primary" onClick={toggleIncludeInactive} className={classes.includeInactiveBtn}>
                Search inactive groups
              </Button>
            </div>
