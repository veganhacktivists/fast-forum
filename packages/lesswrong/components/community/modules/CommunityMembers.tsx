...
const styles = createStyles(
  (theme: ThemeType): JssStyles => ({
    ...
    person: {
      ...
      borderColor: theme.palette.greyAlpha(0.1),
    },
    content: {
      padding: 20,
      [theme.breakpoints.down("xs")]: {
...
const CommunityMembers = ({
  ...
  const CommunityMember = ({ hit }: { hit: SearchUser }) => {
          ...
          <div className={classes.photoRow}>
            ...
            <div className={classes.photoRowText}>
              <div className={classes.nameRow}>
                ...
                <div className={classes.distance}>{distanceToPerson}</div>
              </div>
  ...
  return (
    <InstantSearch indexName={"test_users"} searchClient={getSearchClient()}>
      <div className={classes.filters}>
        <CustomSearchBox />
        <div className={classes.locationFilter}>{locationFilterNode}</div>
        <Link to="/community/map" className={classes.fullMapLink}>
          View full map
        </Link>
      </div>
