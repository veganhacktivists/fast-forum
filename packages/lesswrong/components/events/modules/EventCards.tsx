...
const EventCards = ({
  ...
  const eventCards = events.map((event) => {
    return (
      <Card key={event._id} className={classNames(classes.eventCard, cardClassName)}>
        <Link to={`/events/${event._id}/${event.slug}`}>
          {event.eventImageId ? (
            <CloudinaryImage2
              height={195}
              width={373}
              publicId={event.eventImageId}
              imgProps={{ q: "100" }}
        ...
        <CardContent className={classes.eventCardContent}>
          ...
          {!hideGroupNames && event.group && (
            <div className={classes.eventCardGroup} title={event.group.name}>
