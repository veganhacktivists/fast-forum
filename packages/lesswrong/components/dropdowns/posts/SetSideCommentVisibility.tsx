...
const SetSideCommentVisibility = ({ classes }: { classes: ClassesType }) => {
  ...
  // hide it.
  if (!sideCommentVisibility || !hasSideComments) return null;
  ...
  const submenu = (
    <Paper>
      ...
      {sideCommentModes.map((mode) => (
        ...
        >
          {sideCommentMode === mode.name ? <Check className={classes.check} /> : <div className={classes.notChecked} />}
  ...
  return (
    <LWTooltip title={submenu} tooltip={false} clickable={true} inlineBlock={false} placement="left-start">
      <MenuItem>
        <ListItemIcon>
          <ChatBubbleOutline />
        </ListItemIcon>
        <span className={classes.sideCommentsLabel}>Side-comments</span>
        <span className={classNames(classes.showOnlyIfMobile, classes.currentSelectionPreview)}>Hidden</span>
...
const SetSideCommentVisibilityComponent = registerComponent("SetSideCommentVisibility", SetSideCommentVisibility, {
  styles,
});
