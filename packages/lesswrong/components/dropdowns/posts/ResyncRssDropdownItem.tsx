...
const ResyncRssDropdownItem = ({
  ...
  return <DropdownItem title="Resync RSS" onClick={onClick} icon={() => <RssFeed />} />;
};

const ResyncRssDialog = ({
  onClose,
  post,
  classes,
}: {
  ...
  return (
    <LWDialog title="RSS Resync" open={true} onClose={onClose}>
      <DialogContent>
        <h1>Resync Crosspost</h1>
        <div className={classes.changesPreview}>
          {loading && <Loading />}
          {data?.RssPostChanges && !data.RssPostChanges?.isChanged && <div>No changes were detected</div>}
          {data?.RssPostChanges && data.RssPostChanges?.isChanged && (
            <div>
              <div className={classes.diffExplanation}>
                The version this was auto-crossposted from has changed. Check the <ins>insertions</ins> and{" "}
                <del>deletions</del> below and click Apply to copy the changes.
              </div>
      ...
      <DialogActions>
        <div className={classes.buttons}>
          ...
          <Button className={classes.button} onClick={apply} disabled={loading || !!error}>
            Apply
          </Button>
        </div>
      </DialogActions>
