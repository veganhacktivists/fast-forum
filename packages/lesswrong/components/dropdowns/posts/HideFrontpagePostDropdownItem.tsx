...
const HideFrontpagePostDropdownItem = ({ post }: { post: PostsBase }) => {
  ...
  const toggleShown = () => {
    ...
    if (isHidden) {
      ...
      newHiddenList = alreadyHidden ? oldHiddenList : [...oldHiddenList, { postId: post._id }];
    } else {
  ...
  return <DropdownItem title={title} onClick={toggleShown} icon={icon} />;
};

const HideFrontPageButtonComponent = registerComponent("HideFrontpagePostDropdownItem", HideFrontpagePostDropdownItem, {
  styles,
