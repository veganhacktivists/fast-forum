...
const MoveToAnswersDropdownItem = ({ comment, post }: { comment: CommentsList; post?: PostsBase }) => {
  ...
  if (comment.answer) {
    return <DropdownItem title={preferredHeadingCase("Move To Comments")} onClick={handleMoveToComments} />;
  }

  return <DropdownItem title={preferredHeadingCase("Move To Answers")} onClick={handleMoveToAnswers} />;
};

const MoveToAnswersDropdownItemComponent = registerComponent("MoveToAnswersDropdownItem", MoveToAnswersDropdownItem);
