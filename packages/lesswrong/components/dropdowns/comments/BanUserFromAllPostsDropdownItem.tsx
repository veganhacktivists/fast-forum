...
const BanUserFromAllPostsDropdownItem = ({ comment, post }: { comment: CommentsList; post?: PostsBase }) => {
  ...
  const handleBanUserFromAllPosts = (event: React.MouseEvent) => {
    ...
    if (confirm("Are you sure you want to ban this user from commenting on all your posts?")) {
      ...
      void updateCurrentUser({
        ...
        flash({
          messageString: `User ${comment?.user?.displayName} is now banned from commenting on any of your posts`,
        }),
      );
    }
  };
  ...
  return <DropdownItem title="Ban from all your posts" onClick={handleBanUserFromAllPosts} />;
};
