...
const BanUserFromAllPersonalPostsDropdownItem = ({ comment, post }: { comment: CommentsList; post?: PostsBase }) => {
  const currentUser = useCurrentUser();
  const updateCurrentUser = useUpdateCurrentUser();
  const { flash } = useMessages();

  if (!post || !userCanModeratePost(currentUser, post) || post.frontpageDate || !userOwns(currentUser, post)) {
    return null;
  }
  ...
  const handleBanUserFromAllPosts = (event: React.MouseEvent) => {
    ...
    if (confirm("Are you sure you want to ban this user from commenting on all your personal blog posts?")) {
      ...
      void updateCurrentUser({
        ...
        flash({
          messageString: `User ${comment?.user?.displayName} is now banned from commenting on any of your personal blog posts`,
        }),
      );
    }
  };
  ...
  return <DropdownItem title="Ban from all your personal blog posts" onClick={handleBanUserFromAllPosts} />;
};
