...
const LockThreadDropdownItem = ({ comment }: { comment: CommentsList }) => {
  ...
  if (comment.repliesBlockedUntil) {
    return <DropdownItem title={preferredHeadingCase("Unlock Thread")} onClick={handleUnlockThread} />;
  }

  return <DropdownItem title={preferredHeadingCase("Lock Thread")} onClick={handleLockThread} />;
};

const LockThreadDropdownItemComponent = registerComponent("LockThreadDropdownItem", LockThreadDropdownItem);
