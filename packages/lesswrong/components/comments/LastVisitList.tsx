...
const LastVisitList = ({
  ...
  if (filteredVisits.length > VISITS_TO_SHOW) filteredVisits = _.take(filteredVisits, VISITS_TO_SHOW);

  return (
    <>
      {filteredVisits.map((visit) => (
        <MenuItem key={visit._id} dense onClick={() => clickCallback(visit.createdAt)}>
          Visit at:&nbsp;
          <Components.CalendarDate date={visit.createdAt} />{" "}
        </MenuItem>
      ))}
    </>
  );
