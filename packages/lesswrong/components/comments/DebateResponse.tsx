...
const getParticipantBorderStyle = (classes: ClassesType<typeof styles>, participantIndex: number) =>
  classes[`border${participantIndex}`] ?? classes.border0;

export const DebateResponse = ({
  classes,
  comment,
  replies,
  idx,
  ...
  const { CommentUserName, CommentsItemDate, CommentBody, CommentsEditForm, CommentsMenu, DebateCommentsListSection } =
    Components;
  ...
  const menu = (
    <CommentsMenu comment={comment} post={post} showEdit={() => setShowEdit(true)} className={classes.menu} />
  );
  ...
  const replyLink = showReplyLink && (
    <a
      className={classNames("comments-item-reply-link", classes.replyLink)}
      onClick={(e) => setShowReplyState(!showReplyState)}
    >
  ...
  return (
    ...
    >
      {header}
      {menu}
      <div className={classes.commentWithReplyButton}>{commentBodyOrEditor}</div>
      {replyState}
