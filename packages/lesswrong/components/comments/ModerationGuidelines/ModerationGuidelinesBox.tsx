...
const getPostModerationGuidelines = (post: PostsList, classes: ClassesType) => {
  ...
  const combinedGuidelines = `
    ...
    ${post.frontpageDate ? frontpageGuidelines : html || moderationStyle ? "" : defaultGuidelines}
  `;
...
const ModerationGuidelinesBox = ({
  ...
  const { combinedGuidelines, truncatedGuidelines } = isPostType(document)
    ? getPostModerationGuidelines(document, classes)
    : getSubforumModerationGuidelines(document);
  ...
  return (
    <div className={classes.root} onClick={expandable ? handleClick : undefined}>
      {!!(isPostType(document)
        ? userCanModeratePost(currentUser, document)
        : userCanModerateSubforum(currentUser, document)) && (
