...
const CommentsNode = ({
  ...
  const [collapsed, setCollapsed] = useState(
    !forceUnCollapsed &&
      (comment.deleted || comment.baseScore < karmaCollapseThreshold || comment.modGPTRecommendation === "Intervene"),
  );
  ...
  const beginSingleLine = (): boolean => {
    ...
    if (treeOptions.isSideComment && nestingLevel > 1) return true;
  ...
  const scrollIntoView = useCallback(
    (behavior: "auto" | "smooth" = "smooth") => {
      ...
      setHighlighted(true);
      setTimeout(() => {
        //setTimeout make sure we execute this after the element has properly rendered
  ...
  return (
    <div className={comment.gapIndicator ? classes.gapIndicator : undefined}>
      <CommentFrame
        comment={comment}
      ...
      >
        ...
        {!collapsed && childComments && childComments.length > 0 && (
...
const CommentsNodeComponent = registerComponent("CommentsNode", CommentsNode, {
  ...
  areEqual: {
    treeOptions: "shallow",
    childComments: (oldValue: Array<CommentTreeNode<CommentsList>>, newValue: Array<CommentTreeNode<CommentsList>>) =>
      commentTreesEqual(oldValue, newValue),
  },
