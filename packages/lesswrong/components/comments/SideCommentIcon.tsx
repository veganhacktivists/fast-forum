...
const BadgeWrapper = ({
  ...
  if (commentCount > 1) {
    return (
      <Badge classes={{ badge: classes.badge }} badgeContent={commentCount}>
        {children}
      </Badge>
    );
  } else {
...
const SideCommentIcon = ({
  ...
  const onMouseLeave = () => {
    if (pinned === "closed") setPinned("auto");
  };
  ...
  return (
    <div className={classes.sideCommentIconWrapper} onMouseLeave={onMouseLeave}>
      <span {...eventHandlers} onClick={onClick} className={classes.sideCommentIcon}>
        <BadgeWrapper commentCount={commentIds.length} classes={classes}>
          <CommentIcon className={classNames({ [classes.pinned]: pinned === "open" })} />
        </BadgeWrapper>
        {isOpen && <span className={classes.clickToPinMessage}>Click to {pinned === "open" ? "close" : "pin"}</span>}
...
const SideCommentHover = ({
  ...
  return (
    <div className={classes.sideCommentHover}>
      {commentIds.map((commentId) => (
        <SideCommentSingle key={commentId} commentId={commentId} post={post} dontTruncateRoot={dontTruncateRoot} />
      ))}
    </div>
  );
};

const SideCommentSingle = ({
  commentId,
  post,
  dontTruncateRoot = false,
  classes,
}: {
  commentId: string;
  post: PostsDetails;
        ...
        commentNodeProps={{
          treeOptions: {
            ...
            replaceReplyButtonsWith: (comment) => (
              <a href={"#" + comment._id} className={classes.seeInContext}>
                See in context
              </a>
            ),
