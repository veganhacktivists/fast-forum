...
const CommentBottomCaveats = ({ comment, classes }: { comment: CommentsList; classes: ClassesType }) => {
    ...
    <>
      {blockedReplies && (
        <div className={classes.blockedReplies}>
          ...
          {!blockIsForever && (
            <>
              until <Components.CalendarDate date={comment.repliesBlockedUntil} />
            </>
          )}
        </div>
      )}
      ...
      {commentIsHidden(comment) && !comment.rejected && (
        <Components.MetaInfo className={classes.caveatText}>
          [This comment will not be visible to other users until the moderation team checks it for spam or norm
          violations.]
        </Components.MetaInfo>
      )}
    </>
