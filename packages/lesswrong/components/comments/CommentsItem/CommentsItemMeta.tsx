...
export const CommentsItemMeta = ({
   ...
   */
  const showModeratorCommentAnnotation =
    comment.moderatorHat && (userIsAdmin(currentUser) ? true : !comment.hideModeratorHat);
  ...
  const reviewingForReview =
    isEAForum && comment.reviewingForReview === "2020" ? "the Decade" : comment.reviewingForReview;
  ...
  return (
    ...
    >
      {!parentCommentId && !comment.parentCommentId && isParentComment && <div>○</div>}
      {post && <CommentShortformIcon comment={comment} post={post} />}
      {!showCommentTitle && <CommentDiscussionIcon comment={comment} small />}
      {!hideParentCommentToggle &&
        !(hideParentCommentToggleForTopLevel && comment.parentCommentId === comment.topLevelCommentId) &&
        ...
        parentAnswerId != comment.parentCommentId && (
          <ShowParentComment comment={comment} active={showParentState} onClick={toggleShowParent} />
        )}
        ...
        <a className={classes.collapse} onClick={toggleCollapse}>
          {isFriendlyUI ? (
            <ForumIcon
              ...
              className={classNames(classes.collapseChevron, { [classes.collapseChevronOpen]: !collapsed })}
            />
          ) : (
            <>
              [<span>{collapsed ? "+" : "-"}</span>]
      ...
      {singleLineCollapse && (
        <a className={classes.collapse} onClick={() => setSingleLine && setSingleLine(true)}>
          [<span>{collapsed ? "+" : "-"}</span>]
        </a>
      )}
      ...
      {showModeratorCommentAnnotation && <span className={classes.moderatorHat}>{moderatorCommentAnnotation}</span>}
      {!comment.debateResponse && !comment.rejected && (
        <SmallSideVote document={comment} collection={Comments} hideKarma={post?.hideCommentKarma} />
      )}

      {post && <CommentOutdatedWarning comment={comment} post={post} />}

      {comment.nominatedForReview && (
        <Link to={`/nominations/${comment.nominatedForReview}`} className={classes.metaNotice}>
          {`Nomination for ${comment.nominatedForReview} Review`}
        </Link>
      )}

      {comment.reviewingForReview && (
        <Link to={getReviewLink(comment.reviewingForReview)} className={classes.metaNotice}>
          {`Review for ${reviewingForReview} Review`}
        </Link>
      )}

      {!!relevantTagsTruncated.length && (
        <span className={classes.relevantTags}>
          {relevantTagsTruncated.map((tag) => (
            <FooterTag tag={tag} key={tag._id} className={classes.relevantTag} neverCoreStyling={isBookUI} smallText />
          ))}
          {shouldDisplayLoadMore && (
            <LoadMore loadMore={() => setShowMoreClicked(true)} message="Show more" className={classes.showMoreTags} />
          )}
        </span>
      )}
      ...
      <span className={classes.rightSection}>
        ...
        {!isParentComment && !hideActionsMenu && (
          <AnalyticsContext pageElementContext="tripleDotMenu">
            <CommentsMenu className={classes.menu} comment={comment} post={post} tag={tag} showEdit={setShowEdit} />
          </AnalyticsContext>
        )}
      </span>
    </div>
...
const CommentsItemMetaComponent = registerComponent("CommentsItemMeta", CommentsItemMeta, { styles });
