...
const CommentOutdatedWarning = ({
  ...
  if (!postHadMajorRevision(comment, post)) return null;
  ...
  return (
    <span className={classes.outdatedWarning}>
      <LWTooltip title="The top-level post had major updates since this comment was created. Click to see post at time of creation.">
        <QueryLink query={{ revision: comment.postVersion }} merge>
          <HistoryIcon className={classes.icon} /> Response to previous version{" "}
        </QueryLink>
      </LWTooltip>
    </span>
  );
...
const CommentOutdatedWarningComponent = registerComponent("CommentOutdatedWarning", CommentOutdatedWarning, { styles });
