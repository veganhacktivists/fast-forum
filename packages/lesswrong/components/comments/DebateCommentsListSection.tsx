...
const DebateCommentsListSection = ({
  post,
  totalComments,
  comments,
  newForm = true,
      ...
      {newForm &&
        ...
        (!post?.draft || userIsDebateParticipant || userIsAdmin(currentUser)) && (
          <div id={`posts-debate-thread-new-comment`} className={classes.newComment}>
            ...
            <Components.CommentsNewForm post={post} type="comment" {...newFormProps} />
          </div>
        )}
      {currentUser && post && !userIsAllowedToComment(currentUser, post, postAuthor, false) && (
...
const DebateCommentsListSectionComponent = registerComponent("DebateCommentsListSection", DebateCommentsListSection, {
  styles,
});
