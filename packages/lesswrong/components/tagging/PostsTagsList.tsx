...
export const PostsTagsList = ({
  ...
  const tagButton = (tag: TagWithCount) => (
    <LWTooltip title={tagButtonTooltip(tag)}>
      <div
        key={tag._id}
        className={classNames(classes.tagFilter, { [classes.selected]: currentFilter === tag._id })}
        onClick={() => handleFilter(tag._id)}
      >
  ...
  return (
    <div className={classes.root}>
      ...
      {max === defaultMax && (
        <LWTooltip title={`Show ${expandedNumberOfTags - defaultMax} more tags`}>
          <a className={classes.button} onClick={() => setMax(expandedNumberOfTags)}>
            More
          </a>
        </LWTooltip>
      )}
      {max !== defaultMax && (
        <a className={classes.button} onClick={() => setMax(defaultMax)}>
          Fewer
        </a>
      )}
    </div>
