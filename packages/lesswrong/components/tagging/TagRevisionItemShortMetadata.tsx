...
const TagRevisionItemShortMetadata = ({
  tag,
  revision,
  classes,
}: {
    ...
    <>
      ...
      <Link to={revUrl}>
        <LWTooltip title="View Selected Revision">
          <>
            <MetaInfo>v{revision.version}</MetaInfo>
            <MetaInfo>
      ...
      <MetaInfo>
        <Link to={revUrl}>
          <ChangeMetricsDisplay changeMetrics={revision.changeMetrics} /> {revision.commitMessage}
        </Link>
      </MetaInfo>{" "}
      <MetaInfo>
        <SmallSideVote document={revision} collection={Revisions} />
      </MetaInfo>
    </>
  );
};

const TagRevisionItemShortMetadataComponent = registerComponent(
