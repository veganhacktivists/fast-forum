...
const PostsItemTagRelevance = ({ tagRel, classes }: { tagRel: WithVoteTagRel; classes: ClassesType }) => {
  ...
  const tooltip = (
    <div>
      ...
      <div>
        ({tagRel.voteCount} {tagRel.voteCount === 1 ? "vote" : "votes"})
      </div>
    ...
    <PostsItem2MetaInfo className={classes.root}>
      <Tooltip title={tooltip} placement="left-end">
        <span>
          ...
          <div className={classes.score}>{voteProps.baseScore}</div>
