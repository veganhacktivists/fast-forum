...
const TagEditsByUser = ({ userId, limit, classes }: { userId: string; limit: number; classes: ClassesType }) => {
  ...
  const resultsWithLiveTags = results.filter((tagUpdates) => tagUpdates.tag && !tagUpdates.tag.deleted);

  if (resultsWithLiveTags.length === 0) {
    return (
      <Components.Typography variant="body2" className={classes.wikiEmpty}>
