  ...
  return (
    <div className={classes.root}>
      <div className={classNames(classes.description, { [classes.collapsedDescription]: collapse })}>
        {editing ? (
          <EditTagForm tag={tag} successCallback={() => setEditing(false)} cancelCallback={() => setEditing(false)} />
        ) : (
      ...
      {!showFlags && (
        <div className={classNames(classes.posts, { [classes.collapsedPosts]: collapse })}>
          <div>
            ...
            {tagRels &&
              tagRels.map(
                (tagRel) => tagRel.post && <TagSmallPostLink key={tagRel._id} post={tagRel.post} hideMeta wrap />,
              )}
          </div>
        </div>
      )}
      {showFlags && (
        <div className={classNames(classes.posts, classes.flags, { [classes.collapsedFlags]: collapse })}>
          {(tag as TagWithFlagsFragment)?.tagFlags
            ?.filter((tagFlag) => !tagFlag.deleted)
            .map((tagFlag) => (
              <span key={tagFlag._id}>
