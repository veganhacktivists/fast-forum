...
const TagIntroSequence = ({ tag, classes }: { tag: TagPageFragment; classes: ClassesType }) => {
  ...
  return (
    <div className={classes.root}>
      ...
      <AnalyticsContext listContext={"tagIntroSequnce"}>
        {loading && <Loading />}
        {posts.map((post, i) => (
          <PostsItemIntroSequence key={post._id} post={post} sequence={sequence} withImage={i === 0} />
        ))}
        {totalCount > PREVIEW_N && !loadedMore && (
          <LoadMore
