...
const reformatTagPlaceholder = (moreCount: number, _totalItems: number, moreNode: Element) => {
  moreNode.innerHTML = moreNode.innerHTML.replace(/\d+ more/, `${moreCount} more`);
};

const TruncatedTagsList = ({
  post,
  expandContainer,
  className,
  classes,
  ...
  useEffect(() => {
    ...
    const handler = () => recalculateTruncation(ref, expandContainer, classes, reformatTagPlaceholder);
    handler();
...
const TruncatedTagsListComponent = registerComponent("TruncatedTagsList", TruncatedTagsList, {
  styles,
  stylePriority: -1,
});
