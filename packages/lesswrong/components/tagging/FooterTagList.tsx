...
export function sortTags<T>(list: Array<T>, toTag: (item: T) => TagBasicInfo | null | undefined): Array<T> {
  return sortBy(list, isFriendlyUI ? (item) => !toTag(item)?.core : (item) => toTag(item)?.core);
}
...
const FooterTagList = ({
  ...
  const onTagSelected = useCallback(
    async ({ tagId, tagName }: { tagId: string; tagName: string }) => {
      try {
        setIsAwaiting(true);
        await mutate({
          variables: {
  ...
  const PostTypeTag = ({ tooltipBody, label }: { tooltipBody: React.ReactNode; label: string }) => (
    <LWTooltip
      title={
        <Card className={classes.card}>
          <ContentStyles contentType="comment">{tooltipBody}</ContentStyles>
        </Card>
      }
      tooltip={false}
    >
  ...
  return (
    <>
      <span ref={rootRef} className={classNames(classes.root, { [classes.allowTruncate]: !showAll })}>
        {innerContent}
      </span>
      {displayShowAllButton && (
        <div className={classes.showAll} onClick={onClickShowAll}>
          Show all {taggingNamePluralSetting.get()}
        </div>
      )}
    </>
  );
