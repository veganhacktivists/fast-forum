...
const TaggingDashboard = ({ classes }: { classes: ClassesType }) => {
  const {
    SectionTitle,
    TagsDetailsItem,
    SectionButton,
  ...
  const {
    ...
    terms: fieldIn(query.focus, multiTerms)
    ...
    <div>
      <div className={classes.root}>
        <div className={classes.navigationLinks}>
          ...
          <p>
            <a href="#Pages_in_need_of_work">{dashboardTagsNameAlt} in Need of Work</a>
          </p>
          <p>
            <a href="#Newest_tags">Newest {taggingNameCapitalSetting.get()}</a>
          </p>
          <p>
            <a href="#Tag_voting_activity">{taggingNameCapitalSetting.get()} Voting Activity</a>
          </p>
          <p>
            <a href="#Tag_activity_feed">Combined {dashboardTagsNameAlt} Activity Feed</a>
          </p>
          <div className={classes.navigationLinksDivider} />
        </div>
        <div id="Pages_in_need_of_work">
          <SectionTitle title={`${dashboardTagsNameAlt} in Need of Work`}>
            <SectionButton>
              ...
              >
                {" "}
                {collapsed ? "Uncollapse" : "Collapse"} {taggingNamePluralCapitalSetting.get()}
              </a>
            </SectionButton>
          </SectionTitle>
          <div className={classes.flagList}>
            ...
            {currentUser && (
              <QueryLink query={query.focus === "myPages" ? {} : { focus: "myPages" }}>
                <TagFlagItem itemType={"userPages"} style={query.focus === "myPages" ? "black" : "grey"} />
              </QueryLink>
            )}
            {tagFlags?.map((tagFlag) => (
              <QueryLink key={tagFlag._id} query={query.focus === tagFlag._id ? {} : { focus: tagFlag._id }}>
          ...
          {!loading &&
            tagsFiltered?.map((tag) => (
              <TagsDetailsItem key={tag._id} tag={tag} showFlags flagId={query.focus} collapse={collapsed} />
            ))}
