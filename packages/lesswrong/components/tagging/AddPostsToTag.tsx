...
const AddPostsToTag = ({ classes, tag }: { classes: ClassesType; tag: TagPreviewFragment }) => {
  ...
  return (
    <div className={classNames(classes.root, { [classes.open]: searchOpen })}>
      {!searchOpen && !isAwaiting && (
        <span onClick={() => setSearchOpen(true)} className={classes.addButton}>
          <AddBoxIcon className={classes.icon} /> {preferredHeadingCase("Add Posts")}
        </span>
      )}
      {searchOpen && (
        <div className={classes.search}>
          <InstantSearch indexName={getSearchIndexName("Posts")} searchClient={getSearchClient()}>
            ...
            <Configure facetFilters={formatFacetFilters({ tags: `-${tag._id}` })} hitsPerPage={10} />
            <Hits
              hitComponent={({ hit }: { hit: any }) => (
                <span className={classes.postHit} onClick={() => onPostSelected(hit._id)}>
