...
const TagEditsTimeBlock = ({
  ...
  let tagUpdatesInTimeBlock = [...(data?.TagUpdatesInTimeBlock || [])].sort((update1, update2) => {
  ...
  if (!data?.TagUpdatesInTimeBlock?.length) return null;
  return (
    <div>
      ...
      {tagUpdatesInTimeBlock.map((tagUpdates) => (
      ...
      {!expanded && tagUpdatesInTimeBlock.length >= INITIAL_LIMIT && (
        <LoadMore
