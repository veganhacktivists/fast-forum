...
const TagActivityFeed = ({ classes, pageSize = 50 }: { classes: ClassesType; pageSize?: number }) => {
  const { SingleColumnSection, MixedTypeFeed, TagRevisionItem, CommentsNode, NewTagItem, SectionTitle } = Components;

  return (
    <SingleColumnSection>
      <SectionTitle
        title={`Recent ${taggingNameIsSet.get() ? taggingNameCapitalSetting.get() : "Tag & Wiki"} Activity`}
      />
      <MixedTypeFeed
        pageSize={pageSize}
        ...
        renderers={{
          tagCreated: {
            ...
            render: (tag: TagCreationHistoryFragment) => <NewTagItem tag={tag} />,
          },
          tagRevision: {
            fragmentName: "RevisionTagFragment",
            render: (revision: RevisionTagFragment) => (
              <div>
                {revision?.tag && (
                  <TagRevisionItem
                    tag={revision.tag}
