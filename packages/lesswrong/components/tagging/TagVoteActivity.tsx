...
const TagVoteActivityRow = ({ vote, classes }: { vote: TagVotingActivity; classes: ClassesType }) => {
  ...
  if (!vote.tagRel?.post || !vote.tagRel?.tag) return null;
  ...
  return (
    <tr key={vote._id} className={classes.voteRow}>
      <td>
        <UsersName documentId={vote.userId} />
      </td>
      <td className={classes.postCell}>
        {" "}
        <TagSmallPostLink hideMeta post={vote.tagRel?.post} />{" "}
      </td>
      ...
      <td className={classes.smallCell}>
        {vote.power} {vote.isUnvote && <span title="Unvote">(unv.)</span>}
      </td>
      <td className={classes.smallCell}>
      ...
      <td className={classes.votingCell}>
        <div className={classes.voteButtons}>
          <OverallVoteButton orientation="left" color="error" upOrDown="Downvote" enabled={canVote} {...voteProps} />
          <span className={classes.score}>{voteProps.baseScore}</span>
          <OverallVoteButton orientation="right" color="secondary" upOrDown="Upvote" enabled={canVote} {...voteProps} />
        </div>
      </td>
    </tr>
  );
