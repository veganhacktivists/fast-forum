  ...
  if (!currentUser) {
    return (
      <SingleColumnSection>
        ...
        <div>You must be logged in to define new {taggingNamePluralSetting.get()}.</div>
      </SingleColumnSection>
    );
  }
  ...
  if (!tagUserHasSufficientKarma(currentUser, "new")) {
    return (
      <SingleColumnSection>
        <SectionTitle title={`New ${taggingNameCapitalSetting.get()}`} />
        <div>
          You do not have enough karma to define new {taggingNamePluralSetting.get()}. You must have at least{" "}
          {tagMinimumKarmaPermissions.new} karma.
        </div>
      </SingleColumnSection>
    );
  }
