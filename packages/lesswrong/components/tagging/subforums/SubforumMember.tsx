...
const styles = (theme: ThemeType): JssStyles => ({
  ...
  profileImage: {
    ...
    "box-shadow": "3px 3px 1px " + theme.palette.boxShadowColor(0.25),
    "-webkit-box-shadow": "0px 0px 2px 0px " + theme.palette.boxShadowColor(0.25),
    "-moz-box-shadow": "3px 3px 1px " + theme.palette.boxShadowColor(0.25),
    borderRadius: "50%",
  },
  name: {
    marginBottom: 4,
...
const SubforumMember = ({
  ...
  const { ProfilePhoto, ContentStyles, ContentItemBody, Typography, SocialMediaLink } = Components;

  const userHasSocialMedia = Object.keys(SOCIAL_MEDIA_PROFILE_FIELDS).some(
    (field: keyof typeof SOCIAL_MEDIA_PROFILE_FIELDS) => user[field],
  );

  const userKarma = user.karma || 0;
  const bioNode = (user.biography?.html || user.howICanHelpOthers?.html) && (
    <>
      <div
      ...
      {meritsCollapse && (
        <button
          className={classNames(classes.showMoreButton, { [classes.showLess]: !collapsed })}
      ...
      <div>
        ...
        {isOrganizer && (
          <div className={classes.userTags}>
            <div className={classes.userTag}>Organizer</div>
          </div>
        )}
        ...
        {(user.mapLocation || userHasSocialMedia) && (
          <div className={classes.locationRow}>
            ...
            {userHasSocialMedia && (
              <div className={classes.socialMediaIcons}>
                {Object.keys(SOCIAL_MEDIA_PROFILE_FIELDS).map((field: SocialMediaProfileField) => (
                  <SocialMediaLink key={field} user={user} field={field} className={classes.socialMediaIcon} />
                ))}
              </div>
            )}
          </div>
...
const SubforumMemberComponent = registerComponent("SubforumMember", SubforumMember, { styles });
