  ...
  return (
    <div className={classes.root}>
      <div className={classes.metadata} onClick={(ev) => setExpanded(!expanded)}>
        ...
        {lastRevisedAt ? (
          <LWTooltip placement="right" title={<ExpandedDate date={lastRevisedAt} />}>
            <PostsItem2MetaInfo className={classes.postedAt}>
              {" "}
              {moment(new Date(lastRevisedAt)).fromNow()}{" "}
            </PostsItem2MetaInfo>
          </LWTooltip>
        ) : null}
        ...
        {(changeMetrics.added > 0 || changeMetrics.removed > 0) && (
          <div className={classes.changeMetrics}>
            <ChangeMetricsDisplay changeMetrics={changeMetrics} />
          </div>
        )}
        {!!commentCount && commentCount > 0 && (
          <Link to={tagGetDiscussionUrl(tag)} className={classes.commentBubble}>
            <PostsItemComments
              small={true}
      ...
      {expanded && (
        <div className={classes.expandedBody}>
          {revisionIds.length > 0 && (
            <Link to={tagGetHistoryUrl(tag)} className={classes.history}>
          ...
          {revisionIds.length > 0 && (
            <Link to={`revisions/tag/${tag.slug}`} className={classes.subheading}>
              Edits
            </Link>
          )}
          {revisionIds.map((revId) => (
            <div className={classes.tagRevision} key={revId}>
              <AllPostsPageTagRevisionItem tag={tag} documentId={tag._id} revisionId={revId} />
            </div>
          ))}

          {commentIds && commentIds.length > 0 && (
            <Link to={tagGetDiscussionUrl(tag)} className={classes.subheading}>
              Comment Threads
            </Link>
          )}
          {commentIds &&
