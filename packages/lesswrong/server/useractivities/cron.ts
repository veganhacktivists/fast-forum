...
async function assertTableIntegrity(dataDb: SqlClient) {
  ...
  if (dateCombinations.length > 1) {
    ...
    for (let i = 0; i < dateCombinations.length; i++) {
      ...
      console.error(
        `(${i === 0 ? "correct" : "incorrect"}) startDate: ${dateCombo.startDate}, endDate: ${dateCombo.endDate}, count: ${dateCombo.count}`,
      );
      ...
      const userIds = (
        ...
        LIMIT 10;
      `,
          [dateCombo.startDate, dateCombo.endDate],
        )
...
async function concatNewActivity({
  ...
  const cleanedActivityData = newActivityData.filter(
    ({ userOrClientId }) => userOrClientId.length <= 17 || !userOrClientId.match(/^[A-Za-z0-9]+$/),
  );
  ...
  const paddedStartDate = max([
    prevStartDate,
    new Date(updateEndDate.getTime() - ACTIVITY_WINDOW_HOURS * 60 * 60 * 1000),
  ]) as Date;
  ...
  const newUsersData = cleanedActivityData
    ...
    .map(({ userOrClientId, activityArray: activity_array }) => {
      ...
      const zeroPaddingLength =
        Math.floor((updateEndDate.getTime() - paddedStartDate.getTime()) / (60 * 60 * 1000)) - activity_array.length;
  ...
  if (newUsersData.length > 0) {
    ...
    console.log(
      `Inserting ${newUsersData.length} new rows into UserActivities table, in ${newUsersDataChunked.length} chunks`,
    );

    for (const dataChunk of newUsersDataChunked) {
      const placeholders = dataChunk
        .map(
          (_, index) =>
            `($${index * 5 + 1}, $${index * 5 + 2}, $${index * 5 + 3}, $${index * 5 + 4}, $${index * 5 + 5}, '${visitorIdType}')`,
        )
        .join(", ");
      const insertQuery = `
  ...
  const existingUsersData = cleanedActivityData.filter(({ userOrClientId }) =>
    existingUserIds.includes(userOrClientId),
  );
  if (existingUsersData.length > 0) {
    const existingUsersDataChunked = chunk(1000, existingUsersData);
    console.log(
      `Updating ${existingUsersData.length} existing rows for which there is new activity, in ${existingUsersDataChunked.length} chunks`,
    );

    for (const dataChunk of existingUsersDataChunked) {
      const tempTableValues = dataChunk
        .map(
          ({ userOrClientId, activityArray: activity_array }) =>
            `('${userOrClientId}', ARRAY[${activity_array.join(", ")}])`,
        )
        .join(", ");
      const updateQuery = `
  ...
  const inactiveExistingUserIds = existingUserIds.filter(
    (id) => !existingUsersData.some(({ userOrClientId }) => userOrClientId === id),
  );
  if (inactiveExistingUserIds.length > 0) {
    ...
    console.log(
      `Adding zero-padding to ${inactiveExistingUserIds.length} existing rows for which there is no new activity, in ${inactiveExistingUserIdsChunked.length} chunks`,
    );
...
export async function updateUserActivities(props?: {
  ...
  if (props?.randomWait) {
    ...
    console.log(
      `Sleeping for ${sleepTime.toFixed(0)}ms before updating user activity to avoid multiple jobs running at the same time`,
    );
  ...
  console.log(
    `Updating user activity for ${newActivityData.length} users between ${updateStartDate} and ${updateEndDate}`,
  );
  ...
  await concatNewActivity({
    dataDb,
    newActivityData: userActivityData,
    prevStartDate: prevStartDate ?? updateStartDate,
    updateStartDate,
    updateEndDate,
    visitorIdType: "userId",
  });
  await concatNewActivity({
    dataDb,
    newActivityData: clientActivityData,
    prevStartDate: prevStartDate ?? updateStartDate,
    updateStartDate,
    updateEndDate,
    visitorIdType: "clientId",
  });
}
