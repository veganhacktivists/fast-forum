...
export default class MongoStore extends session.Store {
  ...
  constructor({
    ...
    // Check params
    assert(!!options.collection, "You must provide either mongoUrl|clientPromise|client in options");
    assert(
   ...
   */
  async set(sid: string, session: session.SessionData, callback: (err: any) => void = noop): Promise<void> {
    try {
      ...
      const lastModified = this.options.touchAfter > 0 ? new Date() : null;
  ...
  async touch(
    ...
    try {
      ...
      const rawResp = await this.collection.rawUpdateOne({ _id: sid }, { $set: updateFields });
      if (rawResp === 0) {
