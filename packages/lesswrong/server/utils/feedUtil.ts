...
export function fixedResultSubquery<ResultType extends DbObject, SortKeyType>({
  type,
  result,
  sortKey,
}: {
...
export function fixedIndexSubquery<ResultType extends DbObject>({
  type,
  index,
  result,
}: {
...
export async function mergeFeedQueries<SortKeyType>({
  limit,
  cutoff,
  offset,
  sortDirection,
  ...
  const [numericallyPositionedResults, orderedResults] = _.partition(
    unsortedResults,
  ...
  // Apply cutoff
  const withCutoffApplied = cutoff ? applyCutoff(sortedResults, cutoff, sortDirection) : sortedResults;

  // Merge in the numerically positioned results
  const bothResultKinds = mergeSortedAndNumericallyPositionedResults(
    withCutoffApplied,
  ...
  const nextCutoff =
    nonNumericallyPositionedResults.length > 0
