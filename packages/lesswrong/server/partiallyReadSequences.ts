...
const updateSequenceReadStatusForPostRead = async (
  ...
  const collection = sequence?.canonicalCollectionSlug
    ? await Collections.findOne({ slug: sequence.canonicalCollectionSlug })
    : null;
  const now = new Date();

  const partiallyReadMinusThis =
    user.partiallyReadSequences?.filter(
      (partiallyRead) =>
        partiallyRead.sequenceId !== sequenceId && (!collection || partiallyRead.collectionId !== collection._id),
    ) || [];
...
const userHasPartiallyReadSequence = (user: DbUser, sequenceId: string): boolean => {
  if (!user.partiallyReadSequences) return false;
  return _.some(user.partiallyReadSequences, (s) => s.sequenceId === sequenceId);
};

getCollectionHooks("LWEvents").createAsync.add(async function EventUpdatePartialReadStatusCallback(
  props: CreateCallbackProperties<"LWEvents">,
) {
...
const postsToReadStatuses = async (user: DbUser, postIds: Array<string>) => {
  ...
  for (const postID of postIds) resultDict[postID] = false;
  for (const readPostId of readPostIds) resultDict[readPostId] = true;
  return resultDict;
};
...
addGraphQLResolvers({
  Mutation: {
    async updateContinueReading(
      root: void,
      { sequenceId, postId }: { sequenceId: string; postId: string },
      context: ResolverContext,
    ) {
