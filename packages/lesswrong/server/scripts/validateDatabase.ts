...
type CollectionCustomValidatorFunction<T extends DbObject> = (
  documents: T[],
...
export function registerCollectionValidator<N extends CollectionNameString>({
  collection,
  name,
  validateBatch,
}: {
  ...
  if (!(collection.collectionName in customValidators)) customValidators[collection.collectionName] = [];
...
export async function validateCollection(collection: AnyBecauseTodo) {
  ...
  const rowsWithObjectID = await collection
    .find({
      _id: { $type: "objectId" },
    })
    .count();
  ...
  function recordError(field: AnyBecauseTodo, errorType: AnyBecauseTodo) {
    ...
    if (!errorsByField[fieldGroupedByNums]) errorsByField[fieldGroupedByNums] = {};
    if (!errorsByField[fieldGroupedByNums][errorType]) errorsByField[fieldGroupedByNums][errorType] = 0;
        ...
        if (foreignKeySpec) {
          ...
          if (typeof foreignKeySpec === "string") {
            ...
            if (typeof foreignField !== "string")
              throw new Error(
                `Expected a field name in foreignKey constraint for ${collectionName}.${fieldName}, value wasn't a string`,
              );
            if (typeof foreignCollectionName !== "string")
              throw new Error(
                `Expected a collection name in foreignKey constraint for ${collectionName}.${fieldName}, value wasn't a string`,
              );
          }
          ...
          const foreignRows = await foreignCollection
            .find({ [foreignField]: { $in: foreignValues } }, { [foreignField]: 1 })
            .fetch();

          // Collect a list of values present
          const foreignValuesFound = {};
          for (const foreignRow of foreignRows) (foreignValuesFound as AnyBecauseTodo)[foreignRow[foreignField]] = true;

          // Compare against values referred to, and report an error for any missing
          for (const document of batch) {
            if (
              (document as AnyBecauseTodo)[fieldName] &&
              !((document as AnyBecauseTodo)[fieldName] in foreignValuesFound)
            ) {
              recordError(
                fieldName,
                `foreignKeyViolation: from ${document._id} to ${(document as AnyBecauseTodo)[fieldName]}`,
              );
            }
          }
        }
