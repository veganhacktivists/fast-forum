...
async function recalculateReceivedVoteCounts() {
  await migrateDocuments({
    ...
    migrate: async (users) => {
      ...
      const batchUserVoteCounts = filteredUserVotes.reduce(
        (agg, vote) => {
      ...
      if (batchUpdates.length > 0) {
        ...
        console.log(
          `Updating batch of ${batchUpdates.length} users, _id of first user is ${batchUpdates[0].updateOne.filter._id}`,
        );
        await Users.rawCollection().bulkWrite(batchUpdates, { ordered: false });
      }
    },
  });
}
