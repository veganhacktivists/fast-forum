  ...
  void (async () => {
    ...
    await asyncForeachSequential(allUsers, async (user) => {
      if (user.legacy) {
        ...
        const mainPostKarma =
          upvoteWeight * f(user.legacyData.karma_ups_link_lesswrong) -
        ...
        // @ts-ignore for legacyData
        const discussionPostKarma =
          upvoteWeight * f(user.legacyData.karma_ups_link_discussion) -
          // @ts-ignore for legacyData
          downvoteWeight * f(user.legacyData.karma_downs_link_discussion);

        // @ts-ignore for legacyData
        const discussionCommentKarma =
          upvoteWeight * f(user.legacyData.karma_ups_comment_discussion) -
          // @ts-ignore for legacyData
          downvoteWeight * f(user.legacyData.karma_downs_comment_discussion);

        const karma =
          mainPostKarmaWeight * mainPostKarma +
