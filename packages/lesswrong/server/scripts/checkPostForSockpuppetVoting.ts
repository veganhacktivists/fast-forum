...
const checkForDuplicateClientIds = async (voterIds: string[]): Promise<string[]> => {
  ...
  const clientIds = await ClientIds.find(
    ...
    { projection: { clientId: 1, userIds: 1 } },
  ).fetch();
...
const checkForDuplicateIPs = async (voterIds: string[]): Promise<Record<string, string[]>> => {
  ...
  for (let voterId of voterIds) {
    ...
    const loginEvents = await LWEvents.find(
      ...
      { limit: 5, sort: { createdAt: -1 }, projection: { properties: 1 } },
    ).fetch();
  ...
  Object.keys(ipUsers)
    .filter((ip) => ipUsers[ip].length > 1)
    .forEach((ip) => (suspiciousIps[ip] = ipUsers[ip]));
...
const checkPostForSockpuppetVoting = async (postId: string, voteDirection: "up" | "down" = "down", verbose = false) => {
  ...
  console.log(
    `Checking for duplicate client IDs within comment ${voteDirection}voters (${comments.length ?? 0} comments total)...`,
  );
  for (let comment of comments) {
    ...
    if (verbose)
      console.log(
        `Client IDs associated with multiple ${voteDirection}voters of comment ${comment._id}:`,
        voterSuspiciousClientIds,
      );
    if (voterSuspiciousClientIds.length) commentsSuspiciousClientIds[comment._id] = voterSuspiciousClientIds;
  ...
  if (Object.keys(postVoterSuspiciousIps).length) {
    console.log(
      `IPs associated with multiple post ${voteDirection}voters (with their associated userIds):`,
      postVoterSuspiciousIps,
    );
  } else {
  ...
  console.log(
    ...
    commentsSuspiciousClientIds,
  );
};

Vulcan.checkPostForSockpuppetVoting = wrapVulcanAsyncScript(
