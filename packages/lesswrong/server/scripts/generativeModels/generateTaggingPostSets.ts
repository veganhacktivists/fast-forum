...
import {
  ...
  checkTags,
  getAutoAppliedTags,
  generatePostBodyCache,
  PostBodyCache,
} from "../../languageModels/autoTagCallbacks";
...
function weightedPartition<T>(list: T[], weights: number[]): T[][] {
  ...
  for (let i = 0; i < oddItems; i++) groupSizes[i]++;
...
const frontpagePrompt =
  "Is this post of broad relevance, timeless, apolitical, and aiming to explain rather than persuade?";

async function generateClassifierTuningFile({
  description,
  posts,
  postBodyCache,
  outputFilename,
...
Globals.generateTagClassifierData = async (args: {
  ...
  const {
    tagSlug,
    trainingSetFilename = "ml/tagClassificationPostIds.train.json",
    testSetFilename = "ml/tagClassificationPostIds.test.json",
  } = args || {};
  ...
  const tags: DbTag[] = tagSlug ? [singleTag!] : await getAutoAppliedTags();
  ...
  for (let tag of tags) {
    ...
    await generateClassifierTuningFile({
      ...
      classifyPost: (post: DbPost) =>
        post.tagRelevance && tag._id in post.tagRelevance && post.tagRelevance[tag._id] > 0,
    });
    await generateClassifierTuningFile({
    ...
    try {
      ...
      writeResult(
        `${post.title}\n` +
          ...
          `    Language model: ${filter(tags, (t) => !!tagsPredicted[t.slug])
            .map((t) => t.name)
            .join(", ")}\n` +
...
Globals.evaluateFrontPageClassifier = async (testSetPostIdsFilename: string, outputFilename: string) => {
  ...
  const sb: string[] = [];
  const agreements = filter(
    testSetPostIds,
    (postId) => languageModelFrontpagedIt[postId] === humanFrontpagedIt[postId],
  );
  const onlyHumanFrontpaged = filter(
    testSetPostIds,
    (postId) => !languageModelFrontpagedIt[postId] && humanFrontpagedIt[postId],
  );
  const onlyLMFrontpaged = filter(
    testSetPostIds,
    (postId) => languageModelFrontpagedIt[postId] && !humanFrontpagedIt[postId],
  );
