...
const generateMigration = async ({
  ...
  const diff: string = (await execRun(
    `git diff --no-index ${acceptedSchemaFile} ${toAcceptSchemaFile} --unified=1`,
  )) as string;
...
export const makeMigrations = async ({
  ...
  const {
    acceptsSchemaHash: acceptedHash,
    acceptedByMigration,
    timestamp,
  } = await acceptMigrations({ write: writeSchemaChangelog, rootPath });
  ...
  const collectionNames: CollectionNameString[] = getAllCollections()
