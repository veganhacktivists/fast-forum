...
function imageIsOffsite(imageUrl: string) {
  ...
  for (let i = 0; i < whitelistedImageHosts.length; i++) {
    if (hostname === whitelistedImageHosts[i]) return false;
  }
...
const checkPost = async (post: DbPost) => {
  ...
  for (let i = 0; i < images.length; i++) {
    ...
    if (await urlIsBroken(new URL(imageUrl, baseUrl).toString())) brokenImages.push(imageUrl);
    else if (imageIsOffsite(imageUrl)) offsiteImages.push(imageUrl);
  }
  for (let i = 0; i < links.length; i++) {
    let linkUrl = links[i];
    if (await urlIsBroken(new URL(linkUrl, baseUrl).toString())) brokenLinks.push(linkUrl);
  }

  if (brokenImages.length > 0 || offsiteImages.length > 0 || brokenLinks.length > 0) {
    let sb: Array<string> = [];
    sb.push((await describePost(post)) + "\n");
    for (let i = 0; i < brokenImages.length; i++) sb.push(`    Broken image: ${brokenImages[i]}\n`);
    for (let i = 0; i < brokenLinks.length; i++) sb.push(`    Broken link: ${brokenLinks[i]}\n`);
    for (let i = 0; i < offsiteImages.length; i++) sb.push(`    Offsite image: ${offsiteImages[i]}\n`);
    return sb.join("");
  } else {
    return null;
  }
