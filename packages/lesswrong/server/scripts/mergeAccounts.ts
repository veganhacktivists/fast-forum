...
const transferCollection = async ({
  ...
  try {
    ...
    if (!dryRun) {
      ...
      // eslint-disable-next-line no-console
      console.log(
        `Final target user ${targetUserId} ${collectionName} count: ${finalTargetUserCount} (compare ${sourceUserCount + targetUserCount})`,
      );
    }
  } catch (err) {
    // eslint-disable-next-line no-console
...
const mergeReadStatus = async ({
  ...
  const sourceMostRecentlyUpdated =
    sourceUserStatus && targetUserStatus
      ? new Date(sourceUserStatus.lastUpdated) > new Date(targetUserStatus.lastUpdated)
      : !!sourceUserStatus;
...
Vulcan.mergeAccounts = async ({
  ...
  try {
    if (!dryRun) {
      ...
      const newOldSlugs = [...(targetUser.oldSlugs || []), ...(sourceUser.oldSlugs || []), sourceUser.slug];
      // eslint-disable-next-line no-console
