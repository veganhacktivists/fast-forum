...
const generateInflationAdjustedKarmaReport = async ({
  ...
  const relativeToIdx = Math.floor(
    (new Date(relativeTo).getTime() - karmaInflationSeries.start) / karmaInflationSeries.interval,
  );
  const adjustment =
    karmaInflationSeries.values[relativeToIdx] || karmaInflationSeries.values[karmaInflationSeries.values.length - 1];
  ...
  const fileRows = results
    .map(
      (post: AnyBecauseHard) =>
        `${post._id},${post.postedAt.toISOString()},${post.slug},https://forum.effectivealtruism.org${postGetPageUrl(post)},${post.baseScore},${post.karmaInflationAdjustedScore / adjustment},${post.wordCount}`,
    )
    .join("\n");
