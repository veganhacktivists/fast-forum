 ...
 */
Vulcan.ensureEmailInEmails = wrapVulcanAsyncScript("ensureEmailInEmails", async () => {
  ...
  for (const user of await usersWithEmail.fetch()) {
    ...
    if (
      user.email &&
      user.emails &&
      user.emails.map((email) => email.address?.toLowerCase()).includes(user.email.toLowerCase())
    ) {
      continue;
    }
