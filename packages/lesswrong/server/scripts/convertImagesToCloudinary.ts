...
function getImageUrlWhitelist() {
  ...
  return ["cloudinary.com", "res.cloudinary.com", "www.lesswrong.com", "www.alignmentforum.org"].concat(
    localUploadUrl ? [new URL(localUploadUrl).host] : [],
  );
}
...
function rewriteSrcset(srcset: string, urlMap: Record<string, string>): string {
  ...
  const rewrittenImageVariants = imageVariants.map((variant) => {
    ...
    if (tokens[0] in urlMap) tokens[0] = urlMap[tokens[0]];
    return tokens.join(" ");
  });
