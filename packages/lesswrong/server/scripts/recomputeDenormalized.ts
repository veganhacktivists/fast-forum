...
export const recomputeDenormalizedValues = async <N extends CollectionNameString>({
  collectionName,
  fieldName = null,
  validateOnly = false,
  projection,
  ...
  if (fieldName) {
    ...
    if (denormalizedFields.length === 0) {
      // eslint-disable-next-line no-console
      console.log(
        `${collectionName} does not have any fields with "canAutoDenormalize", not computing denormalized values`,
      );
      return;
    }
    ...
    migrate: async (documents) => {
      ...
      const updates = await Promise.all(
        documents.map(async (doc) => {
          ...
          if ((isNullOrDefined(newValue) && isNullOrDefined(doc[fieldName])) || doc[fieldName] === newValue)
            return null;
      ...
      if (!validateOnly) {
        if (nonEmptyUpdates.length > 0) {
          await collection.rawCollection().bulkWrite(nonEmptyUpdates, { ordered: false });
        }
      } else {
