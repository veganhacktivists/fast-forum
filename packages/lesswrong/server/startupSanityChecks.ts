...
onStartup(async () => {
  ...
  if (expectedDatabaseId || databaseId) {
    if (expectedDatabaseId !== databaseId) {
      ...
      console.error(
        "If you are connecting to a production DB, you must use a matching config file. If you are *not* connecting to a production DB, you should not use a production config file.",
