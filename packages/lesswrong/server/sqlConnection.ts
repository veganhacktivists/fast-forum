...
const getVectorTypeOid = async (client: IClient): Promise<number | null> => {
  const result: IResult<{ oid: number }> = await client.query("SELECT oid FROM pg_type WHERE typname = 'vector'");
...
const queryMethods = ["none", "one", "oneOrNone", "many", "manyOrNone", "any", "multi"] as const;
...
declare global {
  ...
  type SqlClient = Omit<RawSqlClient, (typeof queryMethods)[number]> & {
    ...
    none: (query: string, values?: SqlQueryArgs, describe?: SqlDescription, quiet?: boolean) => Promise<null>;
    one: <T = any>(query: string, values?: SqlQueryArgs, describe?: SqlDescription, quiet?: boolean) => Promise<T>;
    oneOrNone: <T = any>(
      query: string,
      values?: SqlQueryArgs,
    ...
    many: <T = any>(query: string, values?: SqlQueryArgs, describe?: SqlDescription, quiet?: boolean) => Promise<T[]>;
    manyOrNone: <T = any>(
      query: string,
      values?: SqlQueryArgs,
      describe?: SqlDescription,
    ...
    any: <T = any>(query: string, values?: SqlQueryArgs, describe?: SqlDescription, quiet?: boolean) => Promise<T[]>;
    multi: <T = any>(
      query: string,
      values?: SqlQueryArgs,
      describe?: SqlDescription,
