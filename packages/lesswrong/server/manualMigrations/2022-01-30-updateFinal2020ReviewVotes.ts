  ...
  action: async () => {
    ...
    const finalPosts: DbPost[] = isLW
      ? await Posts.find(
          ...
          { sort: { finalReviewVoteScoreHighKarma: -1 } },
        ).fetch()
      : await Posts.aggregate([
          { $match: { reviewCount: { $gt: 0 }, finalReviewVoteScoreHighKarma: { $exists: true } } },
          {
            $project: {
              ...
              combinedScore: {
                $sum: [{ $multiply: ["$finalReviewVoteScoreHighKarma", 2] }, "$finalReviewVoteScoreAllKarma"],
              },
            },
          },
    ...
    const donateButton = (post: DbPost) =>
      isLW
        ? `<td><form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_blank" style="text-align: center">
    ...
    const postTableRow = (post: DbPost, i: number) => `<tr>
        ...
        <td>
          <div class="title-row">
            ...
            <div class="dots-row">
              ${(post.finalReviewVotesHighKarma || [])
                .sort((x, y) => x - y)
                .map((vote) => voteDot(vote))
                .join("")}
            </div>
          </div>
        </td>
