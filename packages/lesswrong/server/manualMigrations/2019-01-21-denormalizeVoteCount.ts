    ...
    for (let collection of VoteableCollections) {
      await migrateDocuments({
        ...
        migrate: async (documents) => {
          ...
          for (let documentId of documentIds) votesByDocument[documentId] = [];
          for (let vote of votes) votesByDocument[vote.documentId].push(vote);

          // Write vote counts
          let updatesByDocument = mapValues(votesByDocument, (votes, documentID) => ({
            updateOne: {
