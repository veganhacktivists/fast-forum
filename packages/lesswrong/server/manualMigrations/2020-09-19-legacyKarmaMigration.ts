      ...
      if (!!authorIds) {
        for (const authorId of authorIds) {
          ...
          if (authorKarma !== undefined && authorAfKarma !== undefined) {
            ...
            if (doesVoteIncreaseKarma(vote, authorId) && documentIsAf)
              userAfKarmaMap.set(authorId, authorAfKarma + afVotePower);
          }
        }
      }
...
const findInvalidVotes = (allVotes: DbVote[], userKarmaMap: Map<string, number>) => {
  return allVotes.flatMap(({ _id, authorIds, userId, cancelled }) => {
    if (!authorIds) return [];
    for (const authorId of authorIds) if (!cancelled && userKarmaMap.get(authorId) === undefined) return [_id];
