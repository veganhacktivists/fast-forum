    ...
    await forEachDocumentBatchInCollection({
      ...
      callback: (users: DbUser[]) => {
        ...
        for (let user of users) {
          ...
          if (!user || !user.legacy || !(user as any).legacyData) continue;
          ...
          if (user.whenConfirmationEmailSent) continue;
          ...
          for (let i = 0; i < user.emails.length; i++) {
            if (legacyData.email && user.emails && user.emails[i].address === legacyData.email) {
