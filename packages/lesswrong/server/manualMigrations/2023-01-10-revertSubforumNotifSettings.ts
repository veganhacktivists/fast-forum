...
registerMigration({
  ...
  action: async () => {
    ...
    if (matchingSubscriptionIds.length < 25) {
      // I happen to know there are only 11, so it's useful to have this in the logs
     ...
     */
    const userEditedTagRels = await LWEvents.find({
      name: "fieldChanges",
      $or: [
        { "properties.before.subforumEmailNotifications": { $exists: true } },
        { "properties.after.subforumEmailNotifications": { $exists: true } },
      ],
    }).fetch();
    const userEditedTagRelIds = [...new Set(userEditedTagRels.map((rel) => rel.documentId))];
    console.log(
      `Found ${userEditedTagRelIds.length} UserTagRels to ignore (because they have been edited by the user)`,
    );
    if (userEditedTagRelIds.length < 25) {
      // I happen to know there are only 11, so it's useful to have this in the logs
    ...
    await UserTagRels.rawUpdateMany(
      { _id: { $nin: userEditedTagRelIds }, subforumEmailNotifications: { $ne: defaultValue } },
      { $set: { subforumEmailNotifications: defaultValue } },
    );
  },
});
