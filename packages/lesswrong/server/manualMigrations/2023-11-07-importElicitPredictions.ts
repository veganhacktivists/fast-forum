...
import {
  ElicitPredictionData,
  getPredictionDataFromElicit,
  getPredictionsFromElicit,
} from "../resolvers/elicitPredictions";
...
const apiPredictionToDBQuestion = (
  prediction: ElicitPredictionData,
  questionId: string,
): Omit<DbElicitQuestionPrediction, "schemaVersion"> => ({
  _id: prediction.id,
...
registerMigration({
  ...
  action: async () => {
    ...
    const questionIds = Array.from(
      new Set(
        [...postsWithPredictions, ...commentsWithPredictions]
          .map((d) => cheerioParse(d.contents.html))
          .map(($) =>
            $("[data-elicit-id]")
              .toArray()
              .map((elt) => $(elt).attr("data-elicit-id")),
          )
          .map(filterNonnull)
          .flat(),
      ),
    ...
    const elicitQuestionsWithPredictions = await executePromiseQueue(
      questionIds.map(
        (id) => () =>
          Promise.all([
            getPredictionDataFromElicit(id)
              .then((q) => apiQuestionToDBQuestion(q, id))
              .catch(() => null),
            getPredictionsFromElicit(id)
              .then((ps) => (ps ?? []).map((p) => apiPredictionToDBQuestion(p, id)))
              .catch(() => []),
          ]),
      ),
      10,
    ...
    await Promise.all(
      elicitQuestionsWithPredictions.flatMap(([questionData, predictionData]) => {
