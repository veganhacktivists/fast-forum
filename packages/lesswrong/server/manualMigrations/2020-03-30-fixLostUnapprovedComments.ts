...
registerMigration({
  ...
  action: async () => {
    ...
    for (let author of authors) authorsById[author._id] = author;

    const commentsToMarkReviewed = _.filter(unreviewedComments, (comment) => !authorsById[comment.userId].banned).map(
      (comment) => comment._id,
    );
    ...
    await Comments.rawUpdateMany(
      { _id: { $in: commentsToMarkReviewed } },
      { $set: { authorIsUnreviewed: false } },
      { multi: true },
    );
  },
});
