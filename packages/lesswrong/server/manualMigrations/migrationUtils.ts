...
export function registerMigration({ name, dateWritten, idempotent, action }: RegisterMigrationProps) {
  ...
  if (!dateWritten) throw new Error(`Migration ${name} is missing required field: dateWritten`);
  if (!action) throw new Error(`Migration ${name} is missing required field: action`);
  ...
  if (!idempotent) {
    throw new Error(
      `Migration ${name} is not marked as idempotent; it can't use registerMigration unless it's marked as (and is) idempotent.`,
    );
  }
...
export async function migrateDocuments<N extends CollectionNameString>({
  ...
  if (!description) description = "Migration on " + collection.collectionName;
  ...
  while (!done) {
    await runThenSleep(loadFactor, async () => {
      let documents = await collection
        .find(unmigratedDocumentQuery, { limit: batchSize, ...(projection ? { projection } : {}) })
        .fetch();
...
export async function forEachDocumentBatchInCollection<N extends CollectionNameString>({
  ...
  while (rows.length > 0) {
    await runThenSleep(loadFactor, async () => {
      ...
      rows = await timedFunc("getNext", () =>
        getNext({ collection, batchSize, filter, projection, lastRows: rows, overrideCreatedAt }),
      );
    });
  }
}

export async function forEachDocumentInCollection({
  collection,
  batchSize = 1000,
  filter = null,
  callback,
...
export async function forEachBucketRangeInCollection<N extends CollectionNameString>({
  ...
  const bucketBoundaries = await collection
    .aggregate([
      ...maybeFilter,
