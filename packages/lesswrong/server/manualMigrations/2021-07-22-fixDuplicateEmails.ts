  ...
  action: async () => {
    ...
    */
    ...
    mergeResults.forEach((mergeResult) => {
      const actionRow =
        mergeResult.action.type === "ManualMergeAction"
          ? ["ManualNeeded", mergeResult.action.sourceIds.join("^")].join(",")
          : [
              mergeResult.action.destinationId,
              mergeResult.action.sourceIds.join("^"),
              mergeResult.action.justification,
            ].join(",");
...
async function runSingleMerge(merge: MergeResult) {
  if (merge.action.type === "ManualMergeAction") return;
