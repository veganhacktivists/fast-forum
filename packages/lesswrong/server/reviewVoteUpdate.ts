...
async function updateVoteTotals(
  ...
  votesByUserId: Dictionary<DbReviewVote[]>,
  ...
  for (let userId of Object.keys(votesByUserId)) {
    ...
    const filteredUserVotes =
      votePhase === "finalVote" ? userVotes.filter((vote) => postIds.includes(vote.postId)) : userVotes;
...
export async function createVotingPostHtml() {
  ...
  const postsHtml = posts
    .map((post, i) => {
      ...
      <td>
        <div class="dots-row">
          ${post.finalReviewVotesAllKarma
            .sort((v1, v2) => v2 - v1)
            .map((vote) => getDot(vote))
            .join("")}
        </div>
      </td>
    </tr>`;
