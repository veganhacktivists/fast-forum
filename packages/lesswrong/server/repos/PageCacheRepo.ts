...
class PageCacheRepo extends AbstractRepo<"PageCache"> {
  ...
  async lookupCacheEntry({
    path,
    completeAbTestGroups,
  }: {
    path: string;
    completeAbTestGroups: CompleteTestGroupAllocation;
  }): Promise<DbPageCacheEntry | null> {
  ...
  async upsertPageCacheEntry(
    path: string,
    abTestGroups: RelevantTestGroupAllocation,
    renderResult: SanitizedRenderResult,
  ): Promise<null> {
    const bundleHash = getServerBundleHash();
    const now = new Date();
