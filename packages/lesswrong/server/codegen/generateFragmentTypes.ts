...
function getParsedFragment(fragmentName: FragmentName) {
  ...
  if (
    !parsedFragment ||
    !("name" in parsedFragment) ||
    !("selectionSet" in parsedFragment) ||
...
function fragmentToInterface(
  ...
  for (let selection of parsedFragment.selectionSet.selections) {
    switch (selection.kind) {
      case "Field":
        ...
        if (subfragment) allSubfragments.push(subfragment);
        break;
      case "FragmentSpread":
        break;
      default:
  ...
  sb.push("}\n\n");
  for (let subfragment of allSubfragments) sb.push(subfragment);
  return sb.join("");
}
...
function getFragmentFieldType(
  ...
  if (!fieldType) {
    throw new Error(
      `Fragment ${fragmentName} contains field ${fieldName} on type ${collection.collectionName} which is not in the schema`,
    );
  }
  ...
  if (parsedFragmentField.selectionSet?.selections?.length > 0) {
    ...
    if (
      ...
      if (!subfieldCollection) {
        ...
        console.log(
          `Field ${fieldName} in fragment ${fragmentName} has type ${fieldType} which does not identify a collection`,
        );
