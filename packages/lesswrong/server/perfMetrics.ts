...
type IncompletePerfMetricProps = Pick<
  PerfMetric,
  | "op_type"
  | "op_name"
  | "parent_trace_id"
...
export function setAsyncStoreValue<T extends keyof AsyncLocalStorageContext>(
  key: T,
  value: AsyncLocalStorageContext[T] | ((previousValue: AsyncLocalStorageContext[T]) => AsyncLocalStorageContext[T]),
) {
...
export function addStartRenderTimeToPerfMetric() {
  setAsyncStoreValue("requestPerfMetric", (incompletePerfMetric) => {
    if (!incompletePerfMetric) {
      ...
      console.log(
        "Missing perf metric for the current request in the asyncLocalStorage context when trying to add start render time to it!",
      );
      return;
    }
...
export function closeRequestPerfMetric() {
  ...
  if (!store.requestPerfMetric) {
    // eslint-disable-next-line no-console
    console.log(
      "Missing perf metric for the current request in the asyncLocalStorage context when trying to close it!",
    );
    return;
  }
