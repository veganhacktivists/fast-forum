...
const crosspostResolvers = {
  Mutation: {
    ...
    unlinkCrossposter: async (_root: void, _args: {}, { req }: ResolverContext) => {
      ...
      if (foreignUserId) {
        ...
        await makeCrossSiteRequest("unlinkCrossposter", { token }, "Failed to unlink crossposting accounts");
        await Users.rawUpdateOne(
          { _id: localUserId },
          {
  ...
  Query: {
    getCrosspost: async (_root: void, { args }: { args: GetCrosspostRequest }) => {
      ...
      if (!promise) {
        promise = makeCrossSiteRequest("getCrosspost", args, "Failed to get crosspost");
