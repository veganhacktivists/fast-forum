...
import {
  ConnectCrossposterRequestValidator,
  ConnectCrossposterResponseValidator,
  CrosspostRequestValidator,
  CrosspostResponseValidator,
  CrosspostTokenResponseValidator,
  GetCrosspostRequestValidator,
  GetCrosspostResponseValidator,
  UnlinkCrossposterRequestValidator,
  UnlinkedCrossposterResponseValidator,
  UpdateCrosspostRequestValidator,
  UpdateCrosspostResponseValidator,
} from "./types";
...
export type ApiRoute =
  | ValidatedGetRoutes[ValidatedGetRouteName]["path"]
  | ValidatedPostRoutes[ValidatedPostRouteName]["path"];

export const addCrosspostRoutes = (app: Application) => {
  const addGetRoute = <RouteName extends ValidatedGetRouteName>(
    route: ValidatedGetRoutes[RouteName],
    callback: GetRouteOf<RouteName>,
  ) => {
    app.get(route.path, async (req, res) => {
      ...
      try {
        ...
        return res.status(200).send({ error: { message: e.message ?? "An unknown error occurred", code: errorCode } });
      }
      ...
      if (isLeft(decodedResponse)) {
        // eslint-disable-next-line no-console
        console.error("Invalid response body when making cross-site GET request", {
          response,
          errors: decodedResponse.left.flatMap((e) => e.context),
        });
        // Return a 200 to avoid this triggering a health check failure,
        // the error is still handled in makeCrossSiteRequest on the server making the request
  ...
  const addPostRoute = <RouteName extends ValidatedPostRouteName>(
    route: ValidatedPostRoutes[RouteName],
    callback: PostRouteOf<RouteName>,
  ) => {
