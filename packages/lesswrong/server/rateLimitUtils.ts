...
import {
  getModeratorRateLimit,
  getTimeframeForRateLimit,
  userHasActiveModeratorActionOfType,
} from "../lib/collections/moderatorActions/helpers";
import {
  EXEMPT_FROM_RATE_LIMITS,
  RATE_LIMIT_THREE_COMMENTS_PER_POST_PER_WEEK,
...
import type {
  ...
  RecentVoteInfo,
  UserRateLimit,
} from "../lib/rateLimits/types";
import {
  calculateRecentKarmaInfo,
  documentOnlyHasSelfVote,
  getAutoRateLimitInfo,
  getCurrentAndPreviousUserKarmaInfo,
  getMaxAutoLimitHours,
  getModRateLimitInfo,
  getRateLimitStrictnessComparisons,
  getStrictestRateLimitInfo,
  getManualRateLimitInfo,
  getManualRateLimitIntervalHours,
  shouldIgnorePostRateLimit,
...
async function getPostsInTimeframe(user: DbUser, maxHours: number) {
  return await Posts.find(
    ...
    { sort: { postedAt: -1 }, projection: { postedAt: 1 } },
  ).fetch();
}
...
function getPostRateLimitInfos(
  ...
  const autoRateLimitInfos =
    autoRatelimits?.map((rateLimit) => getAutoRateLimitInfo(user, features, rateLimit, postsInTimeframe)) ?? [];
  ...
  return [modRateLimitInfo, userPostRateLimitInfo, ...autoRateLimitInfos].filter(
    (rateLimit): rateLimit is RateLimitInfo => rateLimit !== null,
  );
}
...
async function getUserIsAuthor(userId: string, postId: string | null, context: ResolverContext): Promise<boolean> {
  ...
  const userIsNotCoauthor =
    !post.coauthorStatuses || post.coauthorStatuses.every((coauthorStatus) => coauthorStatus.userId !== userId);
  return !(userIsNotPrimaryAuthor && userIsNotCoauthor);
}

function getModPostSpecificRateLimitInfo(
...
async function getCommentsOnOthersPosts(comments: Array<DbComment>, userId: string) {
  ...
  const commentsOnNonauthorPosts = comments.filter(
    (comment) => comment.postId && postsNotAuthoredByCommenterIds.includes(comment.postId),
  );
  return commentsOnNonauthorPosts;
}

async function getCommentRateLimitInfos({
  commentsInTimeframe,
  user,
  modRateLimitHours,
  ...
  const manualRateLimitInfo = userIsAuthor
    ? null
    : getManualRateLimitInfo(manualCommentRateLimit, commentsOnOthersPostsInTimeframe);
  ...
  const autoRateLimitInfos =
    filteredAutoRateLimits?.map((rateLimit) => getAutoRateLimitInfo(user, features, rateLimit, commentsInTimeframe)) ??
    [];
  return [modGeneralRateLimitInfo, modSpecificPostRateLimitInfo, manualRateLimitInfo, ...autoRateLimitInfos].filter(
    (rateLimit): rateLimit is RateLimitInfo => rateLimit !== null,
  );
}
...
export async function rateLimitDateWhenUserNextAbleToComment(
  user: DbUser,
  postId: string | null,
  context: ResolverContext,
...
export async function checkForStricterRateLimits(userId: string, context: ResolverContext) {
  ...
  const { commentRateLimitComparison, postRateLimitComparison } =
    getRateLimitStrictnessComparisons(userKarmaInfoWindow);
