...
export function mjPagePromise(
  ...
  return new Promise((resolve, reject) => {
    ...
    const errorHandler = (
      ...
      sourceFormula: AnyBecauseTodo,
      sourceFormat: AnyBecauseTodo,
      errors: AnyBecauseTodo,
    ) => {
      // eslint-disable-next-line no-console
...
export const cheerioWrapAll = (toWrap: cheerio.Cheerio, wrapper: string, $: cheerio.Root) => {
  ...
  if (toWrap.length < 2 && ($ as any).wrap) {
    // wrap not defined in npm version,
  ...
  toWrap.each(function (k, v) {
    // in Cheerio, we update with the output.
    $(v).remove();
    section.append($(v));
  });
...
export const handleDialogueHtml = async (html: string): Promise<string> => {
  ...
  $(".dialogue-message").each((idx, element) => {
    ...
    if (userId && userDisplayNamesById[userId]) {
      $(element)
        .prepend(
          `<section class="dialogue-message-header CommentUserName-author UsersNameDisplay-noColor"><b></b></section>`,
        )
...
export const backfillDialogueMessageInputAttributes = async (html: string, postId: string) => {
  ...
  const usersWithDisplayNames = await Users.find({ _id: { $in: dialogueParticipantIds } }, undefined, {
    _id: 1,
    displayName: 1,
  }).fetch();
  const displayNamesById = Object.fromEntries(
    usersWithDisplayNames.map((user) => [user._id, user.displayName] as const),
  );
  ...
  const userIdsWithOrders: [number, UserIdAndDisplayName][] = dialogueParticipantIds.map((participantId, idx) => [
    idx + 1,
    { userId: participantId, displayName: displayNamesById[participantId] ?? "[Missing displayName]" },
  ]);
...
const isEmptyParagraphOrBreak = (elem: cheerio.Element) => {
  if (elem.type === "tag" && elem.name === "p") {
    ...
    if (elem.children?.length === 1 && elem.children[0]?.type === "text" && elem.children[0]?.data?.trim() === "")
      return true;
    return false;
  }
...
export async function dataToWordCount(data: AnyBecauseTodo, type: string) {
  ...
  try {
    ...
    const withoutFootnotes = markdown.split(/^1\. {2}\^\*\*\[\^\]\(#(.|\n)*/m)[0].split(/^\[\^1\]:.*/m)[0];
