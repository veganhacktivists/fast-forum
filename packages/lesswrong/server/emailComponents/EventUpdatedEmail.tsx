...
const EventUpdatedEmail = ({ postId, classes }: { postId: string; classes: any }) => {
  ...
  if (post.onlineEvent) {
    eventLocation = post.joinEventLink ? (
      <a className={classes.onlineEventLocation} href={post.joinEventLink} target="_blank" rel="noopener noreferrer">
        {post.joinEventLink}
      </a>
    ) : (
      "Online event"
    ...
    <div className={classes.root}>
      <div className={classes.headingSection}>
        <h1 className={classes.heading}>
          <a href={link} className={classes.headingLink}>
            {post.title}
          </a>{" "}
          has been updated
        </h1>
        {post.group && (
          <p className={classes.groupName}>
            Posted in{" "}
            <a href={`${getSiteUrl().slice(0, -1)}/groups/${post.group._id}`} className={classes.headingLink}>
              {post.group.name}
            </a>
          </p>
        )}
