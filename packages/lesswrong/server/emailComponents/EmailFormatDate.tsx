...
const EmailFormatDate = ({ date, format }: { date: Date; format?: string }) => {
  if (format) return <span>{moment(new Date(date)).format(format)}</span>;
  else return <span>{moment(new Date(date)).format("LLL z").trim()}</span>;
};
