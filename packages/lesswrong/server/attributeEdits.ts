...
export async function annotateAuthors(
  ...
  filteredRevs = filter(
    filteredRevs,
    (r, i) => i === 0 || i === filteredRevs.length - 1 || filteredRevs[i + 1].userId !== r.userId,
  );
...
function annotateInsDel(root: cheerio.Root): InsDelUnc[] {
  ...
  walkHtmlPreorder<InsDelUnc>(root, "unchanged", (node: cheerio.Element, state: InsDelUnc) => {
    ...
    if (node.type === "tag" || node.type === "root") {
      ...
      for (let i = 0; i < text.length; i++) annotations.push(state);
    }
    return state;
  });
...
export const attributeEdits = (
  ...
  newHtml: string,
  userId: string,
  oldAttributions: EditAttributions,
): EditAttributions => {
  ...
  for (; newTextPos < newText.length; ) {
    if (insDelAnnotations[diffPos] === "ins" && newText.charCodeAt(newTextPos) === diffText.charCodeAt(diffPos)) {
      ...
      insDelAnnotations[diffPos] === "del" &&
      oldText.charCodeAt(oldTextPos) === diffText.charCodeAt(diffPos)
    ) {
      oldTextPos++;
      diffPos++;
    } else if (
      insDelAnnotations[diffPos] === "unchanged" &&
      oldText.charCodeAt(oldTextPos) === newText.charCodeAt(newTextPos) &&
      oldText.charCodeAt(oldTextPos) === diffText.charCodeAt(diffPos)
    ) {
      ...
      while (newTextPos < newText.length && isSpace(newText.charAt(newTextPos))) {
        if (newAttributions.length > 0) newAttributions.push(newAttributions[newAttributions.length - 1]);
        else newAttributions.push(null);
        newTextPos++;
        skippedWs = true;
      }
      ...
      if (!skippedWs) {
        throw new Error(
          `Text mismatch: '${oldText.charAt(oldTextPos)}'@${oldTextPos} vs '${newText.charAt(newTextPos)}'@${newTextPos}`,
        );
      }
    }
  }
...
function mapHtmlPostorder(
  ...
  if (node.type === "tag" || node.type === "root") {
    ...
    const mappedChildren = ($copiedNode[0] as cheerio.TagElement).children.map((c) => mapHtmlPostorder($, c, callback));
    $copiedNode.empty();
    for (let mappedChild of mappedChildren) mappedChild.appendTo($copiedNode);
...
function classesToAuthorId(classes: string | null): string | null {
  ...
  for (let singleClass of singleClasses) {
    if (singleClass.startsWith("by_")) return singleClass.substr(3);
  }
  return null;
}
  ...
  walkHtmlPreorder<void>($.root()[0], undefined, (node: cheerio.Element, props: void) => {
    ...
    if (node.type === "tag" || node.type === "root") {
      if (node.attribs?.class) {
        ...
        if (newAuthorId) currentAuthorId = newAuthorId;
      }
    } else if (node.type === "text" && node.data) {
...
export const applyAttributionsToText = (
  $: cheerio.Root,
  node: cheerio.Element,
  attributions: EditAttributions,
  startOffset: number,
): cheerio.Cheerio => {
...
export const attributionsToSpans = (html: string, attributions: EditAttributions): string => {
  ...
  return cheerio.html(
    mapHtmlPostorder($, $.root()[0], ($node: cheerio.Cheerio) => {
      const node = $node[0];
      if (node.type === "text" && node.data) {
