  ...
  Mutation: {
    async RSVPToEvent(
      root: void,
      {
        ...
        nonPublic,
        response,
      }: { postId: string; name: string; email: string; nonPublic: boolean; response: string },
...
export const rsvpToEventsMutations = {
...
addGraphQLResolvers({
  Mutation: {
    async CancelRSVPToEvent(
      ...
      await createNotification({
        userId: post.userId,
        notificationType: "cancelledRSVP",
        documentType: "post",
        documentId: post._id,
        context,
      });
