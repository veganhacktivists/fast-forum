...
const minGatherTownTrackerVersion = new DatabaseServerSetting<number>(
  "gatherTownTrackerVersion",
...
const getGatherTownUsers = async (
  password: string | null,
  roomId: string,
  roomName: string,
): Promise<GatherTownCheckResult> => {
  // Register new user to Firebase
  const authResponse = await fetch(
    "https://www.googleapis.com/identitytoolkit/v3/relyingparty/signupNewUser?key=AIzaSyCifrUkqu11lgjkz2jtp4Fx_GJh58HDlFQ",
    {
      headers: {
        accept: "*/*",
  ...
  const userInformation = await fetch(
    "https://www.googleapis.com/identitytoolkit/v3/relyingparty/getAccountInfo?key=AIzaSyCifrUkqu11lgjkz2jtp4Fx_GJh58HDlFQ",
    {
      headers: {
        accept: "*/*",
  ...
  const registerUserResponse = await fetch(
    `https://gather.town/api/registerUser?roomId=${roomId}%5C${roomName}&authToken=${token}`,
    {
      headers: {
  ...
  if (reloadRequested) {
    ...
    return {
      checkFailed: false,
      gatherTownUsers: toDictionary(
        playerNames,
        (name) => name,
        (name) => playerInfoByName[name],
      ),
      failureReason: null,
    };
...
const messageToJson = (data: any) => {
  ...
  if (isJson(messageBody)) return JSON.parse(messageBody);
  else return null;
};
...
function interpretBinaryMessage(
  data: any,
): { players: { map: string; name: string; id: string; status: string }[] } | null {
  ...
  while (pos < buf.length) {
    ...
    if (messageType === 0) {
      ...
      const unknownField = buf
        .slice(unknownStringFieldStart, unknownStringFieldStart + unknownStringFieldLen)
        .toString("utf8");
