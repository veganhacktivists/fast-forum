...
export async function getPredictionsFromElicit(questionId: string): Promise<null | Array<ElicitPredictionData>> {
  const response = await fetch(
    `${elicitAPIUrl}/binary-questions/${questionId}/binary-predictions?${encode({
...
export async function getPredictionDataFromElicit(questionId: string) {
  const response = await fetch(
    `${elicitAPIUrl}/binary-questions/${questionId}?${encode({
  ...
  if (response.status !== 200)
    throw new Error(
      `Cannot get elicit prediction for questionId ${questionId}, got: ${response.status}: ${response.statusText}`,
    );
  const responseText = await response.text();
  if (!responseText) return null;
  return JSON.parse(responseText);
...
async function sendElicitPrediction(questionId: string, prediction: number, user: DbUser) {
  ...
  if (response.status !== 200)
    throw new Error(`Cannot send elicit prediction, got: ${response.status}: ${response.statusText}`);
  const responseText = await response.text();
...
async function cancelElicitPrediction(questionId: string, user: DbUser) {
  const response = await fetch(
    `${elicitAPIUrl}/binary-questions/${questionId}/binary-predictions?sourceUserId=${user._id}`,
    {
      method: "DELETE",
      headers: {
        Authorization: `API_KEY ${elicitAPIKey.get()}`,
  ...
  if (response.status !== 200)
    throw new Error(`Cannot cancel elicit prediction, got: ${response.status}: ${response.statusText}`);
}
...
async function getElicitQuestionWithPredictions(questionId: string) {
  ...
  const processedPredictions = predictions.map(
    ({ id, prediction, createdAt, notes, creator, sourceUrl, sourceId, binaryQuestionId }) => ({
      _id: id,
      predictionId: id,
      prediction,
...
async function getLocalElicitQuestionWithPredictions(
  questionId: string,
): Promise<ElicitQuestionWithPredictions | Record<any, never>> {
    ...
    const elicitPredictionResolver = {
      ...
      Mutation: {
        async MakeElicitPrediction(
          root: void,
          { questionId, prediction }: { questionId: string; prediction: number },
          ...
          if (useElicitApi) {
            if (prediction) {
              ...
              // If we provide a falsy prediction (including 0, since 0 isn't a valid prediction, we cancel our current prediction)
