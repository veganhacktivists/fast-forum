...
async function notifyUsersMatchingDialogueChecks(
  dialogueCheck: DbDialogueCheck,
  matchingDialogueCheck: DbDialogueCheck,
) {
  await Promise.all([
    createNotifications({
...
defineMutation({
  ...
  fn: async (
    ...
    }: { targetUserId: string; checked: boolean | null; hideInRecommendations: boolean | null },
    { currentUser, repos },
  ) => {
    ...
    if (typeof checked === "boolean") {
      ...
      dialogueCheck = await repos.dialogueChecks.upsertDialogueHideInRecommendations(
        currentUser._id,
        targetUserId,
        hideInRecommendations,
      );
