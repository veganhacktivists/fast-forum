...
addGraphQLResolvers({
  Query: {
    ...
    async RevisionsDiff(
      ...
      if (!afterUnfiltered) throw new Error("Revision not found");
      ...
      const before: Partial<DbRevision> | null = await accessFilterSingle(
        currentUser,
        Revisions,
        beforeUnfiltered,
        context,
      );
      const after: Partial<DbRevision> | null = await accessFilterSingle(
        currentUser,
        Revisions,
        afterUnfiltered,
        context,
