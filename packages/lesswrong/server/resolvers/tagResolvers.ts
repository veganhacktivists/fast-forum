...
const createSubforumFeedResolver =
  <SortKeyType>(sorting: SubforumFeedSort) =>
  async ({
    limit = 20,
    cutoff,
    offset,
...
for (const sortBy of subforumSortings) {
  const sorting =
    subforumFeedSortings[sortBy ?? defaultSubforumSorting] ?? subforumFeedSortings[defaultSubforumSorting];
  defineFeedResolver({
...
addGraphQLResolvers({
  Mutation: {
    async recordSubforumView(
      root: void,
      { userId, tagId }: { userId: string; tagId: string },
      context: ResolverContext,
    ) {
  ...
  Query: {
    async TagUpdatesInTimeBlock(
      ...
      const tagRevisions = await Revisions.find({
        ...
        $or: [{ "changeMetrics.added": { $gt: 0 } }, { "changeMetrics.removed": { $gt: 0 } }],
      }).fetch();
      ...
      const userIds = filterNonnull(
        _.uniq([...tagRevisions.map((tr) => tr.userId), ...rootComments.map((rc) => rc.userId)]),
      );
      ...
      const tagIds = filterNonnull(
        _.uniq([...tagRevisions.map((r) => r.documentId), ...rootComments.map((c) => c.tagId)]),
      );
      ...
      return tags.map((tag) => {
        ...
        const relevantUsersIds = filterNonnull(
          _.uniq([...relevantRevisions.map((tr) => tr.userId), ...relevantRootComments.map((rc) => rc.userId)]),
        );
        ...
        return {
          ...
          lastCommentedAt:
            relevantRootComments.length > 0
              ? _.max(relevantRootComments, (c) => c.lastSubthreadActivity).lastSubthreadActivity
              : null,
    ...
    async RandomTag(root: void, args: void, context: ResolverContext): Promise<DbTag> {
      ...
      if (!sample || !sample.length) throw new Error("No tags found");
      return sample[0];
    },

    async TagUpdatesByUser(
      root: void,
      ...
      const tagsUnfiltered = await loadByIds(context, "Tags", tagIds);
      const tags = (await accessFilterMultiple(context.currentUser, Tags, tagsUnfiltered, context)).reduce(
        (acc: Partial<Record<string, DbTag>>, tag: DbTag) => {
          acc[tag._id] = tag;
          return acc;
        },
        {},
    ...
    resolveAs: {
      ...
      resolver: async (
        ...
        const sortedContributors = orderBy(
          contributorUserIds,
          (userId) => -contributionStatsByUserId[userId]!.contributionScore,
        );
...
async function getContributorsList(tag: DbTag, version: string | null): Promise<ContributorStatsList> {
  if (version) return await buildContributorsList(tag, version);
  else if (tag.contributionStats) return tag.contributionStats;
  else return await updateDenormalizedContributorsList(tag);
}
...
async function buildContributorsList(tag: DbTag, version: string | null): Promise<ContributorStatsList> {
  ...
  const selfVoteScoreAdjustmentByUser = mapValues(selfVotesByUser, (selfVotes) => {
