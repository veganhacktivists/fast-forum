...
defineQuery({
  ...
  fn: async (root: void, args: {}, context: ResolverContext): Promise<NotificationCountsResult> => {
    ...
    const badgeNotifications = newNotifications.filter(
      (notif) => !!getNotificationTypeByName(notif.type).causesRedBadge,
    );
...
const extractLatestDialogueMessageByUser = async (
  dialogueHtml: string,
  userId: string,
): Promise<DialogueMessageInfo | undefined> => {
