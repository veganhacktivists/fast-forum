    ...
    async moderateComment(
      ...
      if (currentUser && userCanModerateComment(currentUser, post, tag, comment)) {
        ...
        if (deleted) {
          ...
          //When you undo delete, reset all delete-related fields
...
createPaginatedResolver({
  ...
  graphQLType: "Comment",
  callback: async (context: ResolverContext, limit: number): Promise<DbComment[]> =>
    context.repos.comments.getPopularComments({ limit }),
...
defineQuery({
  ...
  fn: async (
    root: void,
    { userId, targetUserId, limit }: { userId: string; targetUserId: string; limit: number },
    context: ResolverContext,
    ...
    const getIntersectingUsersVotesOnPoll = async (userId: string, targetUserId: string, pollCommentId: string) => {
      const comments = await context.repos.comments.getPopularPollCommentsWithTwoUserVotes(
        userId,
        targetUserId,
        limit,
        pollCommentId,
      ...
      const result = comments.map((comment) => ({
        comment,
        recommendationReason: "You both reacted on this comment",
        yourVote: comment.yourVote,
        theirVote: comment.theirVote,
      }));
      return result;
    };
    ...
    const getUserVotesOnPoll = async (
      ...
      const comments = await context.repos.comments.getPopularPollCommentsWithUserVotes(
        userId,
        limit * 3,
        pollCommentId,
      );
    ...
    async function* commentSources() {
      ...
      yield await getUserVotesOnPoll(
        targetUserId,
        bensInterestingDisagreementsCommentId,
        "They reacted on this comment",
        "they",
      );
      yield await getUserVotesOnPoll(
        targetUserId,
        worthwhileOpenAIDisagreementsCommentId,
        "They reacted on this comment",
        "they",
      );
      yield await getUserVotesOnPoll(
    ...
    for await (const source of commentSources()) {
      ...
      const newAnnotatedComments = (
        await accessFilterMultiple(context.currentUser, context.Comments, rawComments, context)
      ).flatMap((comment) => source.find(({ comment: rawComment }) => rawComment._id === comment._id) || []);

      recommendedComments = uniqBy(
        [...recommendedComments, ...newAnnotatedComments],
        ({ comment }) => comment._id,
      ).slice(0, limit);
