...
addGraphQLResolvers({
  Mutation: {
    ...
    submitReviewVote: async (
      root: void,
      args: {
        postId: string;
        qualitativeScore: number;
        quadraticChange: number;
        newQuadraticScore: number;
        comment: string;
        year: string;
        dummy: boolean;
        reactions: string[];
      ...
      if (!(await accessFilterSingle(currentUser, Posts, post, context))) throw new Error("Invalid postId");
      ...
      if (!existingVote) {
        ...
        // updateMutator
        const finalQuadraticScore =
          typeof newQuadraticScore !== "undefined"
            ? newQuadraticScore
