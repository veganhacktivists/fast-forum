...
augmentFieldsDict(Posts, {
  ...
  sideComments: {
    resolveAs: {
      ...
      ): Promise<SideCommentsResolverResult | null> => {
...
addGraphQLResolvers({
  Query: {
    ...
    async DigestPlannerData(
      root: void,
      { digestId, startDate, endDate }: { digestId: string; startDate: Date; endDate: Date },
      context: ResolverContext,
    ) {
      const { currentUser, repos } = context;
    ...
    async UsersReadPostsOfTargetUser(
      root: void,
      { userId, targetUserId, limit = 20 }: { userId: string; targetUserId: string; limit: number },
      context: ResolverContext,
    ) {
      const { currentUser, repos } = context;
      if (!currentUser) {
...
createPaginatedResolver({
  ...
  graphQLType: "Post",
  callback: async (context: ResolverContext, limit: number): Promise<DbPost[]> =>
    context.repos.posts.getDigestHighlights({ limit }),
});
...
createPaginatedResolver({
  ...
  graphQLType: "Post",
  callback: async (context: ResolverContext, limit: number): Promise<DbPost[]> =>
    context.repos.posts.getTopWeeklyDigestPosts(limit),
...
createPaginatedResolver({
  ...
  graphQLType: "Post",
  callback: async ({ repos, currentUser }: ResolverContext, limit: number): Promise<DbPost[]> =>
    repos.posts.getCuratedAndPopularPosts({
      currentUser,
      limit,
    }),
  cacheMaxAgeMs: 1000 * 60 * 60, // 1 hour
...
createPaginatedResolver({
  ...
  graphQLType: "Post",
  callback: async ({ repos }: ResolverContext, limit: number): Promise<DbPost[]> =>
    repos.posts.getRecentlyActiveDialogues(limit),
