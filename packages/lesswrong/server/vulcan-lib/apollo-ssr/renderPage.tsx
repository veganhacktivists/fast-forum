...
import {
  getAllUserABTestGroups,
  CompleteTestGroupAllocation,
  RelevantTestGroupAllocation,
} from "../../../lib/abTestImpl";
...
export const renderWithCache = async (req: Request, res: Response, user: DbUser | null) => {
  ...
  if (!getPublicSettingsLoaded())
    throw Error("Failed to render page because publicSettings have not yet been initialized on the server");
  ...
  if ((!isHealthCheck && (user || isExcludedFromPageCache(url, abTestGroups))) || isSlackBot) {
    ...
    const rendered = await cachedPageRender(req, abTestGroups, userAgent, (req: Request) =>
      queueRenderRequest({
        req,
        user: null,
        startTime,
...
const userAgentExclusions = ["health", "bot", "spider", "crawler", "curl", "node", "python"];
...
const renderRequest = async ({
  ...
  clientId,
  userAgent,
}: RenderRequestParams): Promise<RenderResult> => {
  ...
  try {
    ...
    console.error(
      `Error while fetching Apollo Data. date: ${new Date().toString()} url: ${JSON.stringify(getPathFromReq(req))}`,
