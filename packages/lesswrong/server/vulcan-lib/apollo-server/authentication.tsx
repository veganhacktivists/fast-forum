...
function promisifiedAuthenticate(
  req: ResolverContext["req"],
  res: ResolverContext["res"],
  name: string,
  options: any,
...
export async function sendVerificationEmail(user: DbUser) {
  ...
  await wrapAndSendEmail({
    ...
    body: (
      <div>
        <p>Click here to verify your {forumTitleSetting.get()} email</p>
        <p>
  ...
  Mutation: {
    ...
    async resetPassword(root: void, { email }: { email: string }, context: ResolverContext) {
      ...
      const emailSucceeded = await wrapAndSendEmail({
        ...
        body: (
          <div>
            <p>You requested a password reset. Follow the following link to reset your password:</p>
            <p>
      ...
      else return `Failed to send password reset email. The account might not have a valid email address configured.`;
    },
