 ...
 */
function createOAuthUserHandler<P extends Profile>(
  profilePath: string,
 ...
 */
function createOAuthUserHandlerAuth0(
  profilePath: string,
  getIdFromProfile: IdFromProfile<Auth0Profile>,
  getUserDataFromProfile: UserDataFromProfile<Auth0Profile>,
) {
...
function createAccessTokenStrategy(auth0Strategy: AnyBecauseTodo) {
  const accessTokenUserHandler = createOAuthUserHandler(
    "services.auth0",
    (profile) => profile.id,
...
<<<<<<< HEAD
        createOAuthUserHandler<GoogleProfile>(
          "services.google",
          (profile) => profile.id,
          async (profile) => ({
            services: {
  ...
  if (facebookClientId && facebookOAuthSecret) {
    passport.use(
      new FacebookOAuthStrategy(
        ...
        createOAuthUserHandler<FacebookProfile>(
          "services.facebook",
          (profile) => profile.id,
          async (profile) => ({
            email: profile.emails?.[0].value,
  ...
  if (githubClientId && githubOAuthSecret) {
    passport.use(
      new GithubOAuthStrategy(
        ...
        createOAuthUserHandler<GithubProfile>(
          "services.github",
          ...
          async (profile) => ({
            ...
            username: await Utils.getUnusedSlugByCollectionName(
              "Users",
              slugify(profile.username || profile.displayName),
            ),
