...
export const htmlToDraft = convertFromHTML({
  htmlToEntity: (nodeName, node, createEntity) => {
    if (nodeName === "img") {
      ...
      return createEntity("IMAGE", "IMMUTABLE", { src: nodeImg.src });
    }
    if (nodeName === "a") {
      const nodeA = node as HTMLAnchorElement;
      return createEntity("LINK", "MUTABLE", { url: nodeA.href });
    }
    // if (nodeName === 'img') {
    //   return createEntity(
    //     'IMAGE',
  ...
  htmlToBlock: (nodeName, node, lastList, inBlock) => {
    ...
    // }
    if (nodeName === "hr") {
      // This currently appears to be broken, sadly. TODO: Fix this
      return {
        type: "divider",
        data: {},
        text: "as",
