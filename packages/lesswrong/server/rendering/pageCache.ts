<<<<<<< HEAD:packages/lesswrong/server/vulcan-lib/apollo-ssr/pageCache.ts
...
>>>>>>> base/master:packages/lesswrong/server/rendering/pageCache.ts
...
<<<<<<< HEAD:packages/lesswrong/server/vulcan-lib/apollo-ssr/pageCache.ts
  const themeOptions =
    themeCookie && isValidSerializedThemeOptions(themeCookie) ? themeCookie : JSON.stringify(getDefaultThemeOptions());
=======
...
>>>>>>> base/master:packages/lesswrong/server/rendering/pageCache.ts
  ...
  if (timezoneCookie) return `${path}&theme=${themeOptions}&timezone=${timezoneCookie}`;
  else return path;
};
...
function filterLoggedOutActiveAbTestGroups(abTestGroups: CompleteTestGroupAllocation): RelevantTestGroupAllocation {
  ...
  for (const name of Object.keys(abTestGroups)) {
    ...
    if (abTest.active && abTest.affectsLoggedOut) result[name] = abTestGroups[name];
  }
  return result;
}
...
>>>>>>> base/master:packages/lesswrong/server/rendering/pageCache.ts
  ...
  if (cacheKey in inProgressRenders) {
    ...
    //eslint-disable-next-line no-console
    console.log(
      `In progress render merge of ${cacheKey} missed: mismatched A/B test groups (requested: ${JSON.stringify(cacheAffectingAbTestGroups)}, available: ${JSON.stringify(inProgressRenders[cacheKey].map((r) => r.abTestGroups))})`,
    );
  }
...
<<<<<<< HEAD:packages/lesswrong/server/vulcan-lib/apollo-ssr/pageCache.ts
  ...
  if (!inProgressRenders[cacheKey].length) delete inProgressRenders[cacheKey];
...
<<<<<<< HEAD:packages/lesswrong/server/vulcan-lib/apollo-ssr/pageCache.ts
const cacheLookupLocal = (
  cacheKey: string,
  abTestGroups: CompleteTestGroupAllocation,
): RenderResult | null | undefined => {
=======
...
>>>>>>> base/master:packages/lesswrong/server/rendering/pageCache.ts
  ...
  for (let i = 0; i < abTestCombinations.length; i++) {
    if (objIsSubset(abTestCombinations[i], abTestGroups)) {
      const lookupResult = pageCache.get(
        JSON.stringify({
          cacheKey: cacheKey,
  ...
  // eslint-disable-next-line no-console
  console.log(
    `Local cache miss for cacheKey ${cacheKey}: wrong A/B test groups: wanted ${JSON.stringify(abTestGroups)}, had available ${JSON.stringify(cachedABtestsIndex[cacheKey])}`,
  );
  return null;
};

<<<<<<< HEAD:packages/lesswrong/server/vulcan-lib/apollo-ssr/pageCache.ts
const cacheLookupDB = async (
  cacheKey: string,
  abTestGroups: CompleteTestGroupAllocation,
): Promise<RenderResult | null | undefined> => {
  const cacheResult = await new PageCacheRepo().lookupCacheEntry({
    path: cacheKey,
    completeAbTestGroups: abTestGroups,
  });
=======
...
>>>>>>> base/master:packages/lesswrong/server/rendering/pageCache.ts
  ...
  if (!cacheResult?.renderResult) {
    // eslint-disable-next-line no-console
    console.log(
      `DB cache miss for cacheKey ${cacheKey}: no cached page with this cacheKey and a valid A/B test group combination`,
    );
    return null;
  }
...
const objIsSubset = <A extends Record<string, any>, B extends Record<string, any>>(subset: A, superset: B): boolean => {
  for (let key in subset) {
    if (!(key in superset) || subset[key] !== superset[key as AnyBecauseHard]) return false;
  }
  return true;
};
...
const cacheStoreLocal = (cacheKey: string, abTestGroups: RelevantTestGroupAllocation, rendered: RenderResult): void => {
  ...
  if (cacheKey in cachedABtestsIndex) cachedABtestsIndex[cacheKey].push(abTestGroups);
  else cachedABtestsIndex[cacheKey] = [abTestGroups];
};
...
const clearExpiredCacheEntries = (): void => {
  for (let cacheKey of keysToCheckForExpiredEntries) {
    ...
    if (remainingEntriesArray.length > 0) cachedABtestsIndex[cacheKey] = remainingEntriesArray;
    else delete cachedABtestsIndex[cacheKey];
  }
...
<<<<<<< HEAD:packages/lesswrong/server/vulcan-lib/apollo-ssr/pageCache.ts
export function recordDatadogCacheEvent(cacheEvent: {
  path: string;
  userAgent: string;
...
export function checkForMemoryLeaks() {
  ...
  const cachedABtestsIndexArrayElements = sumBy(
    Object.keys(cachedABtestsIndex),
    (key) => cachedABtestsIndex[key]?.length || 0,
  );
