...
export const getEAGApplicationData = async (email: string) => {
  ...
  email = email.replace(new RegExp(/[^a-z0-9+_.@-]/gi), "");
...
const refreshZohoToken = async () => {
  const refreshResponse = await fetch(
    `https://accounts.zoho.com/oauth/v2/token?refresh_token=${zohoRefreshToken.get()}&client_id=${zohoClientId.get()}&client_secret=${zohoClientSecret.get()}&grant_type=refresh_token`,
    {
      headers: {
        accept: "*/*",
      },
