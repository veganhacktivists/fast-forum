...
getCollectionHooks("Users").editAsync.add(async function userEditChangeDisplayNameCallbacksAsync(
  user: DbUser,
  oldUser: DbUser,
) {
...
getCollectionHooks("Users").updateAsync.add(async function userEditDeleteContentCallbacksAsync({
  newDocument,
  oldDocument,
  currentUser,
}) {
...
export async function userDeleteContent(user: DbUser, deletingUser: DbUser, deleteTags = true) {
  ...
  for (let comment of comments) {
    ...
    for (let notification of notifications) {
      await deleteMutator({
        // TODO: This should be a soft-delete not a hard-delete
        collection: Notifications,
        documentId: notification._id,
        validate: false,
      });
...
async function deleteUserTagsAndRevisions(user: DbUser, deletingUser: DbUser) {
  ...
  for (let revision of tagRevisions) {
    ...
    if (document && revision.fieldName) {
      await syncDocumentWithLatestRevision(collection, document, revision.fieldName);
    }
  }
}
