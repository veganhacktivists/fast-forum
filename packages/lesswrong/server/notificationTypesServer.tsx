...
type ServerRegisterNotificationType = Omit<ServerNotificationType, "skip"> &
  Partial<Pick<ServerNotificationType, "skip">>;
...
export const getNotificationTypeByNameServer = (name: string): ServerNotificationType => {
  if (name in notificationTypes) return notificationTypes[name];
  else throw new Error(`Invalid notification type: ${name}`);
};

const serverRegisterNotificationType = ({
  skip = async () => false,
  ...notificationTypeClass
}: ServerRegisterNotificationType): ServerNotificationType => {
  ...
  emailBody: async ({ user, notifications }: { user: DbUser; notifications: DbNotification[] }) => {
    ...
    return (
      <Components.NewPostEmail
        ...
        reason="you are subscribed to this group"
      />
    );
  },
});
...
export const NewDialogueMessageNotification = serverRegisterNotificationType({
  ...
  emailBody: async ({ user, notifications }: { user: DbUser; notifications: DbNotification[] }) => {
    ...
    return (
      <Components.NewDialogueMessagesEmail
        documentId={postId}
        userId={user._id}
...
export const NewMessageNotification = serverRegisterNotificationType({
  ...
  loadData: async function ({ user, notifications }: { user: DbUser; notifications: DbNotification[] }) {
    ...
    const participantIds = _.uniq(
      _.flatten(
        conversations.map((conversation) => conversation.participantIds),
        true,
      ),
    );
  ...
  emailBody: async function ({ user }: { user: DbUser }) {
    return (
      <div>
        ...
        <p>- The {forumTitleSetting.get()} Team</p>
      </div>
    );
  },
});
  ...
  emailBody: async ({ user, notifications }: { user: DbUser; notifications: DbNotification[] }) => {
    ...
    return (
      <Components.NewPostEmail
        documentId={postId}
        hideRecommendations={true}
        reason="you are subscribed to nearby events notifications"
      />
    );
  },
});
...
export const KarmaPowersGainedNotification = serverRegisterNotificationType({
  ...
  emailBody: async ({ user, notifications }: { user: DbUser; notifications: DbNotification[] }) => {
    return <div>Your votes are stronger because your karma went up!</div>;
  },
});
  ...
  emailBody: async ({ user, notifications }: { user: DbUser; notifications: DbNotification[] }) => {
    ...
    return (
      <div>
        <p>Hi {user.displayName},</p>
        <p>
          You've been assigned as a group organizer for <a href={groupLink}>{localGroup.name}</a> on{" "}
          {siteNameWithArticleSetting.get()}.
        </p>
        <p>
          We recommend you check the group's info and update it if necessary. You can also post your group's events on
          the forum, which get advertised to users based on relevance.
        </p>
        <p>- The {forumTitleSetting.get()} Team</p>
      </div>
    );
    ...
    return (
      <div>
        {notifications.map((notification, i) => (
          <div key={i}>
            <div>
              <EmailUsernameByID userID={notification.extraData?.senderUserID} /> commented on{" "}
...
export const NewSubforumMemberNotification = serverRegisterNotificationType({
  ...
  emailBody: async ({ user, notifications }: { user: DbUser; notifications: DbNotification[] }) => {
    ...
    if (!newUser)
      throw new Error(
        `Cannot find user for which this notification is being sent, user id: ${notifications[0].documentId}`,
      );
    if (!subforum)
      throw new Error(
        `Cannot find subforum for which this notification is being sent, subforum id: ${notifications[0].extraData?.subforumId}`,
      );
    ...
    return (
      <div>
        <p>Hi {user.displayName},</p>
        <p>
          Your subforum, <a href={tagGetSubforumUrl(subforum, true)}> {subforum?.name}</a> has a new member:{" "}
          <a href={userGetProfileUrl(newUser, true)}>{newUser?.displayName}</a>.
        </p>
        <p>- The {forumTitleSetting.get()} Team</p>
      </div>
    );
...
export const NewMentionNotification = serverRegisterNotificationType({
  ...
  emailSubject: async ({ user, notifications }: { user: DbUser; notifications: DbNotification[] }) => {
    const summary = await getDocumentSummary(
      notifications[0].documentType as NotificationDocument,
      notifications[0].documentId,
    );
    if (!summary) {
  ...
  emailBody: async ({ user, notifications }: { user: DbUser; notifications: DbNotification[] }) => {
    ...
    return (
      <p>
        {summary.associatedUserName} mentioned you in{" "}
        <a href={makeAbsolute(notifications[0].link)}>{summary.displayName}</a>.
      </p>
    );
  },
});
