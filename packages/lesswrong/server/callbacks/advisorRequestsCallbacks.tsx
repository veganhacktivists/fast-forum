...
getCollectionHooks("AdvisorRequests").updateAsync.add(async function AdvisorRequestsNewAsync({
  document,
  oldDocument,
  currentUser,
}: {
  document: DbAdvisorRequest;
  oldDocument: DbAdvisorRequest;
  currentUser: DbUser | null;
}) {
  const newInterestedJobs = Object.keys(document.jobAds).filter(
    (jobId) => document.jobAds[jobId].state === "interested" && oldDocument?.jobAds[jobId]?.state !== "interested",
  );
