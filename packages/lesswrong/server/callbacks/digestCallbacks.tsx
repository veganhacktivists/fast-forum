...
getCollectionHooks("Digests").updateAsync.add(
  async ({
    newDocument,
    oldDocument,
    context,
  }: {
    newDocument: DbDigest;
    oldDocument: DbDigest;
...
getCollectionHooks("Digests").updateAsync.add(
  async ({
    ...
    if (newDocument.startDate && newDocument.startDate !== oldDocument.startDate) {
      await Digests.rawUpdateOne({ num: newDocument.num - 1 }, { $set: { endDate: newDocument.startDate } });
    }
    ...
    if (newDocument.endDate && newDocument.endDate !== oldDocument.endDate) {
      await Digests.rawUpdateOne({ num: newDocument.num + 1 }, { $set: { startDate: newDocument.endDate } });
    }
  },
);
