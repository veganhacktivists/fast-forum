...
import {
  FLAGGED_FOR_N_DMS,
  MAX_ALLOWED_CONTACTS_BEFORE_BLOCK,
...
async function flagOrBlockUserOnManyDMs({
  ...
  logger("new allUsersEverContacted", allUsersEverContacted, "(length: ", allUsersEverContacted.length, ")");
...
getCollectionHooks("Conversations").createBefore.add(async function flagUserOnManyDMsCreate(document, properties) {
  return flagOrBlockUserOnManyDMs({
    currentConversation: document,
...
getCollectionHooks("Conversations").updateBefore.add(async function flagUserOnManyDMsCreate(data, properties) {
  return flagOrBlockUserOnManyDMs({
    ...
    oldConversation: properties.oldDocument,
    currentUser: properties.currentUser,
    context: properties.context,
  });
});
...
getCollectionHooks("Conversations").updateAsync.add(async function leavingNotication({ newDocument, oldDocument }) {
  const usersWhoLeft = (oldDocument?.participantIds ?? []).filter((id) => !newDocument.participantIds?.includes(id));
