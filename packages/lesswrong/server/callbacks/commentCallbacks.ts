...
getCollectionHooks("Comments").newValidate.add(async function createShortformPost(
  comment: DbComment,
  currentUser: DbUser,
) {
...
getCollectionHooks("Comments").newSync.add(async function CommentsNewOperations(comment: DbComment) {
  ...
  if (comment.postId) {
    ...
    await Promise.all([updateLastCommentedAtPromise, updateReadStatusesPromise]);
...
getCollectionHooks("Comments").removeAsync.add(async function CommentsRemovePostCommenters(
  comment: DbComment,
  currentUser: DbUser,
) {
  ...
  if (postId) {
    const postComments = await Comments.find(
      { postId, debateResponse: false, deleted: false },
      { sort: { postedAt: -1 } },
    ).fetch();
...
getCollectionHooks("Comments").removeAsync.add(async function CommentsRemoveChildrenComments(
  comment: DbComment,
  currentUser: DbUser,
) {
...
getCollectionHooks("Comments").editAsync.add(async function CommentsEditSoftDeleteCallback(
  comment: DbComment,
...
export async function moderateCommentsPostUpdate(
  comment: DbComment,
  currentUser: DbUser,
  ...
  if (comment.postId) {
    ...
    const lastCommentedAt =
      (lastComment && lastComment.postedAt) || (await Posts.findOne({ _id: comment.postId }))?.postedAt || new Date();
...
async function sendModerationPM({
  messageContents,
  lwAccount,
  comment,
  noEmail,
...
async function commentsRejectSendPMAsync(comment: DbComment, currentUser: DbUser) {
  ...
  const noEmail = isEAForum ? false : !(!!commentUser?.reviewedByUserId && !commentUser.snoozedUntilContentCount);
...
getCollectionHooks("Comments").editSync.add(async function validateDeleteOperations(
  modifier,
  comment: DbComment,
  currentUser: DbUser,
) {
  ...
  if (modifier.$set) {
    if (modifier.$set.answer === true) {
      await Comments.rawUpdateMany(
        ...
        { $set: { parentAnswerId: comment._id } },
        { multi: true },
      );
    } else if (modifier.$set.answer === false) {
      await Comments.rawUpdateMany(
        { topLevelCommentId: comment._id },
        { $unset: { parentAnswerId: true } },
        { multi: true },
      );
    }
  }
...
getCollectionHooks("Comments").createBefore.add(async function SetTopLevelCommentId(comment: DbComment, context) {
  ...
  while (rootComment?.parentCommentId) {
    ...
    if (rootComment && visited[rootComment._id]) throw new Error("Cyclic parent-comment relations detected!");
    if (rootComment) visited[rootComment._id] = true;
  }
...
getCollectionHooks("Comments").updateAsync.add(async function updatedCommentMaybeTriggerReview({
  currentUser,
}: UpdateCallbackProperties<"Comments">) {
  if (!currentUser) return;
  currentUser.snoozedUntilContentCount &&
    (await updateMutator({
      collection: Users,
      documentId: currentUser._id,
      set: {
...
getCollectionHooks("Comments").updateAsync.add(async function updateUserNotesOnCommentRejection({
  document,
  oldDocument,
  currentUser,
  context,
