...
export const htmlToPingbacks = async (
  html: string,
  exclusions?: Array<{ collectionName: string; documentId: string }> | null,
): Promise<Partial<Record<CollectionNameString, Array<string>>>> => {
          ...
          if (pingback) {
            if (
              ...
              _.find(
                exclusions,
                (exclusion) =>
                  exclusion.documentId === pingback.documentId && exclusion.collectionName === pingback.collectionName,
              )
            ) {
              // Pingback is excluded
            } else {
              if (!(pingback.collectionName in pingbacks)) pingbacks[pingback.collectionName] = [];
