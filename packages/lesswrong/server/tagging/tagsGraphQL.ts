...
addGraphQLResolvers({
  Mutation: {
    addOrUpvoteTag: async (
      root: void,
      { tagId, postId }: { tagId: string; postId: string },
      context: ResolverContext,
    ) => {
    ...
    addTags: async (
      root: void,
      { postId, tagIds }: { postId: string; tagIds: Array<string> },
      context: ResolverContext,
    ) => {
      const { currentUser } = context;
      if (!currentUser) throw new Error("You must be logged in to tag");
      ...
      await Promise.all(tagIds.map((tagId) => addOrUpvoteTag({ tagId, postId, currentUser, context })));
