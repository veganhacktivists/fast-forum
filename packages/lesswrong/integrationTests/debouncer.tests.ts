...
import {
  ...
  getDailyBatchTimeAfter,
  getWeeklyBatchTimeAfter,
} from "../server/debouncer";
...
describe("EventDebouncer", () => {
      ...
      const testEvent = new EventDebouncer({
        ...
        callback: (key: string, events: Array<string>) => {
          ...
          events.forEach((ev: string) => {
            ...
            if (!(key in eventsHandled)) eventsHandled[key] = {};
            if (!(ev in eventsHandled[key]!)) eventsHandled[key]![ev] = 0;
  ...
  it("times daily batches correctly", async () => {
    getDailyBatchTimeAfter(new Date("1980-01-01 00:20:00Z"), 3)
      .toString()
      .should.equal(new Date("1980-01-01 03:00:00Z").toString());
    getDailyBatchTimeAfter(new Date("1980-01-01 05:20:00Z"), 3)
      .toString()
      .should.equal(new Date("1980-01-02 03:00:00Z").toString());
  });
  it("times weekly batches correctly", async () => {
    getWeeklyBatchTimeAfter(new Date("1980-01-01 00:20:00Z"), 3, "Friday")
      .toString()
      .should.equal(new Date("1980-01-04 03:00:00Z").toString());
    getWeeklyBatchTimeAfter(new Date("1980-01-01 00:20:00Z"), 3, "Tuesday")
      .toString()
      .should.equal(new Date("1980-01-01 03:00:00Z").toString());
    getWeeklyBatchTimeAfter(new Date("1980-01-01 03:20:00Z"), 3, "Tuesday")
      .toString()
      .should.equal(new Date("1980-01-08 03:00:00Z").toString());
  });
});
