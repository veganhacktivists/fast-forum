...
export const userCanSuggestPostForAlignment = ({
  currentUser,
  post,
}: {
  ...
  return (
    ...
    userCanDo(currentUser, "posts.alignment.suggest")
  );
};
...
export const userCanMakeAlignmentPost = (user: DbUser | UsersCurrent | null, post: PostsBase | DbPost) => {
  ...
  return !!(user._id === post.userId && userCanDo(user, "posts.alignment.move") && userOwns(user, post));
};
