...
export function unflattenComments<T extends ThreadableCommentType>(comments: Array<T>): Array<CommentTreeNode<T>> {
  const resultsRestructured = _.map(comments, (comment: T): CommentTreeNode<T> => {
    return { item: comment, children: [] };
  });
  return unflattenCommentsRec(resultsRestructured);
}

export function addGapIndicators<T extends ThreadableCommentType>(
  comments: Array<CommentTreeNode<T>>,
): Array<CommentTreeNode<T & { gapIndicator?: boolean }>> {
...
function unflattenCommentsRec<T extends ThreadableCommentType>(
  array: Array<CommentTreeNode<T>>,
  parent?: CommentTreeNode<T>,
): Array<CommentTreeNode<T>> {
  ...
  if (typeof parent === "undefined") {
    ...
    children = _.filter(
      array,
      (node: CommentTreeNode<T>) => !node.item.parentCommentId || !commentDict[node.item.parentCommentId],
    );
  } else {
...
export function commentTreesEqual<Fragment extends ThreadableCommentType>(
  a: Array<CommentTreeNode<Fragment>>,
  b: Array<CommentTreeNode<Fragment>>,
) {
  if (!!a !== !!b) return false;
  if (!a && !b) return true;
  ...
  for (let i = 0; i < a.length; i++) {
    ...
    if (!commentTreesEqual(a[i].children, b[i].children)) return false;
  }
  return true;
}
