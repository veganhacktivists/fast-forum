...
// instead.
export function getSchema<N extends CollectionNameString>(collection: CollectionBase<N>): SchemaType<N> {
  //return collection._schemaFields;
  return getSimpleSchema(collection)._schema;
}

export function getSimpleSchema<N extends CollectionNameString>(collection: CollectionBase<N>): SimpleSchemaType<N> {
