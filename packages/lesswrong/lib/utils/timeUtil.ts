...
export const useSsrRenderedAt = () => {
  ...
  return typeof window === "undefined" ? currentTime : new Date(window.ssrRenderedAt);
};
...
// zone.
export const convertTimeOfWeekTimezone = (
  timeOfDay: number,
  dayOfWeek: string | number,
  fromTimezone: string,
  toTimezone: string,
) => {
  let time = moment().tz(fromTimezone).day(dayOfWeek).hour(timeOfDay).minute(0).tz(toTimezone);
  return {
    timeOfDay: time.hour(),
    dayOfWeek: time.format("dddd") as
      | "Monday"
      | "Tuesday"
      | "Wednesday"
      | "Thursday"
