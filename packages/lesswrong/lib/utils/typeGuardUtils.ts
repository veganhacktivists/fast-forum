...
export function areFieldsNotNull<T, K extends keyof T>(
  item: T,
  ...fields: [K, ...K[]]
): item is T & FieldsNotNull<T, K> {
  return fields.some((field) => item[field] !== null && item[field] !== undefined);
}

function areFieldsNotNullCurry<T, K extends keyof T>(
  ...fields: [K, ...K[]]
): (item: T) => item is T & FieldsNotNull<T, K> {
  return ((item) => fields.some((field) => item[field] !== null && item[field] !== undefined)) as (
    item: T,
  ) => item is T & FieldsNotNull<T, K>;
}

export function filterWhereFieldsNotNull<T, K extends keyof T>(
  arr: T[],
  ...fields: [K, ...K[]]
): FieldsNotNull<T, K>[] {
