...
export async function getPostPingbackById(
  parsedUrl: RouterLocation,
  postId: string | null,
): Promise<PingbackDocument | null> {
...
export const canMention = (
  currentUser: ValidationUserPartial,
  mentionsCount: number,
  { karmaThreshold = 1, mentionsLimit = 10 }: { karmaThreshold?: number; mentionsLimit?: number } = {},
  ...
  if (currentUser.conversationsDisabled)
    return {
      result: false,
