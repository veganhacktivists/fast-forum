  ...
  return {
    ...
    multi: {
      ...
      async resolver(
        root: void,
        args: {
          input: { terms: ViewTermsBase; enableCache?: boolean; enableTotal?: boolean; createIfMissing?: Partial<T> };
        },
        context: ResolverContext,
        { cacheControl }: AnyBecauseTodo,
      ) {
        const startResolve = Date.now();
        const input = args?.input || {};
        const { terms = {}, enableCache = false, enableTotal = false } = input;
        const logger = loggerConstructor(
          `views-${collectionName.toLowerCase()}-${terms.view?.toLowerCase() ?? "default"}`,
        );
    ...
    single: {
      ...
      async resolver(
        ...
        logger("");
        logGroupStart(`--------------- start \x1b[35m${typeName} Single Resolver\x1b[0m ---------------`);
