...
import {
  ...
  NavigationContext,
} from "./vulcan-core/appContext";
import type { RouterLocation } from "./vulcan-lib/routes";
import { keys, indexOf } from "underscore";
...
export const removeUrlParameters = (url: string, queryParameterBlacklist: string[]): string => {
  ...
  for (let key of keys(parsedQuery)) {
  ...
  return (
    baseUrl +
    (Object.keys(filteredQuery).length > 0 ? "?" + qs.stringify(filteredQuery) : "") +
    (hash ? "#" + hash : "")
  );
};
...
const forumDomainWhitelist: ForumOptions<DomainList> = {
  ...
  default: {
    onsiteDomains: [`localhost:${getServerPort()}`],
    mirrorDomains: [],
  },
};
...
export const classifyHost = (urlOrHost: string | URL): "onsite" | "offsite" | "mirrorOfUs" => {
  ...
  domainWhitelist.onsiteDomains.forEach((domain) => {
    if (isSameDomainModuloWWW(host, domain)) urlType = "onsite";
  });
  domainWhitelist.mirrorDomains.forEach((domain) => {
    if (isSameDomainModuloWWW(host, domain)) urlType = "mirrorOfUs";
  });
