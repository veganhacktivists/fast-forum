...
export type PermissionableUser = UsersMinimumInfo &
  Pick<DbUser, "groups" | "banned" | "allCommentingDisabled" | "isAdmin" | "reviewedByUserId">;
...
export const userGetGroups = (user: PermissionableUser | DbUser | null): Array<string> => {
  ...
  if (user.banned && user.banned > moment().toDate()) {
    // banned users have no membership permissions
    return ["guests"];
  }
...
export const documentIsNotDeleted = (user: PermissionableUser | DbUser | null, document: OwnableDocument) => {
...
export const userHasntChangedName = (
  user: UsersMinimumInfo | DbUser | null,
  document: HasUserIdType | DbUser | UsersMinimumInfo | DbObject,
): boolean => {
  if (!user) return false;
...
export const userIsAdmin = function <T extends UsersMinimumInfo | DbUser | null>(
  user: T,
): user is Exclude<T, null> & { isAdmin: true } {
  if (!user) return false;
  return user.isAdmin;
};
