 ...
 */
export function getGraphQLQueryFromOptions({
  extraVariables,
  collection,
  fragment,
  fragmentName,
  ...
  if (extraVariables) {
    extraVariablesString = Object.keys(extraVariables)
      .map((k) => `$${k}: ${extraVariables[k]}`)
      .join(", ");
  }
...
type TSuccessReturn<FragmentTypeName extends keyof FragmentTypes> = {
  loading: false;
  error: undefined;
...
type TReturn<FragmentTypeName extends keyof FragmentTypes> = (
  | TSuccessReturn<FragmentTypeName>
  | TErrorReturn
  | TLoadingReturn
) & {
  refetch: any;
...
export type UseSingleProps<FragmentTypeName extends keyof FragmentTypes> = DocumentIdOrSlug & {
