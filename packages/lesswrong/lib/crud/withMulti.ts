...
export type UseMultiResult<FragmentTypeName extends keyof FragmentTypes> = {
  loading: boolean;
...
>({
  ...
  const newNextFetchPolicy =
    nextFetchPolicy || fetchPolicy === "cache-and-network" || fetchPolicy === "network-only" ? "cache-only" : undefined;
