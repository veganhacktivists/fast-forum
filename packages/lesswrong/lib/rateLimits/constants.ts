...
const LW: { POSTS: PostAutoRateLimit[]; COMMENTS: CommentAutoRateLimit[] } = {
  POSTS: [
    ...
    // 1 post per 2+ weeks rate limits
    {
      ...
      isActive: (user, features) => user.karma < 0 && features.last20Karma < -30 && features.postDownvoterCount >= 5,
    ...
    },
    {
      ...
      isActive: (user, features) =>
        user.karma < 0 && features.last20PostKarma < -45 && features.postDownvoterCount >= 5,
  ...
  COMMENTS: [
    {
      ...
      isActive: (user, features) => features.last20Karma < 0 && features.downvoterCount >= 3,
    ...
    },
    {
      ...
      appliesToOwnPosts: false,
      isActive: (user, features) => user.karma < 2000 && features.last20Karma < 1, // requires 1 weak upvote from a 1000+ karma user, or two new user upvotes, but at 2000+ karma I trust you more to go on long conversations
    ...
    // 1 comment per 3 days rate limits
    {
      ...
      appliesToOwnPosts: false,
      isActive: (user, features) => user.karma < 500 && features.last20Karma < -15 && features.downvoterCount >= 5,
...
export const autoPostRateLimits: ForumOptions<PostAutoRateLimit[]> = {
  EAForum: [ALL.POSTS.FIVE_PER_DAY],
  LessWrong: [...LW.POSTS],
  default: [ALL.POSTS.FIVE_PER_DAY],
};

export const autoCommentRateLimits: ForumOptions<CommentAutoRateLimit[]> = {
  EAForum: [ALL.COMMENTS.ONE_PER_EIGHT_SECONDS, ...EA.COMMENTS],
  LessWrong: [ALL.COMMENTS.ONE_PER_EIGHT_SECONDS, ...LW.COMMENTS],
  default: [ALL.COMMENTS.ONE_PER_EIGHT_SECONDS],
};
