...
export const editableCollectionsFieldOptions: Record<
  CollectionNameString,
  Record<string, MakeEditableOptions>
> = {} as any;
let editableFieldsSealed = false;
export function sealEditableFields() {
  editableFieldsSealed = true;
}
...
export const makeEditable = <N extends CollectionNameString>({
  ...
  if (editableFieldsSealed)
    throw new Error(
      "Called makeEditable after addAllEditableCallbacks already ran; this indicates a problem with import order",
    );
  ...
  const getLocalStorageId =
    options.getLocalStorageId ||
    ((doc: any, name: string): { id: string; verify: boolean } => {
          ...
          if (version) {
            if (version === "draft") {
              ...
              const revision = await Revisions.findOne(
                { documentId: doc._id, fieldName: field },
                { sort: { editedAt: -1 } },
              );
      ...
      resolveAs: {
        ...
        resolver: async (post: ObjectsByCollectionName[N], args: { limit: number }, context: ResolverContext) => {
          ...
          const loaderResults = await getWithLoader(
            context,
            Revisions,
            `revisionsByDocumentId_${field}_${limit}`,
            { fieldName: field },
            "documentId",
            post._id,
