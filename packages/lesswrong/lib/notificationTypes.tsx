...
const registerNotificationType = ({
...
export const NewGroupPostNotification = registerNotificationType({
  ...
  async getMessage({ documentType, documentId }: GetMessageProps) {
    ...
    if (group)
      return (
        (await postGetAuthorName(document as DbPost)) + ' has created a new post in the group "' + group.name + '"'
      );
    else return (await postGetAuthorName(document as DbPost)) + " has created a new post in a group";
  },
  getIcon() {
    return <Components.ForumIcon icon="Bell" style={iconStyles} />;
  },
...
export const NewCommentNotification = registerNotificationType({
  ...
  async getMessage({ documentType, documentId }: GetMessageProps) {
    ...
    return (
      (await commentGetAuthorName(document)) +
      ' left a new comment on "' +
      (await getCommentParentTitle(document)) +
      '"'
    );
  },
  getIcon() {
...
export const NewDialogueMessagesNotification = registerNotificationType({
  ...
  getLink: ({ documentId }: { documentId: string | null }): string => {
...
export const NewDebateCommentNotification = registerNotificationType({
  ...
  async getMessage({ documentType, documentId }: GetMessageProps) {
    ...
    return (
      (await commentGetAuthorName(document)) +
      ' left a new reply on the dialogue "' +
      (await getCommentParentTitle(document)) +
      '"'
    );
  },
...
export const NewMessageNotification = registerNotificationType({
  ...
  async getMessage({ documentType, documentId }: GetMessageProps) {
    ...
    return (
      (await Users.findOne(document.userId))?.displayName +
      " sent you a new message" +
      (conversation?.title ? " in the conversation " + conversation.title : "") +
      "!"
    );
  },
  getIcon() {
...
export const WrappedNotification = registerNotificationType({
  ...
  allowedChannels: ["onsite", "email", "both"],
  async getMessage() {
    return "Check out your FAST Forum 2023 Wrapped";
  },
  getIcon() {
