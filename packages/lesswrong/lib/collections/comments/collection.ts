...
export const commentMutationOptions: MutationOptions<DbComment> = {
  ...
  editCheck: (user: DbUser | null, document: DbComment | null) => {
    ...
    if (userCanDo(user, "comments.alignment.move.all") || userCanDo(user, "comments.alignment.suggest")) {
      return true;
    }
...
makeEditable({
  ...
  options: {
    ...
    getLocalStorageId: (comment, name) => {
      if (comment._id) {
        return { id: comment._id, verify: true };
      }
      if (comment.parentCommentId) {
        return { id: "parent:" + comment.parentCommentId, verify: false };
      }
