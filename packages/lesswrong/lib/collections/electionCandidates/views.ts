...
const electionCandidateSortOptions = new TupleSet(["mostPreVoted", "name", "recentlyAdded"] as const);

export const isElectionCandidateSort = (value: string): value is ElectionCandidatesSort =>
  electionCandidateSortOptions.has(value);
...
ElectionCandidates.addDefaultView(({ electionName, sortBy }: ElectionCandidatesViewTerms) => {
  return {
    selector: {
      electionName,
