...
const membersActions = ["reviewVotes.new", "reviewVotes.view.own"];
membersGroup.can(membersActions);

const sunshineRegimentActions = ["reviewVotes.edit.all", "reviewVotes.remove.all", "reviewVotes.view.all"];
sunshineRegimentGroup.can(sunshineRegimentActions);

ReviewVotes.checkAccess = async (
  user: DbUser | null,
  document: DbReviewVote,
  context: ResolverContext | null,
): Promise<boolean> => {
  if (!user || !document) return false;
  return document.userId === user._id
    ? userCanDo(user, "reviewVotes.view.own")
    : userCanDo(user, `reviewVotes.view.all`);
};
