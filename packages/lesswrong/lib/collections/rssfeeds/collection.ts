...
const options: MutationOptions<DbRSSFeed> = {
  ...
  editCheck: (user: DbUser | null, document: DbRSSFeed | null) => {
    if (!document || !user) return false;
    return userOwns(user, document) ? userCanDo(user, "rssfeeds.edit.own") : userCanDo(user, "rssfeeds.edit.all");
  },

  removeCheck: (user: DbUser | null, document: DbRSSFeed | null) => {
    if (!document || !user) return false;
    return userOwns(user, document) ? userCanDo(user, "rssfeeds.remove.own") : userCanDo(user, "rssfeeds.edit.all");
  },
};
