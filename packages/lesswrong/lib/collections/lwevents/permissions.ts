...
const membersActions = ["events.new.own", "events.view.own"];
membersGroup.can(membersActions);

const adminActions = ["events.new", "events.edit.all", "events.remove.all", "events.view.all"];
adminsGroup.can(adminActions);

LWevents.checkAccess = async (
  user: DbUser | null,
  document: DbLWEvent,
