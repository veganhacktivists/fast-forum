...
const membersActions = ["reports.new", "reports.view.own"];
membersGroup.can(membersActions);

const sunshineRegimentActions = ["reports.new", "reports.edit.all", "reports.remove.all", "reports.view.all"];
sunshineRegimentGroup.can(sunshineRegimentActions);

Reports.checkAccess = async (
  user: DbUser | null,
  document: DbReport,
  context: ResolverContext | null,
): Promise<boolean> => {
  if (!user || !document) return false;
  return document.userId === user._id ? userCanDo(user, "reports.view.own") : userCanDo(user, `reports.view.all`);
};
