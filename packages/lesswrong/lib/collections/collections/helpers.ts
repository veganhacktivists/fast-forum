...
export const collectionGetAllPostIDs = async (
  collectionID: string,
  ...
  const postsBySequence = toDictionary(
    sequencePostsPairs,
    (pair) => pair[0],
    (pair) => pair[1],
  );
  ...
  const posts = _.flatten(
    books.map((book) => {
      const postsInSequencesInBook = _.flatten(_.map(book.sequenceIds, (sequenceId) => postsBySequence[sequenceId]));
      if (book.postIds) return _.union(book.postIds, postsInSequencesInBook);
      else return postsInSequencesInBook;
    }),
  );
  return posts;
};
