...
export const setKarmaInflationSeries = (newKarmaInflationSeries: TimeSeries) => {
  karmaInflationCache = newKarmaInflationSeries;
};
...
export const timeSeriesIndexExpr = (cmpField: string, startTimestamp: number, interval: number) => {
  return {
    $floor: {
      $divide: [
        {
          $subtract: [cmpField, new Date(startTimestamp)],
        },
        interval,
      ],
    },
