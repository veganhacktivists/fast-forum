...
const membersActions = ["posts.new", "posts.edit.own", "posts.remove.own", "posts.upvote", "posts.downvote"];
...
Posts.checkAccess = async (
  ...
  if (post.onlyVisibleToLoggedIn && !currentUser) {
    if (outReasonDenied) outReasonDenied.reason = "This post is only visible to logged-in users.";
    return false;
  }
  if (userCanDo(currentUser, "posts.view.all")) {
    ...
    userIsSharedOn(currentUser, post) ||
    (await userIsPostGroupOrganizer(currentUser, post, context))
  ) {
    return true;
  } else if (
    !currentUser &&
    !!canonicalLinkSharingKey &&
    constantTimeCompare({ correctValue: canonicalLinkSharingKey, unknownValue: unvalidatedLinkSharingKey })
  ) {
    return true;
...
const votingActions = ["posts.smallDownvote", "posts.bigDownvote", "posts.smallUpvote", "posts.bigUpvote"];
