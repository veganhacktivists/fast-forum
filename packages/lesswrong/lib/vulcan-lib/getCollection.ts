...
export const getCollection = <N extends CollectionNameString>(name: N): CollectionBase<N> => {
  if (name in collectionsByName && collectionsByName[name]) return collectionsByName[name] as CollectionBase<N>;
  ...
  if (!collection) throw new Error("Invalid collection name: " + name);
...
export const getCollectionByTableName = (tableName: string): CollectionBase<any> => {
  if (tableName in collectionsByLowercaseName) return (collectionsByLowercaseName as AnyBecauseTodo)[tableName];
  throw new Error(`Invalid table name: ${tableName}`);
};

export const isValidCollectionName = (name: string): name is CollectionNameString => {
  if (name in collectionsByName) return true;
