...
declare global {
  ...
  // NotEqual.
  type TypesEqual<A, B, Equal, NotEqual> = A extends B ? (B extends A ? Equal : NotEqual) : NotEqual;

  type NonAnyFieldsOfType<Interface, Type> = {
    [K in keyof Interface]: IfAny<Interface[K], never, TypesEqual<Interface[K], Type, K, never>>;
  }[keyof Interface];

  type ReplaceFieldsOfType<Interface, FieldType, ReplacementType> = {
    [K in keyof Interface]: IfAny<
      Interface[K],
      any,
  ...
  type NameOfFieldWithType<ObjType, FieldName, FieldType> = FieldName extends keyof ObjType
