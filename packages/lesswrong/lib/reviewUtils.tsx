...
export function postIsVoteable(post: PostsBase) {
  return (
    getReviewPhase() === "NOMINATIONS" || post.positiveReviewVoteCount >= REVIEW_AND_VOTING_PHASE_VOTECOUNT_THRESHOLD
  );
}
...
export const getCostData = ({ costTotal = 500 }: { costTotal?: number }): Record<number, CostData> => {
  ...
  const overSpentWarning =
    divider !== 1 ? (
      <div>
        <em>Your vote is downweighted because you spent 500+ points</em>
      </div>
    ) : null;
  return {
