...
const commentsTopView: CommentsViewName = isAF ? "afPostCommentsTop" : "postCommentsTop";
...
const getCommentViewNames = (options?: CommentViewsConfig): CommentsViewName[] => [
  ...defaultViews,
  ...(options?.includeAdminViews ? adminViews : []),
  ...(isAF ? afViews : []),
];

export const getCommentViewOptions = (options?: CommentViewsConfig): { value: CommentsViewName; label: string }[] =>
  getCommentViewNames(options).map((view) => ({
    value: view,
    label: customViewNames[view] ?? view,
  }));

export const isValidCommentView = (name: string, options?: CommentViewsConfig): name is CommentsViewName =>
