...
export const voteCallbacks = {
  cancelSync: new CallbackChainHook<VoteDocTuple, [CollectionBase<VoteableCollectionName>, DbUser]>(
    "votes.cancel.sync",
  ),
  cancelAsync: new CallbackHook<[VoteDocTuple, CollectionBase<VoteableCollectionName>, DbUser]>("votes.cancel.async"),
  castVoteSync: new CallbackChainHook<VoteDocTuple, [CollectionBase<VoteableCollectionName>, DbUser]>(
    "votes.castVote.sync",
  ),
  castVoteAsync: new CallbackHook<[VoteDocTuple, CollectionBase<VoteableCollectionName>, DbUser, ResolverContext]>(
    "votes.castVote.async",
  ),
};
...
const addVoteClient = ({
  ...
  const newDocument = {
    ...
    afExtendedScore: isAfVote
      ? votingSystem.addVoteClient({
          voteType: voteType,
          document,
...
const cancelVoteClient = ({
  ...
  if (!document.currentUserVote) return document;
...
export const setVoteClient = async ({
  document,
  collection,
  voteType = "neutral",
  extendedVote = null,
  ...
  if (
    ...
    (!extendedVote &&
      voteType &&
      voteType !== "neutral" &&
      !userCanDo(user, `${collectionName.toLowerCase()}.${voteType}`))
  ) {
