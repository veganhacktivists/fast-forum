...
export const addNameToExistingReactKarmaThreshold = new DatabasePublicSetting(
  "reacts.addNameToExistingReactKarmaThreshold",
  20,
);
export const downvoteExistingReactKarmaThreshold = new DatabasePublicSetting(
  "reacts.downvoteExistingReactKarmaThreshold",
  20,
);
...
registerVotingSystem<NamesAttachedReactionsVote, NamesAttachedReactionsScore>({
  ...
  isAllowedExtendedVote: (
    ...
    if (extendedVote?.reacts && extendedVote.reacts.length > 0) {
      ...
      if (
        userKarma < downvoteExistingReactKarmaThreshold.get() &&
        some(extendedVote.reacts, (r) => r.vote === "disagreed")
      ) {
        return {
          allowed: false,
          reason: `You need at least ${downvoteExistingReactKarmaThreshold.get()} karma to antireact`,
        };
      }
      ...
      if (userKarma < addNameToExistingReactKarmaThreshold.get()) {
        return {
          allowed: false,
          reason: `You need at least ${addNameToExistingReactKarmaThreshold.get()} karma to use reacts`,
        };
      }
      ...
      if (userKarma < addNewReactKarmaThreshold.get()) {
        for (let reaction of extendedVote.reacts) {
          if (!(reaction.react in oldExtendedScore.reacts)) {
            return {
              allowed: false,
              reason: `You need at least ${addNewReactKarmaThreshold.get()} karma to be the first to use a new react on a given comment`,
            };
          }
        }
      }
  ...
  isNonblankExtendedVote: (vote: DbVote) => {
    return (
      (vote?.extendedVoteType?.agreement && vote.extendedVoteType.agreement !== "neutral") ||
  ...
  getPostHighlights: ({ post, voteProps }: { post: PostsBase; voteProps: VotingProps<VoteableTypeClient> }) => {
    return getDocumentHighlights(voteProps);
  },
});
...
function getDocumentHighlights(
  ...
  for (let quote of Object.keys(reactionsByQuote)) {
    result[quote] = ({ children }: { children: React.ReactNode }) => (
      <InlineReactHoverableHighlight quote={quote} reactions={reactionsByQuote[quote]} voteProps={voteProps}>
        {children}
...
function addReactsVote(
  ...
  if (voteReacts) {
    for (let reaction of voteReacts) {
      ...
      else updatedReactions[reaction.react] = [userInfoWithType];
    }
  }
  return updatedReactions;
}

function removeReactsVote(
  old: NamesAttachedReactionsList | undefined,
  currentUser: UsersCurrent,
): NamesAttachedReactionsList {
  let updatedReactions: NamesAttachedReactionsList = old
    ? mapValues(old, (reactionsByType: UserReactInfo[]) =>
...
export function reactionsListToDisplayedNumbers(
  reactions: NamesAttachedReactionsList | null,
  currentUserId: string | undefined,
): { react: EmojiReactName; numberShown: number }[] {
  if (!reactions) return [];
...
export function getNormalizedReactionsListFromVoteProps(
  voteProps: VotingProps<VoteableTypeClient>,
): NamesAttachedReactionsScore | undefined {
  ...
  let normalizedReacts: NamesAttachedReactionsList = mapValues(extendedScore.reacts, (reactsList) =>
    normalizeReactsList(reactsList),
  );
  return {
    ...extendedScore,
...
export function getNormalizedUserVoteFromVoteProps(
  voteProps: VotingProps<VoteableTypeClient>,
): NamesAttachedReactionsVote | undefined {
