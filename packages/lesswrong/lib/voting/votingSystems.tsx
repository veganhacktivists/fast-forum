...
registerVotingSystem({
  ...
  addVoteClient: ({
    ...
    oldExtendedScore: AnyBecauseTodo;
    extendedVote: AnyBecauseTodo;
    currentUser: UsersCurrent;
  }): AnyBecauseTodo => {
    return null;
  },
...
registerVotingSystem({
  ...
  addVoteClient: ({
    ...
    const axisScores = fromPairs(
      reactBallotAxisNames.map((axis) => {
    ...
    const standaloneReactCounts = fromPairs(
      reactBallotStandaloneReactionNames.map((reaction) => {
  ...
  isNonblankExtendedVote: (vote: DbVote) => {
    ...
    for (let key of Object.keys(vote.extendedVoteType)) {
      if (vote.extendedVoteType[key] && vote.extendedVoteType[key] !== "neutral") return true;
    }
    return false;
  },
});
...
registerVotingSystem({
  ...
  addVoteClient: ({
    ...
    const emojiReactCounts = fromPairs(
      emojiReactionNames.map((reaction) => {
        const hasReaction = !!extendedVote?.[reaction];
        return [reaction, (oldExtendedScore?.[reaction] || 0) + (hasReaction ? 1 : 0)];
      }),
    );
...
const getEmojiReactionPower = (value?: boolean) => (value === true ? 1 : 0);
...
registerVotingSystem<{ preVote: boolean }, { preVoteCount: number }>({
  ...
  isAllowedExtendedVote: (user) => {
    try {
      ...
      return {
        allowed: false,
        reason: 'Pre-voting has closed. Click "Vote in the Election" above if you would like to cast your real vote',
      };
    } catch (e) {
      return {
        allowed: false,
  ...
  addVoteClient: ({
    ...
    const oldPreVoteCount = oldExtendedScore && "preVoteCount" in oldExtendedScore ? oldExtendedScore.preVoteCount : 0;
...
export function getVotingSystemByName(name: string): VotingSystem {
  if (votingSystems[name]) return votingSystems[name]!;
  else return getDefaultVotingSystem();
}
...
export async function getVotingSystemNameForDocument(
  document: VoteableType,
