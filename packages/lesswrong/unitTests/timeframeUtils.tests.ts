...
describe("getDateRange", () => {
  ...
  it("handles reversed start and end dates", () => {
    ((() => getDateRange("2019-01-03", "2019-01-01", "day")) as any).should.throw(
      Error,
      /got after .* after the before/,
    );
  });

  it("handles malformed dates", () => {
    ((() => getDateRange("01/01/2019", "2019-01-03", "day")) as any).should.throw(Error, /Invalid after/);
  });

  it("handles malformed timeBlock", () => {
    ((() => getDateRange("2019-01-01", "2019-01-03", "asdf" as any)) as any).should.throw(Error, /Invalid timeBlock/);
  });

  it("handles null timeBlock", () => {
    ((() => getDateRange("2019-01-01", "2019-01-03", null as any)) as any).should.throw(Error, /Invalid timeBlock/);
  });
});
