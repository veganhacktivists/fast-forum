...
function mockRequest(props: { ip?: string; userAgent?: string; userId?: string }) {
  const { ip = generateMockIpAddress(), userAgent = generateMockUserAgent(), userId } = props;
...
describe("PriorityBucketQueue", () => {
  ...
  it("should deprioritize a burst of traffic from a single IP", () => {
    ...
    for (let i = 0; i < threshold; i++) {
      ...
      expect(currentNextResult).toEqual(
        expect.objectContaining<RequestWithPriority<RequestData>>({
          request: requestsFromIP1[0],
          preOpPriority: expectedPriority,
        }),
      );
    }
  ...
  it("should deprioritize a burst of traffic from a single user agent", () => {
    ...
    expect(queue.peek()).toEqual(
      expect.objectContaining<RequestWithPriority<RequestData>>({ request: thirdRequest, preOpPriority: 1 }),
    );
  ...
  it("should penalize crossing the IP threshold as much as crossing the user-agent plus being logged out", () => {
    ...
    expect(queue.dequeue()).toMatchObject<PartialDeep<RequestWithPriority<RequestData>>>({
