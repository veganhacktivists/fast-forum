...
describe("UpdateQuery", () => {
  runTestCases([
    ...
    },
    {
      ...
      expectedSql:
        'UPDATE "TestCollection" SET "b" = $1::TEXT WHERE _id IN ( SELECT "_id" FROM "TestCollection" LIMIT $2 FOR UPDATE) RETURNING "_id"',
      expectedArgs: ["test", 1],
    },
    {
      ...
      expectedSql:
        'UPDATE "TestCollection" SET "b" = $1::TEXT WHERE _id IN ( SELECT "_id" FROM "TestCollection" WHERE "a" = $2 LIMIT $3 FOR UPDATE ) RETURNING "_id"',
      expectedArgs: ["test", 3, 1],
    },
    {
      name: "can build update that returns the result",
      getQuery: () =>
        new UpdateQuery<DbTestObject>(testTable, { a: 3 }, { $set: { b: "test" } }, {}, { returnUpdated: true }),
