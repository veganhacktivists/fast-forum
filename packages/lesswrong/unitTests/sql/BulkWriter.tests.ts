...
describe("BulkWriter", () => {
  it("insertOne creates an InsertQuery", () => {
    const queries = new BulkWriter(testTable, [{ insertOne: { document: {} as DbObject } }]).getQueries();
    expect(queries).toHaveLength(1);
    expect(queries[0] instanceof InsertQuery).toBe(true);
  });
  it("updateOne creates an UpdateQuery", () => {
    const queries = new BulkWriter(testTable, [{ updateOne: { filter: {}, update: {} } }]).getQueries();
    expect(queries).toHaveLength(1);
    expect(queries[0] instanceof UpdateQuery).toBe(true);
  });
  it("updateMany creates an UpdateQuery", () => {
    const queries = new BulkWriter(testTable, [{ updateMany: { filter: {}, update: {} } }]).getQueries();
    expect(queries).toHaveLength(1);
    expect(queries[0] instanceof UpdateQuery).toBe(true);
  });
  it("updateMany cannot use upserts", () => {
    expect(() => new BulkWriter(testTable, [{ updateMany: { filter: {}, update: {}, upsert: true } }])).toThrowError(
      "Upsert not implemented for multi-updates",
    );
  });
  it("deleteOne creates a DeleteQuery", () => {
    const queries = new BulkWriter(testTable, [{ deleteOne: { filter: { a: 3 } } }]).getQueries();
    expect(queries).toHaveLength(1);
    expect(queries[0] instanceof DeleteQuery).toBe(true);
  });
  it("deleteMany creates a DeleteQuery", () => {
    const queries = new BulkWriter(testTable, [{ deleteMany: { filter: { a: 3 } } }]).getQueries();
    expect(queries).toHaveLength(1);
    expect(queries[0] instanceof DeleteQuery).toBe(true);
  });
  ...
  it("replaceOne is not implemented", () => {
    expect(() => new BulkWriter(testTable, [{ replaceOne: { filter: {}, replacement: {} as DbObject } }])).toThrowError(
      "replaceOne in bulkWrite not implemented",
    );
  });
  it("invalid operations throw an error", () => {
    expect(
      () => new BulkWriter(testTable, [{ anInvalidOperation: {} } as unknown as MongoBulkWriteOperation<DbObject>]),
