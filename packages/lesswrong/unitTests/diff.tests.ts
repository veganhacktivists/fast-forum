...
describe("diffHtml", () => {
  ...
  it("represents changes with ins and del tags", () => {
    ...
    for (const testCase of tests) {
      chai.assert.equal(
        normalizeHtml(diffHtml(testCase.before, testCase.after, false)),
        normalizeHtml(testCase.differences),
      );
    }
  });
  ...
  it("trims unchanged sections correctly", () => {
    const tests = [
      {
        untrimmed:
          "<p>This paragraph is not changed</p><p><ins>This is inserted</ins></p><p><del>This is deleted</del></p>",
