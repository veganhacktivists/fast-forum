...
class SimpleBoxEditing extends Plugin {
	...
	init() {
		...
		this.editor.keystrokes.set("Ctrl+Enter", (evt, cancel) => {
			...
			const cursorOnCommentBox = selectedBlocks.some(
				block =>
					block.is( "element", 'dialogueMessageInput') ||
					block.getAncestors().some( ( ancestor ) => ancestor.is( "element", 'dialogueMessageInput') ),
			);
		...
		this.listenTo(viewDocument, "delete", (evt, data) => {
			...
			if (!deletedElement || !deletedElement.parent) {
				return;
			}

			const messageParent = deletedElement
				.getAncestors()
				.find((ancestor) => ancestor.is("element", "dialogueMessage"));
			if (!messageParent) {
				return;
			}
	...
	_defineConverters() {
		...
		conversion.for("dataDowncast").elementToElement({
			...
			view: getDataDowncastViewGenerator("dialogue-message-input", ["user-id", "user-order", "display-name"]),
		});
		conversion.for("editingDowncast").elementToElement({
			model: "dialogueMessageInput",
...
class SubmitDialogueMessageCommand extends Command {
	execute() {
		...
		model.change((writer) => {
			...
			const dialogueMessageInput = findMessageInputs(root).find(
				(node) => node.getAttribute("user-id") === userId,
			);
			...
			if (dialogueMessageInput) {
				...
				const messageAttributes = {
					...
					"submitted-date": new Date().toUTCString(),
				};
...
function findMessageInputs(root) {
	 ...
	 * @type {Element}
	 */
	const dialogueMessageInputWrapper = rootChildren.find((child) =>
		child.is("element", "dialogueMessageInputWrapper"),
	);

	if (!dialogueMessageInputWrapper) {
		return rootInputs;
	}

	const wrapperInputs = Array.from(dialogueMessageInputWrapper.getChildren()).filter((child) =>
		child.is("element", "dialogueMessageInput"),
	);
...
function getDataDowncastViewGenerator(className, attributeList) {
	return (modelElement, { writer: viewWriter }) => {
		const sectionAttributes = Object.fromEntries(
			attributeList.map((attribute) => [attribute, modelElement.getAttribute(attribute)]),
		);
...
function inputEditingDowncastViewGenerator(modelElement, { writer: viewWriter }) {
	...
	const sectionAttributes = {
		class: "dialogue-message-input ContentStyles-debateResponseBody",
